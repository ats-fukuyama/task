...
! Revision History
! ----------------
!Oct 16, 2012   Tariq Rafiq
!               V7.1.1  (minor bug fix in DRIBM model)
...
zami(6,5) = - 0.00054_R8 * vef * zflh / betaein
zbmr(6,5) = 1._R8 + 2._R8*0.00054_R8*zflh / betaein
zbmr(6,6) = - 0.00054_R8
...
! Loop over number of possible modes
   ...
   Do j=1,neq
      ...
      If ( zgamma(j)>zepsqrt .and. ztemp1>zepsqrt ) Then
         ...
         !
         !.. flux from j:th mode
         zflxph = 4._R8* zphsph * max(0._R8,zgamma(j)-abs(wexb))**2
         !AHK    zflxph = 2._R8* zphsph * max(0._R8,zgamma(j)-abs(wexb))**2
         !
         ...
         !
         !.. Flux from j:th mode
         zflxnh = 4._R8* zphsnh * max(0._R8,zgamma(j)-abs(wexb))**2
         !AHK  zflxnh = 2._R8* zphsnh * max(0._R8,zgamma(j)-abs(wexb))**2
         !
         !.. Flux summed over all unstable modes
         ...
         zphspe = - ( zimag * DBLE(zevec(1,j)) &
                  -  zreal * DIMAG(zevec(1,j)) ) / ztemp1
         !
         zflxpe = 4._R8* zphspe * max(0._R8,zgamma(j)-abs(wexb))**2
         !AHK  zflxpe = 2._R8* zphspe * max(0._R8,zgamma(j)-abs(wexb))**2
