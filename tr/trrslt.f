...
C 
      ...
      GVT(NGT,91) = GCLIP(PRFVT(1,1)) ! ECH  to electron
      GVT(NGT,92) = GCLIP(PRFVT(2,1)) ! ECH  to ions
      GVT(NGT,93) = GCLIP(PRFVT(1,2)) ! LH   to electron
      GVT(NGT,94) = GCLIP(PRFVT(2,2)) ! LH   to ions
      GVT(NGT,95) = GCLIP(PRFVT(1,3)) ! ICRH to electron
      GVT(NGT,96) = GCLIP(PRFVT(2,3)) ! ICRH to ions
C
C     *** FOR 3D ***
C
      IF(RHOA.NE.1.D0) NRMAX=NROMAX
...
C         ENDIF
         ...
         G3D(NR,NGT,33) = GCLIP(ZEFF(NR))
         G3D(NR,NGT,34) = GCLIP(AK(NR,1))
         G3D(NR,NGT,35) = GCLIP(AK(NR,2))
C
         G3D(NR,NGT,36) = GCLIP(PRFV(NR,1,1))
         G3D(NR,NGT,37) = GCLIP(PRFV(NR,1,2))
         G3D(NR,NGT,38) = GCLIP(PRFV(NR,1,3))
         G3D(NR,NGT,39) = GCLIP(PRFV(NR,2,1))
         G3D(NR,NGT,40) = GCLIP(PRFV(NR,2,2))
         G3D(NR,NGT,41) = GCLIP(PRFV(NR,2,3))
C
         G3D(NR,NGT,42) = GCLIP(AJRFV(NR,1))
         G3D(NR,NGT,43) = GCLIP(AJRFV(NR,2))
         G3D(NR,NGT,44) = GCLIP(AJRFV(NR,3))
C
         G3D(NR,NGT,45) = GCLIP(RW(NR,1)+RW(NR,2))
         G3D(NR,NGT,46) = GCLIP(ANC(NR)+ANFE(NR))
         G3D(NR,NGT,47) = GCLIP(BP(NR))
C
         G3D(NR,NGT,48) = GCLIP(RMJRHO(NR))
         G3D(NR,NGT,49) = GCLIP(RMNRHO(NR))
         RMN=(DBLE(NR)-0.5D0)*DR
         CALL SPL1DI(RMN,F0D,RM,U,U0,NRMAX,IERR)
         G3D(NR,NGT,50) = GCLIP(F0D)
         G3D(NR,NGT,51) = GCLIP(EKAPPA(NR))
         G3D(NR,NGT,52) = GCLIP(1.D0) ! DELTAR
         G3D(NR,NGT,53) = GCLIP(AR1RHO(NR))
         G3D(NR,NGT,54) = GCLIP(AR2RHO(NR))
C         IF(NT.EQ.0) THEN
C            G3D(NR,NGT,55) = GCLIP(RTU(NR,2,1))
C         ELSE
C            G3D(NR,NGT,55) = GCLIP(RTU(NR,2,NT))
C         ENDIF
C
      ENDDO
      IF(RHOA.NE.1.D0) NRMAX=NRAMAX
