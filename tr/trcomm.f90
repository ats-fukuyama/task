...
! TRPRM
  REAL(8)   :: &
       ...
       SYNCABS, SYNCSELF, SPT0, SPT1, KAI0, KAIPED0, KAIPED1, &
		 AD1, AD2, PELT0, PELT1, PELITV, PELRAT, NHE0
  REAL(8), DIMENSION(3) :: &
       ALP
  REAL(8), DIMENSION(8) :: &
       CDW
  REAL(8), DIMENSION(NSMM) :: &
       PA,PZ,PN,PNS,PT,PTS,PU,PUS,PNS0,PNS1
  INTEGER(4):: &
...
!     ******************************************************
  ...
  SUBROUTINE ALLOCATE_TRCOMM(ierr)
    ...
    ALLOCATE(POH(NRMAX),PRL(NRMAX),PCX(NRMAX),PIE(NRMAX),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(SIE(NRMAX),SCX(NRMAX),TSIE(NRMAX),TSCX(NRMAX),SPT(NRMAX),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
 ...
 900 continue
  ...
  SUBROUTINE DEALLOCATE_TRCOMM
    ...
    DEALLOCATE(SIE,SCX,TSIE,TSCX,PIN,SSIN,PBCL,SPE,SPT)
  ...
  SUBROUTINE DEALLOCATE_ERR_TRCOMM
    ...
    IF(ASSOCIATED(SPT      ))     DEALLOCATE(SPT      )
