...
! TRPRM
 ...
 &     CDH, CNN, CWEB, DT, EPSLTR, CHP, CK0, CK1, CKALFA, CKBETA, CKGUMA, CNB, CALF, CSPRS, TSST
  ...
  REAL(8), DIMENSION(:),POINTER :: PA,PZ,PN,PNS,PT,PTS                            ! (NSTM)
...
! TRMDL
  ...
  REAL(8), DIMENSION(:),POINTER :: PELPAT                                        ! (NSTM)
  INTEGER(4):: MDLNB, MDLEC, MDLLH, MDLIC, MDLCD, MDLPEL, MDLJBS, MDLST, MDLNF, IZERO

!     ****** CONTROL VARIABLES ******
!TRCTL
  REAL(4)   :: GTCPU1
  REAL(8)   :: T, TST, TPRE, WPPRE, RIP, DR, FKAP, RHOA, RIPA, VSEC, RDPS,DIPDT
  REAL(8), DIMENSION(:),POINTER :: PNSS                                          ! (NSTM)
  INTEGER(4)::   NT, NRAMAX, NROMAX, NREDGE, NTMAX_SAVE, IREAD

!     ****** MATRIX VARIBALES ******
! TRMTX
  REAL(8), DIMENSION(:,:),POINTER :: XV                                  ! (NVM,NRM)
  REAL(8), DIMENSION(:,:),POINTER :: YV, AY, Y                           ! (NFM,NRM)
  REAL(8), DIMENSION(:,:),POINTER :: ZV, AZ, Z                           ! (NSM,NRM)
  REAL(8), DIMENSION(:,:),POINTER :: AX                                  ! (LDAB,MLM)
  REAL(8), DIMENSION(:)  ,POINTER :: X                                   ! (MLM)
!
!     ****** FUNDAMENTAL VARIABLES ******
! TRVAR
  REAL(8), DIMENSION(:)  ,POINTER :: RG, RM, RHOM, RHOG, BP, RDP, RPSI   ! (NRM)
  REAL(8), DIMENSION(:,:),POINTER :: RN, RT, RU                          ! (NRM,NSTM)
  REAL(8), DIMENSION(:,:),POINTER :: RW                                  ! (NRM,NFM)

!     ****** PROFILE VARIABLES ******
! TRPFL
  REAL(8), DIMENSION(:,:),POINTER :: RNF, RTF                            ! (NRM,NFM)
  REAL(8), DIMENSION(:)  ,POINTER :: ANC, ANFE, ANNU, ZEFF, PZC, PZFE, BETA, BETAP, &
 &     BETAL, BETAPL, BETAQ, PBM, PADD, VTOR, VPAR, VPRP, VPOL, WROT, ER, VEXB, WEXB, AGMP   ! (NRM)

!     ****** SOURCE VARIABLES ******
! TRSRC
  REAL(8), DIMENSION(10)      :: RTG
  REAL(8), DIMENSION(:)    ,POINTER :: AJ, AJOH, EZOH, QP, AJTOR, AJNB, AJRF, AJBS, PNB, SNB, &
 &     PBIN, PNF, SNF, PFIN, POH, PRL, PCX, PIE, SIE, SCX, TSIE, TSCX        ! (NRM)
  REAL(8), DIMENSION(:,:)  ,POINTER :: PIN, SSIN, PBCL, PFCL, PRF, SPE   ! (NRM,NSTM)
  REAL(8), DIMENSION(:,:)  ,POINTER :: RGFLX                             ! (NRM,NSM)
  REAL(8), DIMENSION(:,:)  ,POINTER :: AJRFV                             ! (NRM,3)
  REAL(8), DIMENSION(:,:,:),POINTER :: PRFV                              ! (NRM,NSTM,3)

!     ****** COEFFICIENT VARIABLES ******
! TRCEF
  REAL(8), DIMENSION(:)  ,POINTER :: ETA, S, ALPHA, RKCV, TAUB, TAUF, TAUK                                     ! (NRM)
  REAL(8), DIMENSION(:,:),POINTER :: AK, AVK, AD, AV, AKNC, AKDW, ADNC, ADDW, AVNC, AVDW, AVKNC, AVKDW ! (NRM,NSTM)
  REAL(8), DIMENSION(:,:),POINTER :: VGR1, VGR2, VGR3, VGR4                                            ! (NRM,4)
...
! TRGLB
 ...
 &     RPE, Q0, ZEFF0, QF, WPDOT, TAUE1, TAUE2, TAUE89, TAUE98, H98Y2, BETAP0, BETAPA, BETA0, BETAA, BETAQ0, BETAN
  ...
  REAL(8), DIMENSION(:)  ,POINTER :: ANS0, TS0, ANSAV, ANLAV, TSAV, WST, PRFT, PBCLT, PFCLT, PLT, SPET, SLT  ! (NSTM)
  REAL(8), DIMENSION(:,:),POINTER :: PRFVT                                ! (NSTM,3)
...
! TRPLR
  ...
  REAL(4), DIMENSION(:,:,:),POINTER :: G3D                       ! (NRM,NGM,NCTM)
...
! TREQV1
  ...
  REAL(8), DIMENSION(:),POINTER :: &
       ...
       & PSITRHO, PSIPRHO, PPPRHO, PIQRHO, PIRHO, FACTQ   ! (NRM)
  REAL(8), DIMENSION(:),POINTER :: QRHO  ! (NRMP)
...
! TRADD
  ...
  REAL(8), DIMENSION(:)      ,POINTER :: RTM, AMZ, PEXT, PNSSA, PNSA, PTSA   ! (NSTM)
  REAL(8), DIMENSION(:,:)    ,POINTER :: PEX, SEX                            ! (NRM,NSTM)
  REAL(8), DIMENSION(:,:,:)  ,POINTER :: AKDWD, AKDWP, ADDWD, ADDWP          ! (NRM,NSM,NSM)
  REAL(8), DIMENSION(:,:,:,:),POINTER :: VV,DD                               ! (NVM,NVM,4,3)
  REAL(8), DIMENSION(:,:,:,:),POINTER :: VI,DI                               ! (NVM,NVM,2,3)
  INTEGER(4)                      :: NTEQIT
...
! TRMDS
 ...
 &     NSCMAX, NSTMAX, MDLWLD, MDDIAG, MDDW, MDLFLX, MDLER, MDCD05, MDEDGE
  INTEGER(4), DIMENSION(:)  ,POINTER :: NSS, NSV, NNS, NST                   ! (NEQM)
  INTEGER(4), DIMENSION(:,:),POINTER :: NEA                                  ! (0:NSTM,0:3)

!     ****** NCLASS ******
! TRNCL
  REAL(8), DIMENSION(:)    ,POINTER :: AJBSNC, ETANC, AJEXNC                               !  (NRM)
  REAL(8), DIMENSION(:,:)  ,POINTER :: CJBSP, CJBST, ADNCG, AVNCG                          !  (NRM,NSM)
  REAL(8), DIMENSION(:,:,:),POINTER :: AKNCP, AKNCT, ADNCP, ADNCT, AKLP, AKLD, ADLP, ADLD  !  (NRM,NSM,NSM)
  REAL(8), DIMENSION(:,:,:),POINTER :: RGFLS,   RQFLS                                      !  (NRM,5,NSM)
...
! TRSVU
  ...
  REAL(8), DIMENSION(:,:)  ,POINTER :: QPU,  AJU, AJNBU, BPU, PRLU, PECU, POHU, DVRHOU, AJBSU, ZEFFU, PBMU, &
 &     RNFU, WROTU, ZEFFU_ORG, RKPRHOU, RMJRHOU, RMNRHOU, ARRHOU, AR1RHOU, AR2RHOU, ABRHOU, TTRHOU, SWLU  ! (NTUM,NRMP)
  REAL(8), DIMENSION(:,:)  ,POINTER :: PTSU, PNSU, PTSUA, PNSUA                                       ! (NTUM,NSTM)
  REAL(8), DIMENSION(:,:,:),POINTER :: RNU, RTU, PNBU, PICU, SNBU,RNU_ORG                             ! (NTUM,NRMP,NSM)
...
!     ************
!TRPROF
  REAL(8), DIMENSION(:),POINTER ::  PNSSO,PTSO,PNSSAO,PTSAO                              ! (NSTM)
...
!    ALLOCATE(PNSS(NSTMAX) ,STAT=IERR)
    ...
    ALLOCATE(RHOTR(NRMAX),PRHO(NRMAX),HJRHO(NRMAX),VTRHO(NRMAX),TRHO(NRMAX) ,STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(TTRHO(NRMAX),DVRHO(NRMAX),RKPRHO(NRMAX),ABRHO(NRMAX),ABVRHO(NRMAX) ,STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(PSITRHO(NRMAX),PSIPRHO(NRMAX),PPPRHO(NRMAX),PIQRHO(NRMAX),PIRHO(NRMAX) ,STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ...
    ALLOCATE(EPSRHO(NRMAX),BPRHO(NRMAX),RMJRHO(NRMAX),RMNRHO(NRMAX) ,STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(TTRHOG(NRMAX),DVRHOG(NRMAX),RKPRHOG(NRMAX),ABRHOG(NRMAX),ABVRHOG(NRMAX) ,STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ...
    ALLOCATE(AIB2RHOG(NRMAX),ARHBRHOG(NRMAX),RMJRHOG(NRMAX),RMNRHOG(NRMAX) ,STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(RDPVRHOG(NRMAX),FACTQ(NRMAX),QRHO(NRMP) ,STAT=IERR)
      IF(IERR.NE.0) GOTO 900
...
!    DEALLOCATE(PA,PZ,PN,PNS,PT,PTS,PELPAT)
    ...
    DEALLOCATE(TTRHOG,DVRHOG,RKPRHOG,ABRHOG,ABVRHOG,ARRHOG,AR1RHOG,AR2RHOG,ABB2RHOG)
    DEALLOCATE(AIB2RHOG,ARHBRHOG,RMJRHOG,RMNRHOG,RDPVRHOG,FACTQ,QRHO)
...
!    IF(ASSOCIATED(PZ       ))     DEALLOCATE(PZ       )
!    IF(ASSOCIATED(PN       ))     DEALLOCATE(PN       )
!    IF(ASSOCIATED(PNS      ))     DEALLOCATE(PNS      )
!    IF(ASSOCIATED(PT       ))     DEALLOCATE(PT       )
!    IF(ASSOCIATED(PTS      ))     DEALLOCATE(PTS      )
!    IF(ASSOCIATED(PELPAT   ))     DEALLOCATE(PELPAT   )
    ...
    IF(ASSOCIATED(G3D      ))     DEALLOCATE(G3D      )
