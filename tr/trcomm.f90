...
! TRPRM
  REAL(8)   :: &
       ...
       PNNUS, PROFN1, PROFN2, PROFT1, PROFT2, PROFU1, PROFU2, PROFJ1, &
...
! TRMDL
  ...
  REAL(8), DIMENSION(:),POINTER :: & ! (NSTM)
       PELPAT
  INTEGER(4):: &
       MDLNB, MDLEC, MDLLH, MDLIC, MDLCD, MDLPEL, MDLJBS, MDLST, MDLNF, IZERO
...
! TRSRC
  REAL(8), DIMENSION(10)      :: &
       RTG
  REAL(8), DIMENSION(:),     POINTER :: & ! (NRM)
       AJ, AJOH, EZOH, QP, AJTOR, AJNB, AJRF, AJBS, PNB, SNB, &
...
! TRPLR
  ...
  REAL(4), DIMENSION(:,:,:),POINTER :: GRT                       ! (NRM,NGM,NCRTM)
...
!    ALLOCATE(PA(NSTMAX),PZ(NSTMAX),PN(NSTMAX),PNS(NSTMAX),&
!             PT(NSTMAX),PTS(NSTMAX),PELPAT(NSTMAX),STAT=IERR)
...
!    ALLOCATE(PNSS(NSTMAX),STAT=IERR)
    ...
    ALLOCATE(RG(NRMAX),RM(NRM),RHOM(NRMAX),RHOG(NRMAX),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ...
    ALLOCATE(GPNB(4*NRMAX,10),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(GVR(NRMP,NGM,NCGM),GRT(NRMAX,NGM,NCRTM),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
...
!    DEALLOCATE(PA,PZ,PN,PNS,PT,PTS,PELPAT)
    ...
    DEALLOCATE(GRM,GRG,GJB,GAD,GET,GAK,GYR,GER,GPNB,GVR,GRT)
...
!    IF(ASSOCIATED(PELPAT   ))     DEALLOCATE(PELPAT   )
    ...
    IF(ASSOCIATED(GRT      ))     DEALLOCATE(GRT      )
