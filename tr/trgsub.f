...
C
      AMP=1.D-3
      KFID='TE0'
      CALL UF1DG(KFID,GT,TMU,TE0  ,AMP,NGT,IERR)
      KFID='TI0'
      CALL UF1DG(KFID,GT,TMU,TI0  ,AMP,NGT,IERR)
      AMP=1.D-6
      KFID='WTOT'
      CALL UF1DG(KFID,GT,TMU,WTOT ,AMP,NGT,IERR)
      IF(MDLUF.EQ.3) AMP=1.D0
      KFID='IBOOT'
      CALL UF1DG(KFID,GT,TMU,RIBS ,AMP,NGT,IERR)
      IF(MDLUF.EQ.3) AMP=1.D-6
      KFID='IP'
      CALL UF1DG(KFID,GT,TMU,RIPL ,AMP,NGT,IERR)
      IF(MDLUF.EQ.3) THEN
         KFID='PLH'
         CALL UF1DG(KFID,GT,TMU,PICRH,AMP,NGT,IERR)
      ELSE
         KFID='PICRH'
         CALL UF1DG(KFID,GT,TMU,PICRH,AMP,NGT,IERR)
      ENDIF
      KFID='PNBI'
      CALL UF1DG(KFID,GT,TMU,PNBI ,AMP,NGT,IERR)
C
      CALL PAGES
C
      DO NG=1,NGT
