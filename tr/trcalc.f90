...
!     ***********************************************************
      ...
      USE TRCOMM, ONLY : AJBS, AJRF, ARRHOG, DT, DVRHOG, MDLEQ0, MDLEQB, MDLJBS, MDLUF, MDNCLS, MODELG, NRAMAX, NRM, NRMAX, &
     &                   NROMAX, NSM, NSMAX, PBCL, PBIN, PCX, PELTIM, PEX, PFCL, PFIN, PI, PIE, PIN, PN, PNB, PNF, POH, PRF,&
     &                   PRL, Q0, QP, RDP, RG, RHOA, SCX, SEX, SIE, SNB, SNF, SPE, SSIN, T, TTRHOG
      IMPLICIT NONE
...
!     *** RADIAL ELECTRIC FIELD ***
      ...
      DO NWR=1,NWRMAX
         ...
         PRF(1:NRMAX,1:NSMAX)=PRF(1:NRMAX,1:NSMAX)+PWRPL(1:NRMAX,1:NSMAX)
         AJRF(1:NRMAX)=AJRF(1:NRMAX)+AJWRPL(1:NRMAX)
      ENDDO

      DO NWM=1,NWMMAX
         CALL PLDATA_GETWM(NWM,KID,PWMPL,AJWMPL)
         PRF(1:NRMAX,1:NSMAX)=PRF(1:NRMAX,1:NSMAX)+PWMPL(1:NRMAX,1:NSMAX)
         AJRF(1:NRMAX)=AJRF(1:NRMAX)+AJWMPL(1:NRMAX)
      ENDDO
      ...
      DO NR=1,NRMAX
         ...
         ELSEIF(MDLEQ0.EQ.1) THEN
            ...
            SSIN(NR,8)=                                    SNB(NR)
         ENDIF
         PIN(NR,1)=PBCL(NR,1)+PFCL(NR,1)+PRF(NR,1)+POH(NR)-PRL(NR)-PIE(NR)+PEX(NR,1)
         PIN(NR,2)=PBCL(NR,2)+PFCL(NR,2)+PRF(NR,2)-PN(2)*PCX(NR)/(PN(2)+PN(3))+PEX(NR,2)
         PIN(NR,3)=PBCL(NR,3)+PFCL(NR,3)+PRF(NR,3)-PN(3)*PCX(NR)/(PN(2)+PN(3))+PEX(NR,3)
...
!     **********************************************************
      ...
      USE TRCOMM, ONLY : AEE, AMM, BB, BP, DR, EPSRHO, ER, MDLER, NRMAX, PA, PADD, PBM, PNSS, PTS, PZ, QP, RHOG, RHOM, &
     &                   RJCB, RKEV, RN, RNF, RT, SUMPBM, VPOL, VTOR
      IMPLICIT NONE
      INTEGER(4):: NR
      REAL(8)   :: ALPHA_NEO, CS, DPD, DRL, EPS, F_UNTRAP, RHO_S, RLNI, RLTI, TEL, TERM_DP, TIL
      REAL(8)   :: DERIV3P
...
!     ***********************************************************
     ...
     &                   MDLJQ, MDLUF, NRMAX, POH, RDP, RMU0, RR, TTRHO, TTRHOG
      IMPLICIT NONE
      INTEGER(4):: NR
...
!     ***********************************************************
 ...
 2000 IONE=NR
      ...
      IF(LQ.EQ.1) THEN
         ...
         RDP(1:IZEROX)=TTRHOG(1:IZEROX)*ARRHOG(1:IZEROX)*DVRHOG(1:IZEROX)/(4.D0*PI**2*QP(1:IZEROX))
