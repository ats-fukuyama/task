...
!     ***********************************************************
      ...
      USE TRCOMM, ONLY : ABRHOG, AJ, AJU, AMM, ANC, ANFE, ANNU, AR1RHOG, AX, AY, AZ, BB, BP, DR, DT, DVRHO, DVRHOG, EPSLTR,  &
     &                   LDAB, LMAXTR, MDLEQB, MDLEQE, MDLEQN, MDLPCK, MDLUF, MDTC, MLM, MODELG, NEQMAX, NFM, NGR, NGRSTP,   &
     &                   NGT, NGTSTP, NRAMAX, NRM, NRMAX, NROMAX, NSM, NSMAX, NSS, NST, NSV, NT, NTEQIT, NTMAX, NTSTEP, NTUM,&
     &                   PA, PZ, PZC, PZFE, Q0, QP, RDP, RG, RHOA, RIP, RIPE, RIPS, RIPU, RMU0, RN, RR, RT, RU, RW, T, TPRST,&
     &                   TST, TTRHO, TTRHOG, VLOOP, VSEC, X, XV, Y, YV, Z, ZV ,NEQMAXM
...
!     /* Matrix Solver */
      IF(MDLPCK.EQ.0) THEN
         ...
         IF(IERR.EQ.30000) THEN
            WRITE(6,*) 'XX ERROR IN TRLOOP : MATRIX AA IS SINGULAR  AT ',NT,' STEP.'
            GOTO 9000
         ENDIF
...
!     i.e. rho < rhoa
      ...
      ENDIF
      RIP=RIP+DIP
!      write(6,'(A,1P3E12.5)') "RIP,RIPE,DIP=",RIP,RIPE,DIP
