...
C
     ...
     &                       +RN(NR,2)*TISIML)*DVRHO(NR)*DR
            WEXP=WEXP+1.5D0*( RNU(NTL,NR,1)*TEEXPL
     &                       +RNU(NTL,NR,2)*TIEXPL)*DVRHO(NR)*DR
            WSIME=WSIME+1.5D0*RN(NR,1)*TESIML*DVRHO(NR)*DR
            WSIMI=WSIMI+1.5D0*RN(NR,2)*TISIML*DVRHO(NR)*DR
            WEXPE=WEXPE+1.5D0*RNU(NTL,NR,1)*TEEXPL*DVRHO(NR)*DR
            WEXPI=WEXPI+1.5D0*RNU(NTL,NR,2)*TIEXPL*DVRHO(NR)*DR
         ENDDO
      ELSE
         DO NR=1,NRMAX
            TESIML=RT(NR,1)
     ...
     &                       +RNU(NTL,NR,2)*TIEXPL)*DVRHO(NR)*DR
      ...
      WTO2E=(WSIME/WEXPE-1.D0)**2
      WTO2I=(WSIMI/WEXPI-1.D0)**2
C
...
C      WRITE(21,620) 'AVERW =' ,WTO1
C      WRITE(21,620) 'RW    =' ,WTO2
      WRITE(21,620) 'AVERW =' ,WTO2E
      WRITE(21,620) 'RW    =' ,WTO2I
