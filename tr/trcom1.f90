MODULE TRCOM1
  USE TRCOM0, ONLY : NRMAX, NRMU, NTUM, NEQMAXM
  IMPLICIT NONE
...
! TRERU
  REAL(8), DIMENSION(:,:),ALLOCATABLE :: RTEXU, RTIXU, RNEXU, RTEXEU, RTIXEU, RNEXEU   ! (NTUM,NRMU)
...
! TRUFC
  ...
  REAL(4),DIMENSION(:,:),ALLOCATABLE :: GRE   ! (NRM,2)
  ...
  SUBROUTINE ALLOCATE_TRCOM1(IERR)
    ...
    ALLOCATE(RTEXU(NTUM,NRMU), RTIXU(NTUM,NRMU), RNEXU(NTUM,NRMU),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(RTEXEU(NTUM,NRMU), RTIXEU(NTUM,NRMU), RNEXEU(NTUM,NRMU),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ...
    ALLOCATE(RD(NEQMAXM,NRMAX), PPA(NEQMAXM,NRMAX), PPB(NEQMAXM,NRMAX),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(PPC(NEQMAXM,NRMAX), GRE(NRMAX,2),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    RETURN
 900 CONTINUE
