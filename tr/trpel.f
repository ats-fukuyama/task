...
C
      FSUM = 0.D0
      DO 10 NR=1,NRMAX
         FSUM=FSUM+DEXP(-((RM(NR)-PELR0)/PELRW)**2)*RM(NR)
   10 CONTINUE
C
      FSUM=FSUM*2.D0*PI*RR*2.D0*PI*DR*RKAP
      S0  =PELTOT/FSUM
C
      DO 20 NR=1,NRMAX
         SPEL=S0*DEXP(-((RM(NR)-PELR0)/PELRW)**2)
      DO 20 NS=1,NSM
...
C
      IF(RP.LE.0.D0) THEN
         RPE=RA-DR*(NR-1)+DR*RPPRE/(RPPRE-RP)
         GOTO 9000
      ENDIF
C
      IF(NR.EQ.1) THEN
      ...
      ENDIF
C
      SPEL=ANS*RP*0.5D0*(RP+RPPRE)*RPDOT/(PI*RR*RM(NR)*RKAP*PELVEL)
      DO 10 NS=1,NSM
         SPE(NR,NS)=PELPAT(NS)*SPEL
   10 CONTINUE
C
...
C
      ...
      ENDIF
C
      SPEL=ANP*RP*0.5D0*(RP+RPPRE)*RPDOT/(PI*RR*RM(NR)*RKAP*PELVEL)
      DO 10 NS=1,NSM
         SPE(NR,NS)=PELPAT(NS)*SPEL
   10 CONTINUE
C
