...
!     ***********************************************************
      ...
      USE TRCOMM, ONLY : GT, G3D, GRM, RTU, NGT, NTM, NRM, NRMAX, MDLUF
      USE TRCOM1, ONLY : TMU, TMU1, NTXMAX, NTXMAX1, NTUM
      IMPLICIT NONE
      INTEGER(4), INTENT(IN):: INQ
      INTEGER(4) :: I, NR, IERR
      REAL(4),DIMENSION(NTM,NRM) :: GYL
      REAL(8) :: TSL, RTEL, RTDL
      REAL(4) :: GUCLIP
      CALL PAGES

      DO I=1,NGT
         GYL(I,1:NRMAX) = G3D(1:NRMAX,I, 1)
      ENDDO
      CALL TRGR1DC( 3.0,12.0,14.0,17.0,GT,GRM,GYL,NTM,NGT,NRM,NRMAX,'@TE [keV]  vs t@')

      DO I=1,NGT
         GYL(I,1:NRMAX) = G3D(1:NRMAX,I, 2)
      ENDDO
      CALL TRGR1DC( 3.0,12.0, 9.7,12.7,GT,GRM,GYL,NTM,NGT,NRM,NRMAX,'@TD [keV]  vs t@')

      DO I=1,NGT
         GYL(I,1:NRMAX) = G3D(1:NRMAX,I, 5)
      ENDDO
      CALL TRGR1DC( 3.0,12.0, 5.4, 8.4,GT,GRM,GYL,NTM,NGT,NRM,NRMAX,'@NE [10$+20$=/m$+3$=]  vs t@')

      DO I=1,NGT
         GYL(I,1:NRMAX) = G3D(1:NRMAX,I, 9) * 1.E-6
      ENDDO
      CALL TRGR1DC( 3.0,12.0, 1.1, 4.1,GT,GRM,GYL,NTM,NGT,NRM,NRMAX,'@AJ [MA]  vs t@')
      ...
      IF(MDLUF.EQ.1.OR.MDLUF.EQ.3) THEN
         DO I=1,NGT
            ...
            ENDDO
         ENDDO
         CALL TRGR1DC(15.0,24.0,14.0,17.0,GT,GRM,GYL,NTM,NGT,NRM,NRMAX,'@TE [keV] (XP) vs t@')
         ...
         DO I=1,NGT
            ...
            ENDDO
         ENDDO
         CALL TRGR1DC(15.0,24.0, 9.7,12.7,GT,GRM,GYL,NTM,NGT,NRM,NRMAX,'@TD [keV] (XP) vs t@')
      ENDIF

      DO I=1,NGT
         GYL(I,1:NRMAX) = G3D(1:NRMAX,I,13) * 1.E-6
      ENDDO
      CALL TRGR1DC(15.0,24.0, 5.4, 8.4,GT,GRM,GYL,NTM,NGT,NRM,NRMAX,'@AJBS [MA]  vs t@')

      DO I=1,NGT
         GYL(I,1:NRMAX) = (G3D(1:NRMAX,I,11) + G3D(1:NRMAX,I,12)) * 1.E-6
      ENDDO
      CALL TRGR1DC(15.0,24.0, 1.1, 4.1,GT,GRM,GYL,NTM,NGT,NRM,NRMAX,'@AJCD [MA]  vs t@')
...
!     ***********************************************************
      ...
      USE TRCOMM, ONLY : GT, GRG, GRM, G3D, NGT, NTM, NRM, NRMAX
      IMPLICIT NONE
      INTEGER(4), INTENT(IN):: INQ
      INTEGER(4) :: I
      REAL(4),DIMENSION(NTM,NRM) :: GYL
      ...
      DO I=1,NGT
         GYL(I,1:NRMAX) = G3D(1:NRMAX,I,27)
      ENDDO
      CALL TRGR1DC( 3.0,12.0,14.0,17.0,GT,GRG,GYL,NTM,NGT,NRM,NRMAX,'@QP  vs t@')

      DO I=1,NGT
         GYL(I,1:NRMAX) = G3D(1:NRMAX,I,31)
      ENDDO
      CALL TRGR1DC(15.0,24.0,14.0,17.0,GT,GRM,GYL,NTM,NGT,NRM,NRMAX,'@VLOOP [V]  vs t@')

      DO I=1,NGT
         GYL(I,1:NRMAX) = G3D(1:NRMAX,I,60)
      ENDDO
      CALL TRGR1DC( 3.0,12.0, 9.7,12.7,GT,GRM,GYL,NTM,NGT,NRM,NRMAX,'@VTOR [m/s]  vs t@')

      DO I=1,NGT
         GYL(I,1:NRMAX) = G3D(1:NRMAX,I,63)
      ENDDO
      CALL TRGR1DC(15.0,24.0, 9.7,12.7,GT,GRM,GYL,NTM,NGT,NRM,NRMAX,'@ER [V/m] vs t@')
...
!     ***********************************************************
      ...
      GYT(1:NGT,6)=GVT(1:NGT,101)
      CALL TRGR1D( 3.0,12.0, 9.7,12.7,GT,GYT,NTM,NGT,6,'@IPPRL,IOH,INB,IRF,IBS,IP [MA]  vs t@',2+INQ)
...
!     ***********************************************************
      ...
      USE TRCOMM, ONLY : DR, G3D, GRM, GT, GTS, GYT, GVT, MDLUF, NGT, NGST, NGPST, NRM, NRMAX, NTM, IZERO
