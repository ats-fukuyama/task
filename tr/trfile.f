...
C         ARRHOU(1,NR)=1.D0/RMJRHOU(1,NR)**2
         ARRHOU(1,NR)=1.D0/RRU(1)**2
         TTRHOU(1,NR)=BBU(1)*RRU(1)
      ENDDO
C
      KFID='RMINOR'
      CALL UF2DS(KFID,DR,TMU,FAS,AMP,NRMAX,0,IERR)
...
C         DSRHOU(1,NR)=DVRHOU(1,NR)/(2.D0*PI*RMJRHOU(1,NR))
         DSRHOU(1,NR)=DVRHOU(1,NR)/(2.D0*PI*RRU(1))
      ENDDO
C
C     *****
C
...
C
      DO NTX=1,NTXMAX1
         IF(ABS(TMU1(NTX)-TMU(1)).LE.1.D-6) THEN
            RR   = RRU(NTX)
            RA   = RAU(NTX)
            RIPS = RIPU(NTX)
            BB   = BBU(NTX)
     ...
     &           'THAT OF 2D UFILES!'
      WRITE(6,*) '## THE PROCESS CONTINUES THROUGH USING THE LAGLANGE ',
     &           'INTERPOLATION...'
      CALL LAGLANGE(TMU(1),RR  ,TMU1,RRU  ,NTXMAX1,NTUM,IERR)
      CALL LAGLANGE(TMU(1),RA  ,TMU1,RAU  ,NTXMAX1,NTUM,IERR)
...
C
      KFID='QRAD'
      CALL UF2DT(KFID,DR,DT,TMU,PRLU,AMP,NTAMAX,NTXMAX,NRMAX,TMUMAX,0,0,
     &     ICK,IERR)
C
      KFID='QOHM'
      CALL UF2DT(KFID,DR,DT,TMU,POHU,AMP,NTAMAX,NTXMAX,NRMAX,TMUMAX,0,0,
     &     ICK,IERR)
C
C     *****
C
