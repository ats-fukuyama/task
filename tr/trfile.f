...
C
      ...
      DIMENSION UPRE0(4,NRMU)          ,TMUS(NTURM)
      CHARACTER KFILE*20
C
      DR = 1.D0/DBLE(NRMAX)
      ICK=0
...
C
     ...
     &                  NRMU,NRFMAX,NTXMAX,IERR)
      ...
      DO NTA=1,NTAMAX
         TMLCL=DT*DBLE(NTA)
         RGN=DBLE(NRMAX)*DR
         CALL SPL2DF(RGN,TMLCL,F0,RUF,TMU,UNETU,NRMU,NRFMAX,NTXMAX,IERR)
...
C
      KFILE='QECH'
      CALL UFREAD2_TIME(KFILE,RUF,TMU,F2,NRFMAX,NTXMAX,MDCHK,IERR)
      CALL PRETREATMENT2(KFILE,UQECTU,NRFMAX,NTXMAX,TMUMAX,ICK,0,IERR)
      DO NTA=1,NTAMAX
         TMLCL=DT*DBLE(NTA)
         DO NR=1,NRMAX
            RMN=(DBLE(NR)-0.5D0)*DR
...
C
      ...
      DO NTA=1,NTAMAX
         ...
         DO NR=1,NRMAX
            RMN=(DBLE(NR)-0.5D0)*DR
            CALL SPL2DF(RMN,TMLCL,F0,RUF,TMU,UGR2TU,
     &                  NRMU,NRFMAX,NTXMAX,IERR)
            IF(IERR.NE.0)
