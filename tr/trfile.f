...
C
      KFID='RGEO'
      CALL UF1D(KFID,DT,TMU1,RRU  ,NTAMAX1,NTXMAX1,TMUMAX,ICK,IERR)
      IF(IERR.EQ.1.AND.KUFDEV.EQ.'jt60u') THEN
         MDRGEO=IERR
         IERR=0
      ENDIF
C
      KFID='AMIN'
      CALL UF1D(KFID,DT,TMU1,RAU  ,NTAMAX1,NTXMAX1,TMUMAX,ICK,IERR)
      IF(IERR.EQ.1.AND.KUFDEV.EQ.'jt60u') THEN
         MDAMIN=IERR
         IERR=0
      ENDIF
...
C
      KFID='KAPPA'
      CALL UF1D(KFID,DT,TMU1,RKAPU,NTAMAX1,NTXMAX1,TMUMAX,ICK,IERR)
      IF(IERR.EQ.1.AND.KUFDEV.EQ.'jt60u') THEN
         MDKAPPA=IERR
         IERR=0
      ENDIF
...
C
      AMP=1.D-20
      KFID='NFAST'
      CALL UF2DT(KFID,DR,DT,TMU,RNFU,AMP,NTAMAX,NTXMAX,NRMAX,TMUMAX,0,1,
     &           ICK,IERR)
      AMP=1.D0
      KFID='PBEAM'
      CALL UF2DT(KFID,DR,DT,TMU,PBMU,AMP,NTAMAX,NTXMAX,NRMAX,TMUMAX,0,1,
     &           ICK,IERR)
      KFID='Q'
      CALL UF2DT(KFID,DR,DT,TMU,QPU ,AMP,NTAMAX,NTXMAX,NRMAX,TMUMAX,1,1,
     &           ICK,IERR)
      IF(IERR.NE.0) MDLJQ=0
      IF(MDNI.NE.2) THEN
      KFID='ZEFFR'
      CALL UF2DT(KFID,DR,DT,TMU,ZEFFU,AMP,NTAMAX,NTXMAX,NRMAX,TMUMAX,
     &           1,1,ICK,IERR)
      ENDIF
      KFID='CURTOT'
      CALL UF2DT(KFID,DR,DT,TMU,AJU ,AMP,NTAMAX,NTXMAX,NRMAX,TMUMAX,0,1,
     &           ICK,IERR)
      IF(IERR.NE.0) MDLJQ=1
      KFID='CURNBI'
...
C
      KFID='QRAD'
      CALL UF2DT(KFID,DR,DT,TMU,PRLU,AMP,NTAMAX,NTXMAX,NRMAX,TMUMAX,0,1,
     &     ICK,IERR)
C
      KFID='QOHM'
      CALL UF2DT(KFID,DR,DT,TMU,POHU,AMP,NTAMAX,NTXMAX,NRMAX,TMUMAX,0,1,
     &     ICK,IERR)
C
C     *****
C
...
C
      KFID='RMAJOR'
      CALL UF2DT(KFID,DR,DT,TMU,RMJRHOU,AMP,NTAMAX,NTXMAX,NRMAX,TMUMAX,
     &           0,1,ICK,IERR)
      DO NTX=1,NTXMAX
         DO NR=1,NRMAX
...
C            ARRHOU(NTX,NR)=(1.D0+1.5D0*EPS**2)/RRU(NTX)**2
         ...
         ENDDO
         IF(KUFDEV.EQ.'jt60u') RRU(NTX)=RMJRHOU(NTX,1)
      ENDDO
C
      KFID='RMINOR'
...
C
      KFID='GRHO1'
      CALL UF2DT(KFID,DR,DT,TMU,AR1RHOU,AMP,NTAMAX,NTXMAX,NRMAX,TMUMAX,
     &           0,1,ICK,IERR)
      IF(KUFDEV.EQ.'jt60u') THEN
         DO NTX=1,NTXMAX
...
C
      KFID='GRHO2'
      CALL UF2DT(KFID,DR,DT,TMU,AR2RHOU,AMP,NTAMAX,NTXMAX,NRMAX,TMUMAX,
     &           0,1,ICK,IERR)
      DO NTX=1,NTXMAX
         DO NR=1,NRMAX
...
C
      AMP=1.D0
      KFID='ZEFFR'
      CALL UF2DT(KFID,DR,DT,TMU,FAT,AMP,NTAMAX,NTXMAX,NRMAX,TMUMAX,1,1,
     &           ICK,IERR)
      DO NTX=1,NTXMAX
         DO NR=1,NRMAX
            ZEFFU(NTX,NR)=FAT(NTX,NR)
...
C
     ...
     &           ICK,IERR)
      IF(KUFDEV.EQ.'X'.AND.KUFDCG.EQ.'14') THEN
         KFID='CURBS'
         CALL UF2DT(KFID,DR,DT,TMU,AJBSU,AMP,NTAMAX,NTXMAX,NRMAX,TMUMAX,
     &              0,1,ICK,IERR)
      ENDIF
C
      KFID='QNBIE'
...
C
      KFID='VRO'
      CALL UF2DT(KFID,DR,DT,TMU,DVRHOU,AMP,NTAMAX,NTXMAX,NRMAX,TMUMAX,
     &           0,0,ICK,IERR)
      DO NTX=1,NTXMAX
         DO NR=1,NRMAX
C            DSRHOU(NTX,NR)=DVRHOU(NTX,NR)/(2.D0*PI*RMJRHOU(NTX,NR))
...
C
      KFID='AAT'
      CALL UF2DT(KFID,DR,DT,TMU,ARRHOU,AMP,NTAMAX,NTXMAX,NRMAX,TMUMAX,
     &           0,1,ICK,IERR)
C
      KFID='HDT'
      CALL UF2DT(KFID,DR,DT,TMU,FAT,AMP,NTAMAX,NTXMAX,NRMAX,TMUMAX,0,1,
...
C     *****************************************************
C
      SUBROUTINE UF1D(KFID,DT,TL,F1,NTLMAX,NTXMAX,TLMAX,ICK,IERR)
C
      INCLUDE 'trcom0.inc'
...
C
      SUBROUTINE UF2DTP(KFID,DR,DT,PV,PVA,TL,FOUT,AMP,NTLMAX,NTXMAX,
     &                  RHOA,NRAMAX,NRMAX,TLMAX,NSW,ICK,IERR)
C
      INCLUDE 'trcom0.inc'
      DIMENSION RL(NRMU),TL(NTUM),F2(NTUM,NRMU),FOUT(NTUM,NRMP)
      DIMENSION U(4,NRMU),DERIV(NRMU)
