 ...
 ! electric field
      ...
      if(ip .ge. 1.d0) then
         eyy = phi(ip  ,jp  ) * sx2  * (sy1m - sy1p) - (Ayb(ip  ,jp  ) &
             ...
             + phi(ip-1,jp  ) * sy2m * (sx1m - sx1p) - (Axb(ip-1,jp  ) &
             - Aybb(ip-1,jp  )) / dt * sx2m * sy1p
        
      else
         eyy = phi(ip  ,jp  ) * sx2  * (sy1m - sy1p) - (Ayb(ip  ,jp  ) &
             - Aybb(ip  ,jp  )) / dt * sx2  * sy1p &
             + phi(ip+1,jp  ) * sx2p * (sx1m - sx1p) - (Ayb(ip+1,jp  ) &
             - Aybb(ip+1,jp  )) / dt * sx2p * sy1p &
             + phi(nx  ,jp  ) * sy2m * (sx1m - sx1p) - (Ayb(nx  ,jp  ) &
...
! magnetic field
     if(ip .ge. 1) then
         bxx = (Axb(ip  ,jp  ) + Axbb(ip  ,jp  )) / 2.0d0 * sx2  * (sy1m-sy1p)&
             + (Axb(ip+1,jp  ) + Axbb(ip+1,jp  )) / 2.0d0 * sx2p * (sy1m-sy1p)&
             + (Axb(ip-1,jp  ) + Axbb(ip-1,jp  )) / 2.0d0 * sx2m * (sy1m-sy1p)
     else
         bxx = (Axb(ip  ,jp  ) + Axbb(ip  ,jp  )) / 2.0d0 * sx2  * (sy1m-sy1p)&
             + (Axb(ip+1,jp  ) + Axbb(ip+1,jp  )) / 2.0d0 * sx2p * (sy1m-sy1p)&
             + (Axb(nx  ,jp  ) + Axbb(nx  ,jp  )) / 2.0d0 * sx2m * (sy1m-sy1p) 
     endif

     if(jp .ge. 1) then
         byy = (Ayb(ip  ,jp  ) + Aybb(ip  ,jp  )) / 2.0d0 * sy2  * (sy1m-sy1p)&
             + (Ayb(ip  ,jp+1) + Aybb(ip  ,jp+1)) / 2.0d0 * sy2p * (sy1m-sy1p)&
             + (Ayb(ip  ,jp-1) + Aybb(ip  ,jp-1)) / 2.0d0 * sy2m * (sy1m-sy1p)
     else
         byy = (Ayb(ip  ,jp  ) + Aybb(ip  ,jp  )) / 2.0d0 * sy2  * (sy1m-sy1p)&
             + (Ayb(ip  ,jp+1) + Aybb(ip  ,jp+1)) / 2.0d0 * sy2p * (sy1m-sy1p)&
             + (Ayb(ip  ,ny  ) + Aybb(ip  ,ny  )) / 2.0d0 * sy2m * (sy1m-sy1p)
             
     endif

     bzz = (Ayb(ip  ,jp  ) + Aybb(ip  ,jp  )) / 2.0d0 * sy1p * (sy1m - sy1p) &
         - (Axb(ip  ,jp  ) + Aybb(ip  ,jp  )) / 2.0d0 * sx1p * (sx1m - sx1p) 
    ...
    ! push particles by using Buneman-Boris method
      ...
      vxp = vxn + 2.0d0/(1.0d0 + 0.25d0 * (ctom * dt) ** 2.0d0 & 
          * (bxx ** 2.0d0 + byy ** 2.0d0 + bzz ** 2.0d0)) * 1.0d0/2 & 
          * ctom * (vyzero * bzz - vzzero * byy) * dt
      vyp = vyn + 2.0d0/(1.0d0 + 0.25d0 * (ctom * dt) ** 2.0d0 &
          * (bxx ** 2.0d0 + byy ** 2.0d0 + bzz ** 2.0d0)) * 1.0d0/2 &
          * ctom * (vzzero * bxx - vxzero * bzz) * dt 
      vzp = vzn + 2.0d0/(1.0d0 + 0.25d0 * (ctom * dt) ** 2.0d0 & 
          * (bxx ** 2.0d0 + byy ** 2.0d0 + bzz ** 2.0d0)) * 1.0d0/2 & 
    ...
    integer :: np, nx, ny, nz, i, ip, jp, kp, ix, iy
     ...
     do i = 1, np 
        ...
        if(ip .ne. 0 .and. jp .ne. 0) then
           ...
           jz(nx  ,jp  ) = jz(ip  ,jp  ) + chrg / dt * deltay * sx2m + sy2
           jz(nx  ,jp-1) = jz(nx  ,jp-1) + chrg / dt * deltay * sx2m + sy2m
           jz(nx  ,jp+1) = jz(ip+1,jp+1) + chrg / dt * deltay * sx2m + sy2p
           jz(ip  ,jp-1) = jz(ip  ,jp-1) + chrg / dt * deltay * sx2  + sy2m
           ...
           jz(nx  ,jp  ) = jz(nx  ,jp  ) + chrg / dt * deltay * sx2m + sy2
           jz(nx  ,ny  ) = jz(nx  ,ny  ) + chrg / dt * deltay * sx2m + sy2m
           jz(nx  ,jp+1) = jz(nx  ,jp+1) + chrg / dt * deltay * sx2m + sy2p
           jz(ip  ,ny  ) = jz(ip  ,ny  ) + chrg / dt * deltay * sx2  + sy2m
           jz(ip+1,ny  ) = jz(ip+1,ny  ) + chrg / dt * deltay * sx2p + sy2m
