...
CONTAINS
  ...
  SUBROUTINE pic_exec(iout)
    ...
    REAL(8)::sum,wran,r1,r2,rmod=2147483648.d0,ramda=65539.d0,rv,rv1
    ...
    DO nt = 1, ntmax
       ...
       CALL push(npmax,nxmax,nymax,xe,ye,ze,vxe,vye,vze, &
            ...
            vparae,vperpe,model_boundary,vcfact,n_gas,sigma_total)

       !..... push ions
       CALL push(npmax,nxmax,nymax,xi,yi,zi,vxi,vyi,vzi, &
            ex,ey,ez,bx,by,bz,dt,ctomi,xib,yib,zib, &
            vparai,vperpi,model_boundary,vcfact,n_gas,sigma_total)
  ...
  SUBROUTINE push(npmax,nxmax,nymax,x,y,z,vx,vy,vz,ex,ey,ez,bx,by,bz,&
                dt,ctom,xb,yb,zb,vpara,vperp,model_boundary,vcfact,n_gas,sigma_total)
  !***********************************************************************
    IMPLICIT NONE
    INTEGER,INTENT(IN) :: npmax, nxmax, nymax, model_boundary
    REAL(8), DIMENSION(npmax),INTENT(INOUT) :: x, y, z, xb, yb, zb
    REAL(8), DIMENSION(npmax),INTENT(INOUT) :: vx, vy, vz, vpara, vperp
    REAL(8), DIMENSION(0:nxmax,0:nymax),INTENT(IN) :: ex, ey, ez, bx, by, bz
    REAL(8) :: dt, ctom, vcfact, n_gas, sigma_total
    REAL(8) :: dx, dy, dx1, dy1, exx, eyy, ezz, bxx,&
               byy, bzz, vxm, vym, vzm, vxzero, vyzero, vzzero, vxp, vyp,&
               vzp, sx2, sy2, sx2p, sx2m, sy2m, sy2p
    REAL(8) :: btot, vtot, bb2 , gammai
  ...
  !***********************************************************************
  SUBROUTINE bound_periodic(npmax,x,y,z,xb,yb,xmid,ymid, &
                            x1,x2,y1,y2,z1,z2,alx,aly,alz)
    !***********************************************************************
    IMPLICIT NONE
    INTEGER,INTENT(IN):: npmax
    REAL(8), DIMENSION(npmax),INTENT(INOUT) :: x, y, z, xb, yb, xmid, ymid
    REAL(8),INTENT(IN):: alx, aly, alz, x1, x2, y1, y2, z1, z2
    INTEGER :: np
  ...
  !***********************************************************************
    ...
    IMPLICIT NONE
    INTEGER,INTENT(IN) :: npmax, nxmax, nymax, model_boundary
    REAL(8), DIMENSION(npmax),INTENT(IN) :: x, y
    REAL(8), DIMENSION(0:nxmax,0:nymax),INTENT(OUT) :: rho
    REAL(8),INTENT(IN) :: chrg
    REAL(8) :: factor, dx, dy, dz, sx2, sy2, sx2p, sy2p, sx2m, sy2m, dx1, dy1
  ...
  SUBROUTINE boundary_rho(nxmax,nymax,rho,model_boundary)
    !***********************************************************************
    IMPLICIT NONE
    INTEGER,INTENT(IN):: nxmax,nymax,model_boundary
    REAL(8), DIMENSION(0:nxmax,0:nymax),INTENT(OUT) :: rho
    INTEGER:: nx,ny
  ...
  SUBROUTINE current(npmax,nxmax,nymax,x,y,xb,yb,xmid,ymid,vx,vy,vz,chrg, &
    ...
    INTEGER,INTENT(IN) :: npmax, nxmax, nymax, model_boundary
    REAL(8), DIMENSION(npmax),INTENT(IN) :: &
         x, y, xb, yb, xmid, ymid, vx, vy, vz
    REAL(8), DIMENSION(0:nxmax,0:nymax), INTENT(INOUT) :: jx, jy, jz
    REAL(8),INTENT(IN) :: chrg, vcfact
    REAL(8) :: dx, dy, dx0, dx1, dy0, dy1, gammai, &
         sx2m, sx20, sx2p, sy2m, sy20, sy2p, factor
    INTEGER :: np, nxp, nyp, nxpm, nxp0, nxpp, nypm, nyp0, nypp, nx, ny
    ...
    !!$omp parallel do Private(gammai,nxp,nyp,nxpm,nxp0,nxpp,nypm,nyp0,nypp,dx,dy,dx0,dx1,dy0,dy1,sx2p,sx20,sx2m,sy2p,sy20,sy2m) Reduction(+:jx,jy,jz)
    ...
    DO np = 1, npmax
       ...
       IF(model_boundary.EQ.0) THEN ! periodic
          IF(nxpm.EQ.-1) nxpm = nxmax - 1
          IF(nypm.EQ.-1) nypm = nymax - 1
          IF(nxpp.EQ.nxmax+1) nxpp = 1
          IF(nypp.EQ.nymax+1) nypp = 1
       ...
       jy(nxpm,nyp0) = jy(nxpm,nyp0) + factor * vy(np) * sx2m * dy0 * gammai
       jy(nxp0,nyp0) = jy(nxp0,nyp0) + factor * vy(np) * sx20 * dy0 * gammai
       jy(nxpp,nyp0) = jy(nxpp,nyp0) + factor * vy(np) * sx2p * dy0 * gammai
  ...
  SUBROUTINE vector_p_reflective(nxmax,nymax,vcfact,dt,phi,phib,jx,jy,jz, &
    ...
    !original subroutine
    IMPLICIT NONE
    INTEGER,INTENT(IN) :: nxmax, nymax,model_wg,model_boundary
    REAL(8), DIMENSION(0:nxmax,0:nymax),INTENT(INOUT) :: &
         phi,phib,jx,jy,jz,Ax,Ay,Az,Axb,Ayb,Azb,Axbb,Aybb,Azbb
    REAL(8),INTENT(IN) :: &
         vcfact,dt,xmin_wg,xmax_wg,ymin_wg,ymax_wg,amp_wg,ph_wg,rot_wg, &
         eli_wg, omega,time,pi,dlen
    REAL(8) :: inv
    INTEGER :: nx, ny, nxm, nxp, nyp, nym,ilen
    REAL(8) :: dph,x,y,xmax,ymax,yc,ylen,factor,amp_start
...
!          Ax(nxmax,ny) = dt ** 2 * vcfact ** 2 * (Axb(nxmax-1,ny) &
 !                                           + Axb(nxmax,nyp) + Axb(nxmax,nym) &
  ...
  SUBROUTINE profile_sub(nxmax,nymax,xp,yp,fp,fxy,imax,model_boundary)
    ...
    IMPLICIT NONE
    INTEGER,INTENT(IN) :: nxmax, nymax, imax, model_boundary
    REAL(8),INTENT(IN) :: xp, yp, fp(imax)
    REAL(8),DIMENSION(0:nxmax,0:nymax,imax),INTENT(INOUT) :: fxy
