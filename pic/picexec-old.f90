...
CONTAINS
  ...
  SUBROUTINE pic_exec(iout)
    ...
    REAL(8)::sum,wran,r1,rmod=2147483648.d0,ramda=65539.d0,rv,rv1
    ...
    DO nt = 1, ntmax
       ...
       CALL push(npmax,nxmax,nymax,xe,ye,ze,vxe,vye,vze, &
            ...
            vparae,vperpe,model_boundary,vcfact,r1,r2,n_gas,sigma_total)

       !..... push ions
       CALL push(npmax,nxmax,nymax,xi,yi,zi,vxi,vyi,vzi, &
            ex,ey,ez,bx,by,bz,dt,ctomi,xib,yib,zib, &
            vparai,vperpi,model_boundary,vcfact,r1,r2,n_gas,sigma_total)
  ...
  SUBROUTINE push(npmax,nxmax,nymax,x,y,z,vx,vy,vz,ex,ey,ez,bx,by,bz,&
                dt,ctom,xb,yb,zb,vpara,vperp,model_boundary,vcfact,r1,r2,n_gas,sigma_total)
  !***********************************************************************
    IMPLICIT NONE
    ...
    REAL(8) :: dt, ctom, vcfact,r1,r2,n_gas,sigma_total
    ...
    DO np = 1, npmax
      ! calculate the electric field at the particle position
       ...
       END IF
      ! subroutine for ionization
      !newx = n_gas * sigma_total * vx
      !newy = n_gas * sigma_total * vy
      !P_mx = 1 - exp(-newx * dt)
      !P_my = 1 - exp(-newy * dt)
      !IF(P_mx > r1)
      !  npxmax = npxmax+1
  ...
  SUBROUTINE current(npmax,nxmax,nymax,x,y,xb,yb,xmid,ymid,vx,vy,vz,chrg, &
    ...
    REAL(8) :: dx, dy, dx1, dy1, gammai, &
         sx2m, sx2, sx2p, sy2m, sy2, sy2p, factor
    INTEGER :: np, nxp, nyp, nxpm, nxpp, nxppp, nypm, nypp, nyppp, nx, ny
