...
9000 CONTINUE
  ...
  SUBROUTINE wi_scan_tau(ierr)
    ...
    USE wicomm,ONLY: qkind,ikind,ntaumax,taumin,taumax,alfa,any,kfscan,rkind
    ...
    REAL(qkind):: tau,dtau,rk0l,ratea,any_save
    REAL(qkind),DIMENSION(ntaumax):: taua,rateaa
    REAL(rkind),DIMENSION(ntaumax):: gx,gy
    ...
    DO ntau=1,ntaumax
       ...
       taua(ntau)=tau
       rateaa(ntau)=ratea
       gx(ntau)=DBLE(taua(ntau))
       gy(ntau)=DBLE(rateaa(ntau))
    END DO
    any=any_save
    ...
    CALL PAGES
    CALL GRD1D(0,gx,gy,ntaumax,ntaumax,1,TITLE='@abs vs tau@')
    CALL PAGEE
  ...
  SUBROUTINE wi_scan_alfa(ierr)
    ...
    USE wicomm,ONLY: qkind,ikind,nalfamax,alfamin,alfamax,alfa,beta,any, &
         xmax,xmin,dx0,nxmax,kfscan,pi,xwint,xgrid,rkind
    USE wiexec,ONLY: wi_exec
    ...
    INTEGER(ikind):: nalfa
    INTEGER(ikind),PARAMETER:: nfl=21
    REAL(qkind):: dalfa,rk0l,ratea,anb
    REAL(qkind):: alfa_save,xmax_save,xmin_save,dx0_save,xwint_save
    REAL(qkind),DIMENSION(nalfamax):: rk0la,rateaa
    REAL(rkind),DIMENSION(nalfamax):: gx,gy
    EXTERNAL PAGES,PAGEE
...
!          xmin=-0.5D0
       ...
       rateaa(nalfa)=ratea
       gx(nalfa)=DBLE(rk0la(nalfa))
       gy(nalfa)=DBLE(rateaa(nalfa))
    ...
    IF(NALFAMAX.NE.1) THEN
       CALL PAGES
       CALL GRD1D(0,gx,gy,nalfamax,nalfamax,1,TITLE='@abs vs k0L@',&
                  MODE_LS=1,FMIN=0.D0)
       CALL PAGEE
    END IF
