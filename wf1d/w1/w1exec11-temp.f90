...
!     ******* ELECTROMAGNETIC FIELD IN PLASMA *******
  ...
  SUBROUTINE W1EPWQ(NZ)
    ...
    REAL(rkind):: RW,RKV,RCE,DX,RNZ
    REAL(rkind):: PIN1,PIN2,PIN3,PIN4,PIN,POUT1,POUT2,POUT3,POUT4,POUT,PCONV
    REAL(rkind):: PABSL,PABS0L,PABS1L,PABS2L,PABS3L
    COMPLEX(rkind):: CPABSL,CPABS0L,CPABS1L,CPABS2L,CPABS3L
    COMPLEX(rkind):: CDEY,CDEZ,CBY,CBZ
    COMPLEX(rkind):: CLH0,CLH1,CLH2,CLH3
    COMPLEX(rkind):: CAJ0L,CAJ1L,CAJ2L,CAJ3L
    ...
    DO NS=1,NSMAX
       ...
       DO NX=1,NXMAX-1
          ...
          DO NX1=MAX(1,NX+NXDMIN),MIN(NX+NXDMAX,NXMAX-1)
             ...
             NCL0=NCLA(NXD0,NX,NS)  ! NX1 position on NX line 
             NXD1=NX-NX1+NXDMAX+1   ! positive
             NCL1=NCLA(NXD1,NX1,NS) ! NX position on NX1
             CPABSL=0.D0
                ...
                DO IL=1,4
                   ...
                   DO IA=1,3
                      ...
                      DO IB=1,3
                         ...
                         CLH3=CONJG(CL(IB,IA,IL,NCL0))
                         CAJ0L=CAJ0L+CLH0*CA(MM+IB)
                         CAJ1L=CAJ1L+CLH1*CA(NN+IB)
                         CAJ2L=CAJ2L+CLH2*CA(MM+IB) 
                         CAJ3L=CAJ3L+CLH3*CA(MM+IB)
                      END DO
                      CPABS0L=CPABS0L+CONJG(CA(NN+IA))*CAJ0L
                      CPABS1L=CPABS1L+CONJG(CA(MM+IA))*CAJ1L
                      CPABS2L=CPABS2L+CONJG(CA(NN+IA))*CAJ2L
                      CPABS3L=CPABS3L+CONJG(CA(NN+IA))*CAJ3L
                      CAJ0(NN+IA)=CAJ0(NN+IA)+CAJ0L
                      CAJ1(MM+IA)=CAJ1(MM+IA)+CAJ1L
                   END DO
                   CPABSL=CPABSL+CPABS0L+CPABS1L
                END DO
             END IF
             CPABS(NX,NS)= -CI*RCE*CPABS0L*RKV*RKV
             PABS(NX,NS)=PABS(NX,NS)+REAL(CPABS(NX,NS))
          END DO
       END DO
