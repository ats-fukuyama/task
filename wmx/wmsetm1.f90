...
MODULE wmsetm1
  ...
  PRIVATE
  PUBLIC wm_setm1,wm_setv1

CONTAINS

!     ****** CALCULATE LOCAL COEFFICIENT MATRIX ******

  SUBROUTINE wm_setm1(CA,CB,IGD,LA,NRP)
    ...
    INTEGER:: ICOMP,IGD1,MLXD,MLXDF,IGD2,NKXD,NKXDF,NB,NR,MB
...
!     ICOMP=3 : PHI COMPONENT OF MAXWELL EQUATION
      ...
      IF (NB <= NR_S -2)THEN
         ...
         IF(NR.NE.NRP.OR.NBMODE.EQ.0) THEN
            ...
            NBMODE=1
            IF(NB.EQ.NBST) THEN
               CALL WMSETM_M_in(NR,1)   ! with initial setup
            ELSE
               CALL WMSETM_M_in(NR,0)
            END IF
            CALL WMSETM_V(NR)
            CALL WMSETM_B(NR)
         END IF
      ELSEIF (NB  >= NR_S + 1) THEN
         ...
         IF(NR.NE.NRP.OR.NBMODE.EQ.2) THEN
            ...
            IF(NB.EQ.NBST.OR.NB.EQ.NR_S+1) THEN
               CALL WMSETM_M_out(NR,1)   ! with initial setup
            ELSE
               CALL WMSETM_M_out(NR,0)
            END IF
            CALL WMSETM_V(NR)
            CALL WMSETM_B(NR)
         END IF
      ELSEIF (NB == NR_S-1.OR.NB == NR_S)THEN
         ...
         IF(NR.NE.NRP.OR.NBMODE.EQ.1) THEN
            ...
            NBMODE=2
            IF(NB.EQ.NBST) THEN
               CALL WMSETM_M_b(NR,1)     ! with initial setup
            ELSE
               CALL WMSETM_M_b(NR,0)
            END IF
            CFVP=0d0
         END IF
      ENDIF
      ...
      RETURN
  END SUBROUTINE wm_setm1
  
!     ****** CALCULATE LOCAL right_hand_side vector ******

  SUBROUTINE wm_setv1(NR,CFVP_)
...
!     ****** ASSEMBLE TOTAL ELEMENT COEFFICIENT MATRIX ******
  ...
  SUBROUTINE WMSETM_M_b(NR,IND)

    USE wmcomm
    USE wmsetf1
    IMPLICIT NONE
    INTEGER,INTENT(IN):: NR,IND
    INTEGER:: KDX,LDX,J,I,LBAND,NKX,MLX,ND,NDX,NKD,NN,NK,KD
    INTEGER:: MLD,MM,ML,LD,LBND,ID,MD,MDX
    REAL(rkind):: XRHOM,XRHOC,XRHOP,DRHOM,DRHOP,DRHOPM
    REAL(rkind):: QPMH,QPC,QPPH,FMHM,FMHC,FPHC,FPHP,FCMH,FCPH
    REAL(rkind):: DPSIPDRHOMH,DPSIPDRHOPH,DPSIPDRHOC
    REAL(rkind):: DFMHM,DFMHC,DFCM,DFCC,DFCP,DFCMH,DFCPH
    REAL(rkind):: DDFMHM,DDFMHC,DDFPHC,DDFPHP
    REAL(rkind):: FACT1M,FACT1C,FACT1P
    REAL(rkind):: FACT2M,FACT2C,FACT2P
    REAL(rkind):: FACT3M,FACT3C,FACT3P
    REAL(rkind):: XRHOMH,XRHOPH
    COMPLEX(rkind):: CMAC(3,3,3),CDVC(3,3)
    COMPLEX(rkind):: CMACH(3,3,3),CIMAC(3,3,3),CIMACH(3,3,3)
    ...
    IF(NR.LT.NRMAX) CALL WM_SETF1(NR+2,0)
...
!        ML : l = m + m'      = m0 + (MD + LD)
    ...
    DO ND=NDMIN,NDMAX
                         ...
                         DO I=1,3
                            ...
                            IF(J==1)THEN 
                               ...
                               CIMACH(I,J,2)=((CRMAF(I,J,LDX,KDX,1)/XRHOM &
                                              +CRMAF(I,J,LDX,KDX,2)/XRHOC) &
                                            /2d0)*XRHOMH
                               CIMACH(I,J,3)=((CRMAF(I,J,LDX,KDX,2)/XRHOC &
                                              +CRMAF(I,J,LDX,KDX,3)/XRHOP) &
                                            /2d0)*XRHOPH
                            ENDIF
                            IF( I==2 .and. J==2)THEN
                               CMACH(I,J,2)=((CMAF(I,J,LDX,KDX,1)/XRHOM &
       ...
       ENDDO
    ENDDO
  END SUBROUTINE WMSETM_M_b

  !     ****** ASSEMBLE TOTAL ELEMENT COEFFICIENT MATRIX ******

  SUBROUTINE WMSETM_M_in(NR,IND)
    ...
    INTEGER,INTENT(IN):: NR,IND
    INTEGER:: KDX,LDX,J,I,LBAND,NKX,MLX,ND,NDX,NKD,NN,NK,KD
    INTEGER:: MLD,MM,ML,LD,LBND,ID,KDD,KDDX,KDXF,KKD,KKDX
    INTEGER:: L1,L2,L3,LDD,LDDX,LDXF,LLD,LLDX,MD,MDX,MLFX,NKXF,MLXF
    REAL(rkind):: XRHOM,XRHOC,XRHOP,DRHOM,DRHOP,DRHOPM
    REAL(rkind):: XRHOMH,XRHOPH
    REAL(rkind):: QPMH,QPC,QPPH,FMHM,FMHC,FPHC,FPHP,FCMH,FCPH
    REAL(rkind):: DPSIPDRHOMH,DPSIPDRHOPH,DPSIPDRHOC
    ...
    REAL(rkind):: FACT2M,FACT2C,FACT2P
    REAL(rkind):: FACT3M,FACT3C,FACT3P
    COMPLEX(rkind):: CMAC(3,3,3),CDVC(3,3),CEMP_TMP(9,3)
    COMPLEX(rkind):: CMACH(3,3,3),CDVM(3,3),CDVP(3,3)
    COMPLEX(rkind):: CDDVM(3,3),CDDVC(3,3),CDDVP(3,3)
    COMPLEX(rkind):: CDVMH(3,3),CDVCH(3,3),CDVPH(3,3),CDDPC(3,3)
...
!     --- PHI COMPONENT OF MAXWELL EQUATION ---
    ...
    RETURN
  END SUBROUTINE WMSETM_M_in

  SUBROUTINE WMSETM_M_out(NR,IND)
    ...
    INTEGER,INTENT(IN):: NR,IND
    INTEGER:: KDX,LDX,J,I,LBAND,NKX,MLX,ND,NDX,NKD,NN,NK,KD
    INTEGER:: MLD,MM,ML,LD,LBND,ID,KDDX,KDXF,L1,L2,L3,LDDX,LDXF,MD,MDX
    INTEGER:: MLXF,NKXF
    REAL(rkind):: XRHOM,XRHOC,XRHOP,DRHOM,DRHOP,DRHOPM
    REAL(rkind):: XRHOMH,XRHOPH
    REAL(rkind):: QPMH,QPC,QPPH,FMHM,FMHC,FPHC,FPHP,FCMH,FCPH
    REAL(rkind):: DPSIPDRHOMH,DPSIPDRHOPH,DPSIPDRHOC
    REAL(rkind):: DFMHM,DFMHC,DFCM,DFCC,DFCP,DFCMH,DFCPH
    REAL(rkind):: DDFMHM,DDFMHC,DDFPHC,DDFPFPC
    REAL(rkind):: DDFPHP
    REAL(rkind):: FACT1M,FACT1C,FACT1P
    REAL(rkind):: FACT2M,FACT2C,FACT2P
    REAL(rkind):: FACT3M,FACT3C,FACT3P
    ...
    IF(NR.LT.NRMAX) CALL WM_SETF1_OUT(NR+2,0)
...
!     --- PHI COMPONENT OF MAXWELL EQUATION ---
  ...
  SUBROUTINE WMSETM_V(NR)
    ...
    IMPLICIT NONE
    INTEGER,INTENT(IN):: NR
    INTEGER:: MDX,NDX,NRI,ND,NKX,NN,MD,MM,NRANT
    REAL(rkind):: RN(nsmax),RTPR(nsmax),RTPP(nsmax),RU(nsmax)
    COMPLEX(rkind):: CW,CC,CJTHM,CJPHM,CJR,CJTHP,CJPHP
    REAL(rkind):: DPH,DTH,XRHO1,XRHO2,DRHO,XRHOC,RT,RJFACT
    REAL(rkind):: DPSIPDRHOC,FACTM,FACTP,QPC
...
!     ****** SET BOUNDARY CONDITION ******
  ...
  SUBROUTINE WMSETM_B(NR)
    ...
    IMPLICIT NONE
    INTEGER,INTENT(IN):: NR
    INTEGER:: ID0,ID,ND,NDX,NKD,KD,KDX,MD,MDX,MLD,MLX,LD,LDX,MM,LBND,MB,NKX
    REAL(rkind):: DRHO1,DRHO2,A1,A2,XRHO1,XRHO2,XRHO3,XRHO4
    REAL(rkind):: XRHOH1,XRHOH2,XRHOH3
