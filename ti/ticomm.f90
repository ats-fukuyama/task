...
!     ****** MATRIX VARIBALES ******
  ...
  REAL(rkind),DIMENSION(:),ALLOCATABLE :: SOL_ORG,SOL_PREV,SOL_NEW
...
!     ****** MATRIX VARIABLES ******
  ...
  REAL(rkind),DIMENSION(:,:),ALLOCATABLE:: & ! (NSAMAX,NRMAX)
       DD,VV
  REAL(rkind),DIMENSION(:,:,:),ALLOCATABLE:: & ! (NSAMAX,NSAMAX,NRMAX)
       CC
...
!     ****** SOURCE VARIABLES ******
    ...
    ALLOCATE(CC(NSAMAX,NSAMAX,NRMAX),STAT=IERR); IF(IERR.NE.0) GOTO 900
 ...
 900 CONTINUE
  ...
  SUBROUTINE allocate_neqmax(ierr)
    ...
    ALLOCATE(NSA_NEQ(NEQMAX),NV_NEQ(NEQMAX),NEQ_NVNSA(3,0:NSAMAX),STAT=IERR)
             IF(IERR.NE.0) GOTO 900
    ...
    ALLOCATE(VEC_GLOBAL(NEQMAX*NRMAX),STAT=IERR)
             IF(IERR.NE.0) GOTO 900
    ALLOCATE(SOL_ORG(NEQMAX*NRMAX),STAT=IERR)
             IF(IERR.NE.0) GOTO 900
 ...
 900 CONTINUE
  ...
  SUBROUTINE deallocate_neqmax
    ...
    DEALLOCATE(MAT_LOCAL,VEC_LOCAL,VEC_GLOBAL,SOL_ORG,SOL_PREV,SOL_NEW)
