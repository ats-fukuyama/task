...
C
      IMPLICIT NONE
      INTEGER IDOPEN, IST1, IST2, SHOT, DIM, KL
      REAL STIME
      LOGICAL LEX
      CHARACTER*80 LINE, DIR, DEV, ID
...
C
      ...
      IMPLICIT NONE
      COMMON /MDSPLUS/ MDS
      COMMON /DATA/ KDIR,KDEV,SNUM,NDIM,KFID,STIME
...
C
      ...
      COMMON /MDSPLUS/ MDS
      COMMON /DATA/ KDIR,KDEV,SNUM,NDIM,KFID,STIME
      CHARACTER*80 KDIR,KDEV,KDEV1,KFID,KFID1
      INTEGER MDS,SNUM,SNUM1,NDIM,NDIM1
...
C
      ...
      STIME1=STIME
C
      CALL SETDIR(KDIR,KDEV1,SNUM1,NDIM1,KFID1,IERR)
      IF(IERR.NE.0) RETURN
...
C
      SUBROUTINE SETDIR(KDIR,KDEV1,IDCG,IDM,KFID1,IERR)
C
      IMPLICIT NONE
      INTEGER MDS,IERR,IKDIR,IKDEV,IKDCG,IDCG,IKDIRX,IDM,IKFID,IKDIRR1,
     &        IKDIRR2,IKFIDCK
      COMMON /MDSPLUS/ MDS
      ...
      CHARACTER KDIR*80,KDIRX*90,KFIDCK*90
      CHARACTER KERR*100
      LOGICAL LEX
C
      IERR=0
...
C
     ...
     &     //'/in/'
      ...
      IF(LEX.EQV..FALSE.) THEN
         CALL KTRIM(KDIRX,IKDIRX)
         WRITE(KERR,*) '@XX: DIRECTORY DOES NOT EXIST! ',
     &                 KDIRX(1:IKDIRX),'@'
...
C
      ...
      IF(LEX.EQV..FALSE.) THEN
         CALL KTRIM(KFIDCK,IKFIDCK)
         WRITE(KERR,*) '@XX: FILE DOES NOT EXIST! ',
     &                 KFIDCK(1:IKFIDCK),'@'
         CALL GTEXTX(10.0,10.0,KERR,2)
         IERR=1
      ENDIF
...
C
     ...
     &                     R,T,F3,NRXMAX,NTXMAX,IERR)
      ...
      IF(IERR.NE.0) THEN
         WRITE(KERR,*)
     &        '@XX: DEVICE, DISCHARGE OR FILE ID DOES NOT EXIST!@'
         CALL GTEXTX(10.0,10.0,KERR,2)
         GOTO 9000
      ENDIF
C
...
C
      ...
      ENDDO
C
 100  WRITE(KERR,*) '@XX: SLICE TIME IS NOT IN THE RANGE OF ',
...
C
      ...
      IMPLICIT NONE
      COMMON /DATA/ KDIR,KDEV,SNUM,NDIM,KFID,STIME
      CHARACTER*80 KDIR,KDEV,KDEV1,KFID
      INTEGER SNUM,NDIM
      INTEGER I,L
      REAL STIME
C
...
C
      ...
      END
C
      SUBROUTINE SETNUM(SNUM1)
      IMPLICIT NONE
      COMMON /DATA/ KDIR,KDEV,SNUM,NDIM,KFID,STIME
      CHARACTER*80 KDIR,KDEV,KFID
      INTEGER SNUM,SNUM1,NDIM
      REAL STIME
C
      SNUM=SNUM1
C
...
C
      ...
      IMPLICIT NONE
      COMMON /DATA/ KDIR,KDEV,SNUM,NDIM,KFID,STIME
      CHARACTER*80 KDIR,KDEV,KFID,KFID1
      INTEGER SNUM,NDIM
      INTEGER I,L
      REAL STIME
C
