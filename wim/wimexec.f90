...
!     *****  DIVIDE Z-MESH  *****
  ...
  SUBROUTINE DIVZ(NZMAX)
    ...
    USE wimcomm, ONLY: rkind,NZMAX,ZA,PN0,PB0,PB1,ANX,BETA,ZMAX,DZMAX,DZWID
    IMPLICIT NONE
    INTEGER:: NZ,NZMAX
...
!     *****  SET ELECTRIC FIELD (FULL MATRIX)  ***** 
  ...
  SUBROUTINE SUBFY(NZMAX)
    ...
    USE wimcomm,ONLY: NZMAX,CE,CK,CV
    IMPLICIT NONE
    INTEGER,INTENT(IN):: NZMAX
    INTEGER:: I,J
...
!     *****  CALCULATE ABSORBED POWER  ***** 
  ...
  SUBROUTINE SUBPOW(NZMAX,NWMAX,CPTOT)
    ...
    INTEGER:: NZ,MM,NN,NS,NE,I,J,ID,JD
    REAL(rkind):: DZI,DZJ,AD,BD,KI,KJ,YY,YYI,YYJ,VP,VM,VPI,VPJ,VMI,VMJ, &
