...
!    TYPE(pl_plf_type),DIMENSION(nsmax),INTENT(IN):: plf
    ...
    COMPLEX(rkind):: CSM11,CSM12,CSM13,CSM22,CSM23,CSM33
      ...
      NCMIN = NDISP1(NS)
      NCMAX = NDISP2(NS)
      NHMAX=MAX(ABS(NCMIN),ABS(NCMAX),2)+2
...
!    TYPE(pl_plf_type),DIMENSION(nsmax),INTENT(IN):: plf
    COMPLEX(rkind),INTENT(OUT):: CLDISP(6)
    REAL(rkind),DIMENSION(:),ALLOCATABLE:: ADJ,ADJD
    INTEGER:: NCMIN,NCMAX,NHMAX,NTH,NP,NC,NCD,NP1,INC,NP2,N
...
!    cdelta=CI*0.003D0
    cdelta=CI*0.0
      ...
      NCMAX = NDISP2(NS)
      NHMAX=MAX(ABS(NCMIN),ABS(NCMAX),2)+2
!      WRITE(6,'(A,3I5)') 'NC:',NCMIN,NCMAX,NHMAX
      ALLOCATE(ADJ(0:NHMAX),ADJD(0:NHMAX))
...
!   PNEAR1
            ...
            IF (PNEAR1.LT.0.D0.OR.PNEAR1.GT.DELP(NS)*NPMAX) THEN
               GOTO 302
...
!     PNEAR2
  ...
  302       PNEAR2 = (AMP*PA(NS)*VC)/(1-(DNPR*TCSM(NTH))**2)* &
                    (DNPR*NC*DCWC*TCSM(NTH)-SQRT(D-1))/PTH0
            IF (PNEAR2.LT.0.D0.OR.PNEAR2.GT.DELP(NS)*NPMAX) THEN 
               GOTO 310
