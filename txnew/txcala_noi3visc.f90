...
!       & rGASPFA, rNueHLphthVR, rNuiHLphthVR   ! for UHphSwitch == 1 miki_m 2010/8/26
  real(8), dimension(:), allocatable :: &
       ...
       & aatq, BSmb, Bpsqbbt, PsitdotVBth, &
       & fipolsdt, FQLcoef1, FQLcoef2, PNbVinv, fipolsdtPNbV
!!rp_conv       &, rNubLL
...
!***************************************************************
  ...
  SUBROUTINE TXCALA
    ...
    INTEGER(4) :: NE, NR, NC, NQ, N, iHvsLC, itest
...
!       &     rGASPFA(0:N), rNueHLphthVR(0:N), rNuiHLphthVR(0:N))
    allocate(UsrgV(0:N,NSM), rrtinv(0:N), aatinv(0:N), bthcoinv(0:N), &
       ...
       &     aatq(0:N), BSmb(0:N), Bpsqbbt(0:N), PsitdotVBth(0:N), &
       &     fipolsdt(0:N), FQLcoef1(0:N), FQLcoef2(0:N), &
       &     PNbVinv(0:N),fipolsdtPNbV(0:N))
...
!$omp parallel do private(iHvsLC)
    do NQ = 1, NQMAX
       NC = 0
          do NE = 1, NEMAX
             NR = NE - 1
...
!       end if
    ...
    !   NOTE: This conditions originally should be satisfied by the radial force balance
    ...
    !         is favorable.
!!    CALL BOUNDARY(0    ,LQe3,0,Var(0,1)%RUph*bbt(0)/fipol(0)) ! unstable
    CALL BOUNDARY(0    ,LQi3,0,Var(0,2)%RUph*bbt(0)/fipol(0))
...
!       &       rGASPFA, rNueHLphthVR, rNuiHLphthVR)
    deallocate(UsrgV, rrtinv, aatinv, bthcoinv, FqhatsqI, &
       ...
       &       BUsparVbbt, FQLcoef1, FQLcoef2, PNbVinv)
...
!    rGASPFA(NRMAX) = rGASPF
    ...
    FQLcoef1(0) = 0.d0
    FQLcoef2(0) = 0.d0
    FQLcoef1(1:NRMAX) = FQLcoef(1:NRMAX) * sdt(1:NRMAX) * bbt  (1:NRMAX) / bri(1:NRMAX)
    FQLcoef2(1:NRMAX) = FQLcoef(1:NRMAX) * sdt(1:NRMAX) * fipol(1:NRMAX) / bri(1:NRMAX)
    ...
    do NR = 0, NRMAX
       if(PNbV(NR) == 0.d0) then
          BSmb(NR) = 0.d0 ! tentative
          PNbVinv(NR) = 0.d0
       else
          BSmb(NR) = amb * amp * Vbpara(NR) * bb * MNB(NR) ! tentative
...
!!$    NLC( 5,NEQ) = 0
    ...
    ! Perpendicular viscosity force

    ELM(1:NEMAX,1:4,16,NEQ) = - fem_int(27,sst,rMue)
    NLC(16,NEQ) = LQe4

    ELM(1:NEMAX,1:4,17,NEQ) =   fem_int(37,sst,rMue,Var(:,1)%RUph)
    NLC(17,NEQ) = LQe1

    !  Diffusion of electrons (***AF 2008-06-08) (37+L3 is not possible because it is a diffusion term)

    ELM(1:NEMAX,1:4,16,NEQ) = - fem_int(37,sst,DMAGe,Var(:,1)%n)
    NLC(16,NEQ) = LQe3
...
!***************************************************************
  ...
  SUBROUTINE LQe4CC
    ...
    ELM(1:NEMAX,1:4, 3,NEQ) = - fem_int(24,Vmps(:,1),suft)
    NLC( 3,NEQ) = LQe4
...
!!$       NLC(18,NEQ) = LQe1
       ...
       ELM(1:NEMAX,1:4,23,NEQ) = achgb * amqp / rKilo * fem_int(-20,SNB,BUsparVbbt(:,1),PNbVinv)
       NLC(23,NEQ) = LQb3

       ! Simpified Alpha heating

       ELM(1:NEMAX,1:4,24,NEQ) = 1.D0 / (1.D20 * rKeV) * fem_int(-1,PALFe)
       NLC(24,NEQ) = 0
       
       ! Direct heating (RF)

       ELM(1:NEMAX,1:4,25,NEQ) =   1.D0 / (1.D20 * rKeV) * fem_int(-1,PRFe)
       NLC(25,NEQ) = 0

       ! Radiation loss (Bremsstrahlung)

       ELM(1:NEMAX,1:4,26,NEQ) = - 1.D0 / (1.D20 * rKeV) * fem_int(-1,PBr)
       NLC(26,NEQ) = 0

       !  Diffusion of electrons (***AF 2008-06-08)

       ELM(1:NEMAX,1:4,27,NEQ) = - fem_int(27,sst,DMAGe)
       NLC(27,NEQ) = LQe5
...
!***************************************************************
!
!   Electron Heat Parallel Flow (SUPG)
!
!***************************************************************
...
!
!     Note: Ion pressure gradient term tends to destabilize the neutrality at the edge at initial.
!
!***************************************************************
...
!!$    NLC( 5,NEQ) = 0
    ...
    ELM(1:NEMAX,1:4,22,NEQ) = - fem_int(37,sst,DMAGi,Var(:,2)%n)
    NLC(22,NEQ) = LQi3
...
!***************************************************************
  ...
  SUBROUTINE LQi4CC
    ...
    ELM(1:NEMAX,1:4, 3,NEQ) = - fem_int(24,Vmps(:,2),suft)
    NLC( 3,NEQ) = LQi4
...
!!$       NLC(16,NEQ) = LQn3
       ...
       ELM(1:NEMAX,1:4,20,NEQ) = achgb * amqp / rKilo * fem_int(-20,SNB,BUsparVbbt(:,2),PNbVinv)
       NLC(20,NEQ) = LQb3
...
!***************************************************************
  ...
  SUBROUTINE LQb4CC
    ...
    ELM(1:NEMAX,1:4, 0,NEQ) =   fem_int(1+L4) * invDT
    NLC( 0,NEQ) = LQb4

    ! Advection + Grid velocity

    ELM(1:NEMAX,1:4, 1,NEQ) =   fem_int(5+L4,UgV)
    NLC( 1,NEQ) = LQb4

    if( MDBEAM /= 0 ) then

    ELM(1:NEMAX,1:4, 2,NEQ) = - FSADVB * fem_int(3+L4,RUbphV)
    NLC( 2,NEQ) = LQb2
    ...
    ELM(1:NEMAX,1:4, 3,NEQ) =   (amas(1) / amb) * fem_int(2+L4,lfb(:,1,1,1))
    NLC( 3,NEQ) = LQe4

    ! Collisional friction force with bulk ions

    ELM(1:NEMAX,1:4, 4,NEQ) =   (amas(1) / amb) * fem_int(20+L4,lfb(:,2,1,1),RatPN(:,1))
    NLC( 4,NEQ) = LQi4

    ! Collisional friction force with beam ions (self collision)

    ELM(1:NEMAX,1:4, 5,NEQ) =   (amas(1) / amb) * fem_int(20+L4,lff(:,1,1),RatPNb(:,1))
    NLC( 5,NEQ) = LQb4

    ! Toroidal E force

    ELM(1:NEMAX,1:4, 6,NEQ) =   achgb / (amb * amqp) * fem_int(2+L4,PNbV)
    NLC( 6,NEQ) = LQm3
    ...
    ELM(1:NEMAX,1:4, 7,NEQ) =   achgb / (amb * amqp) * fem_int(6+L4,PsiV)
    NLC( 7,NEQ) = LQb2
    ...
    ELM(1:NEMAX,1:4, 8,NEQ) = - fem_int(2+L4,rNuLB) * fact
    NLC( 8,NEQ) = LQb4

    ! Momentum loss due to thermalization

    ELM(1:NEMAX,1:4, 9,NEQ) = - fem_int(2+L4,rNuB)
    NLC( 9,NEQ) = LQb4

    ! Collisional friction force with neutrals

    ELM(1:NEMAX,1:4,10,NEQ) = - fem_int(2+L4,rNu0b)
    NLC(10,NEQ) = LQb4

    ! Charge exchange force

    ELM(1:NEMAX,1:4,11,NEQ) = - fem_int(2+L4,rNubCX)
    NLC(11,NEQ) = LQb4

    ! NBI momentum source

    ELM(1:NEMAX,1:4,12,NEQ) =   1.d0 / (amb * amp) * fem_int(-(2+L4),ParatoZeta,BSmb)
    NLC(12,NEQ) = 0

!    ! Loss cone loss
!
!    ELM(1:NEMAX,1:4,13,NEQ) =   fem_int(-(1+L4),SiLCph) * rr
!    NLC(13,NEQ) = 0
!
!    ! Ion orbit loss
!
!    ELM(1:NEMAX,1:4,14,NEQ) = - fem_int(2+L4,rNuOL)
!    NLC(14,NEQ) = LQi4
