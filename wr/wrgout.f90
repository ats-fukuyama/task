...
!*********************** GRAPHIC DATA OUTPUT ************************
  ...
  SUBROUTINE WR_GOUT(NSTAT)
    ...
    1 WRITE(6,*)'INPUT GRAPH TYPE : 1,2,3,4,5, 6,7,8,9  X'
...
!     ----- draw deposition profile -----
    ...
    CALL GMNMX2(GPY,NITM+1,1,NRZMAX,1,1,NRAYMAX,1,GYMIN,GYMAX)
...
!  ***** TOROIDAL TRAJECTORY AND POWER *****
  ...
  SUBROUTINE WRGRF2
    ...
    REAL(rkind):: RSU(NSUMAX),ZSU(NSUMAX)
    REAL(4),ALLOCATABLE:: GRS(:),GZS(:)
    REAL(4):: GUX(NSTMAX+1),GUY(NSTPMAX+1)
    REAL(4):: GPX(NSTPMAX+1),GPY(NSTPMAX+1,NRAYMAX)
...
C
      ...
      ENDDO
C
C     ----- major radius dependence of absorbed power -----
C
      GDRL=GRLEN/NRZMAX
      DO NRZ=1,NRZMAX
         GPX(NRZ)=GRMIN+(NRZ-1)*GDRL
      ENDDO
      DO NRAY=1,NRAYMX
         DO NRZ=1,NRZMAX
             GPY(NRZ,NRAY)=0.0
      ...
      DO NRAY=1,NRAYMX
         DO IT=0,NITMAX(NRAY)-1
            ...
            YL=RAYS(2,IT,NRAY)
            RL1=SQRT(XL**2+YL**2)
	    NRL1=INT((RL1-GRMIN)/GDRL)+1
            XL=RAYS(1,IT+1,NRAY)
            YL=RAYS(2,IT+1,NRAY)
            RL2=SQRT(XL**2+YL**2)
            NRL2=INT((RL2-GRMIN)/GDRL)+1
            NDR=ABS(NRL2-NRL1)
            IF(NDR.EQ.0) THEN
               GPY(NRL1,NRAY)=GPY(NRL1,NRAY)+GUCLIP(RAYS(8,IT+1,NRAY))
            ELSE IF(NRL1.LT.NRL2) THEN
               SDR=(RL2-RL1)/GDRL
               DELP=RAYS(8,IT+1,NRAY)/SDR
               GPY(NRL1,NRAY)=GPY(NRL1,NRAY)
     &                       +GUCLIP((DBLE(NRL1)-(RL1-GRMIN)/GDRL)*DELP)
               DO NRL=NRL1+1,NRL2-1
                  GPY(NRL,NRAY)=GPY(NRL,NRAY)+GUCLIP(DELP)
               ENDDO
               GPY(NRL2,NRAY)=GPY(NRL2,NRAY)
     &                     +GUCLIP(((RL2-GRMIN)/GDRL-DBLE(NRL2-1))*DELP)
            ELSE
               SDR=(RL1-RL2)/GDRL
               DELP=RAYS(8,IT+1,NRAY)/SDR
               GPY(NRL2,NRAY)=GPY(NRL2,NRAY)
     &                       +GUCLIP((DBLE(NRL2)-(RL2-GRMIN)/GDRL)*DELP)
               DO NRL=NRL2+1,NRL1-1
                  GPY(NRL,NRAY)=GPY(NRL,NRAY)+GUCLIP(DELP)
               ENDDO
               GPY(NRL1,NRAY)=GPY(NRL1,NRAY)
     &                     +GUCLIP(((RL1-GRMIN)/GDRL-DBLE(NRL1-1))*DELP)
            ENDIF
...
C
     ...
     &                 /GUCLIP(2*PI*(DBLE(NRL)-0.5D0)*GDRL*GDRL)
       ENDDO
C         WRITE(6,'(5(I3,1PE12.4))') (NRL,GPY(NRL,NRAY),NRL=1,NRZMAX)
      ENDDO
C
...
C
      ...
      CALL GDEFIN(13.5,23.5,9.0,16.0,GRMIN,GRMAX,0.0,GYSMAX)
      CALL SETLIN(0,2,7)
      CALL GFRAME
      CALL GSCALE(0.0,  GRSTEP,0.0,  GYSCAL,0.1,9)
      CALL GVALUE(0.0,2*GRSTEP,0.0,0.0,NGULEN(2*GRSTEP))
      CALL GVALUE(0.0,0.0,0.0,GYSCAL,NGULEN(GYSCAL))
      DO NRAY=1,NRAYMX
         CALL SETLIN(0,2,7-MOD(NRAY-1,5))
...
C     ----- DRAW POWER FLUX ----- major radius ---
     ...
     &              'PF:',NSTP,XL,YL,ZL,RHON,RAYS(7,NSTP,NRAY)
      ...
      END
C
C     ***** Parallel and perpendicular wave number *****
C
      SUBROUTINE WRGRFC
C
C
...
C
      ...
      CALL SETLIN(0,2,7)
C
C     ----- minor radius dependence of wave number -----
C
      DO NRAY=1,NRAYMX
         DO IT=0,NITMAX(NRAY)
            XL=RAYS(1,IT,NRAY)
...
C
      ...
      ENDDO
C
C     ----- major radius dependence of wave number -----
C
      DO NRAY=1,NRAYMX
         DO IT=0,NITMAX(NRAY)
            XL=RAYS(1,IT,NRAY)
            ...
            ZL=RAYS(3,IT,NRAY)
            CALL WRCALK(IT,NRAY,RKPARA,RKPERP)
            GKX( IT+1,NRAY)=GUCLIP(SQRT(XL**2+YL**2))
            GKY1(IT+1,NRAY)=GUCLIP(RKPARA*VC
     &                             /(2*PI*RAYIN(1,NRAY)*1.D6))
            GKY2(IT+1,NRAY)=GUCLIP(RKPERP)
         ENDDO
...
!     ----- weight of power for each ray-------
      ...
      DO NRDIV=1,NRDIVMAX
         DO NRAY=2,NRAYMAX
            GPRY(NRDIV,1)=GPRY(NRDIV,1)+GPY(NRDIV,NRAY)*GUCLIP(FACTA)
         ENDDO
            GPRY(NRDIV,1)=GPRY(NRDIV,1)+GPY(NRDIV,1)*GUCLIP(FACTB)
      ENDDO
