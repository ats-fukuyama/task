...
!    IF(chi_cdbm.GT.1.D3) THEN
!       WRITE(6,'(A,4ES12.4)') 'bb,rr,rs,rkap     =',bb,rr,rs,rkap
!       WRITE(6,'(A,4ES12.4)') 'qp,shear,pne,rhoni=',qp,shear,pne,rhoni
!       WRITE(6,'(A,4ES12.4)') 'dpdr,dvexbdr      =',dpdr,dvexbdr
!       WRITE(6,'(A,3ES12.4)') 'calf,ckap,cexb    =',calf,ckap,cexb
!       WRITE(6,'(A,I8)')      'model             =',model
!       WRITE(6,'(A,ES12.4)')  'chi_cdbm          =',chi_cdbm
!       WRITE(6,'(A,4ES12.4)') 'ckcdbm,fs,fk,fe   =',ckcdbm,fs,fk,fe
!       WRITE(6,'(A,3ES12.4)') 'alpha,delta2,va   =',alpha,delta2,va
!       WRITE(6,'(A,3ES12.4)') 'shear,calf*,ckap* =',shear,calf*alpha,ckap*curv
!       STOP
!    END IF
    RETURN
...
! *** Form factor in CDBM model ***
  ...
  REAL(rkind) FUNCTION trcofs(shear,alpha,curv)
    ...
    REAL(rkind),PARAMETER:: shear_min=0.01D0
