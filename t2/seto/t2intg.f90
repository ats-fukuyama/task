...
MODULE T2INTG
  ...
  USE T2CNST, ONLY: i0rkind,i0ikind,rkind,ikind
   ...
   REAL(   rkind)::&
    ...
    DO k_n = 1, nnmax
       DO i_d = 1, ndmax
          ...
          DO j_q = 1, nqmax
          DO i_q = 1, nqmax
             intgNi = ifncArray(i_q,j_q,0,  i_n) 
             intgNj = ifncArray(i_q,j_q,i_d,j_n) 
             intgNk = ifncArray(i_q,j_q,0,  k_n) 
             weight = wghtArray(i_q,j_q        ) 
             sumGaussQuad = sumGaussQuad + intgNi*intgNj*intgNk*weight
             
             intgNj = ifncArray(i_q,j_q,0,  j_n) 
             intgNk = ifncArray(i_q,j_q,i_d,k_n) 
    ...
    DO k_n = 1, nnmax
       ...
       DO i_d = 1, ndmax
          ...
          ENDDO
          ENDDO
          diffTenIntgPG(i_d,j_d,k_n,i_n,i_j) = sumGaussQuad
       ENDDO
       ENDDO
    ENDDO
    ENDDO
