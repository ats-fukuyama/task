...
!!  if(mod(nt,ntstep)==0) write(6,*) INTG_F(X(LQi4,0:NRMAX))/(0.5D0*RB**2),INTG_F(X(LQi4,0:NRMAX))/INTG_F(PNiV)
  ...
  ANFINT = INTG_F(PNbV) + INTG_F(PNbrpV)
  RWINT  = INTG_F(SNB)
  WFT(1) = 0.5D0*AMb*RWINT**2*1.5D0*2.D0*PI*RR*2.D0*PI*RKAP*1.D-6!rKeV*1.D14
...
!!$       &        *PN01V(0:NRMAX)*1.D20
  SCX(0:NRMAX) = FSCX * SiVcxA(0:NRMAX) * PNiV(0:NRMAX) * PN01V(0:NRMAX) * 1.D40
  PCX(0:NRMAX) = 1.5D0*PNiV(0:NRMAX)*rNuiCXT(0:NRMAX)*1.D20*PTiV(0:NRMAX)*rKeV
