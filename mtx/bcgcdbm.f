...
C**********************************************************************
C
      SUBROUTINE BCGCDBM(A,N,M,LA,LD,B,EPS,ITR,X,D,R1,R2,P1,P2,Q1,Q2,
...
C
         IF(MYRANK.EQ.NRANK-1) THEN
            CALL MPI_SEND(R1(IEND-NBSIZ+1-ISTA+1),NBSIZ,
     &           MPI_DOUBLE_COMPLEX,MYRANK+1,1,MPI_COMM_WORLD,IERR)
         ELSE IF(MYRANK.EQ.NRANK) THEN
            IF(MYRANK.NE.0) THEN
               CALL MPI_RECV(R1(1-NBSIZ),NBSIZ,MPI_DOUBLE_COMPLEX,
     &              MYRANK-1,1,MPI_COMM_WORLD,ISTATUS,IERR)
            ENDIF
C
            DO IBN=NBST,NBED
...
C
         IF(MYRANK.EQ.NRANK+1) THEN
            CALL MPI_SEND(R1(1),NBSIZ,MPI_DOUBLE_COMPLEX,
     &           MYRANK-1,1,MPI_COMM_WORLD,IERR)
         ELSE IF(MYRANK.EQ.NRANK) THEN
            IF(MYRANK.NE.NPROCS-1) THEN
               CALL MPI_RECV(R1(IEND+1-ISTA+1),NBSIZ,
...
C
         IF(MYRANK.EQ.NRANK-1) THEN
            CALL MPI_SEND(Q1(IEND-NBSIZ+1-ISTA+1),NBSIZ,
     &           MPI_DOUBLE_COMPLEX,MYRANK+1,1,MPI_COMM_WORLD,IERR)
         ELSE IF(MYRANK.EQ.NRANK) THEN
            IF(MYRANK.NE.0) THEN
               CALL MPI_RECV(Q1(1-NBSIZ),NBSIZ,MPI_DOUBLE_COMPLEX,
     &              MYRANK-1,1,MPI_COMM_WORLD,ISTATUS,IERR)
            ENDIF
C
            DO IBN=NBST,NBED
...
C
         IF(MYRANK.EQ.NRANK+1) THEN
            CALL MPI_SEND(Q1(1),NBSIZ,MPI_DOUBLE_COMPLEX,
     &           MYRANK-1,1,MPI_COMM_WORLD,IERR)
         ELSE IF(MYRANK.EQ.NRANK) THEN
            IF(MYRANK.NE.NPROCS-1) THEN
               CALL MPI_RECV(Q1(IEND+1-ISTA+1),NBSIZ,
     &              MPI_DOUBLE_COMPLEX,MYRANK+1,1,
     &              MPI_COMM_WORLD,ISTATUS,IERR)
            ENDIF
C
...
C
      IF(X1.NE.0.D0) THEN
         RES = DSQRT(X2/X1)
C         if(myrank.eq.0) write(6,*) kk,res
         IF(RES.LE.EPS) THEN
            ITR  = KK
            IERR = 0
            EPS  = RES
