...
! TRSRC
  ...
  REAL(rkind), DIMENSION(:),     ALLOCATABLE :: & ! (NRM)
       ...
       PBIN, PFIN, POH, PRB, PRC, PRL, PRSUM, &
...
! TRUFC
  ...
  SUBROUTINE allocate_trcomm(ierr)
    ...
    ALLOCATE(AJRF(NRMAX),AJBS(NRMAX),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(SNB(NSMAX,NRMAX),SNF(NSMAX,NRMAX),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(SNB_NNB(NNBMAX,NRMAX),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(PNB_NNB(NNBMAX,NRMAX),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(PNBNR(NRMAX),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(PNFNR(NRMAX),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ...
    ALLOCATE(SPSC(NRMAX,NSM),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(PNF(NSMAX,NRMAX),PNB(NSMAX,NRMAX),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(PBIN(NRMAX),PFIN(NRMAX),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
