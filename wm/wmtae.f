...
C            EM(NWW)=DBLE(CWALFK(1-MDMIN+1,0-NDMIN+1))
         ...
         ENDDO
C
         CALL DSBTRD('N','L',MMMAX,MHMAX,FM,MMM,DM,EM,QM,1,WORK,INFO1)
C
         EPSEG=0.D0
...
C
         CALL DSTEBZ('V','B',MMMAX,VL,VU,IMIN,IMAX,EPSEG,
...
C
      ...
      CALL GVALUE(GXMIN,2*GXSTEP,0.0,0.0,NGVLEN(2*GXSTEP))
      CALL GVALUE(0.0,0.0,GYMIN,2*GYSTEP,NGVLEN(2*GYSTEP))
      CALL SETMKS(1,0.1)
      DO 200 NX=1,NXMAX
C         WRITE(6,*) NX,NGY(NX)
         DO 100 NY=1,NGY(NX)
