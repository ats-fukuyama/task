...
C
      ...
      DIMENSION NGY(NRM)
      DIMENSION RNX(NSM),RTPRX(NSM),RTPPX(NSM),RUX(NSM)
C
      DIMENSION DM(MMM),EM(MMM),FMX(MMM,MMM),QM(1,MMM),W(MMM)
...
C            ENDDO
C         ENDDO
C         RIOTA_AV=RIOTA_AV/(NTHMAX*NHHMAX)
C         BABS_AV=BABS_AV/(NTHMAX*NHHMAX)
C
...
C
               ...
               IF(NW1.GE.1.AND.NW1.LE.MHMAX+1) THEN
                 FMX(NW1,NW)=DBLE(CWALFK(MDX1,NDX1))/(RKPR1*RKPR2)
     &                      *RIOTAL**2
               ENDIF
            ENDDO
            ENDDO
