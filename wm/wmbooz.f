...
C
      DO MN=1,MNMAX
         ...
         IF(XM(MN).EQ.0.D0) THEN
            FX1(1)=0.D0
            CALL SPL1D(XSHRHO,BSPL,FX1,U1(1,1,MN),NSRMAX+1,1,IERR)
            IF(IERR.NE.0) WRITE(6,*) 'XX WMHBRZ: SPL1D: BBOZH'
            FX2(1)=0.D0
            CALL SPL1D(XSHRHO,RSPL,FX2,U2(1,1,MN),NSRMAX+1,1,IERR)
            IF(IERR.NE.0) WRITE(6,*) 'XX WMHBRZ: SPL1D: RBOZH'
            FX3(1)=0.D0
            CALL SPL1D(XSHRHO,ZSPL,FX3,U3(1,1,MN),NSRMAX+1,1,IERR)
            IF(IERR.NE.0) WRITE(6,*) 'XX WMHBRZ: SPL1D: ZBOZH'
            FX3(1)=0.D0
            CALL SPL1D(XSHRHO,PSPL,FX4,U4(1,1,MN),NSRMAX+1,1,IERR)
            IF(IERR.NE.0) WRITE(6,*) 'XX WMHBRZ: SPL1D: PBOZH'
         ELSE
            CALL SPL1D(XSHRHO,BSPL,FX1,U1(1,1,MN),NSRMAX+1,0,IERR)
            IF(IERR.NE.0) WRITE(6,*) 'XX WMHBRZ: SPL1D: BBOZH'
            CALL SPL1D(XSHRHO,RSPL,FX2,U2(1,1,MN),NSRMAX+1,0,IERR)
            IF(IERR.NE.0) WRITE(6,*) 'XX WMHBRZ: SPL1D: RBOZH'
            CALL SPL1D(XSHRHO,ZSPL,FX3,U3(1,1,MN),NSRMAX+1,0,IERR)
            IF(IERR.NE.0) WRITE(6,*) 'XX WMHBRZ: SPL1D: ZBOZH'
            CALL SPL1D(XSHRHO,PSPL,FX4,U4(1,1,MN),NSRMAX+1,0,IERR)
            IF(IERR.NE.0) WRITE(6,*) 'XX WMHBRZ: SPL1D: PBOZH'
         ENDIF
C
         DO NR=1,NRMAX+1
            CALL SPL1DD(XRHO(NR),SBMNC(MN,NR),DBMNC(MN,NR),
     &                  XSHRHO,U1(1,1,MN),NSRMAX+1,IERR)
            IF(IERR.NE.0) WRITE(6,*) 'XX WMHBRZ: SPL1DD: BMNC: NR=',NR
            CALL SPL1DD(XRHO(NR),SRMNC(MN,NR),DRMNC(MN,NR),
     &                  XSHRHO,U2(1,1,MN),NSRMAX+1,IERR)
            IF(IERR.NE.0) WRITE(6,*) 'XX WMHBRZ: SPL1DD: RMNC: NR=',NR
            CALL SPL1DD(XRHO(NR),SZMNS(MN,NR),DZMNS(MN,NR),
     &                  XSHRHO,U3(1,1,MN),NSRMAX+1,IERR)
            IF(IERR.NE.0) WRITE(6,*) 'XX WMHBRZ: SPL1DD: ZMNC: NR=',NR
            CALL SPL1DD(XRHO(NR),SPMNS(MN,NR),DPMNS(MN,NR),
     &                  XSHRHO,U4(1,1,MN),NSRMAX+1,IERR)
