...
C
      SUBROUTINE WMSETM1(CA,CB,IGD,LA,NRP)
C
      INCLUDE 'wmcomm.inc'
C
      DIMENSION CA(LA)
...
C
      IF(NR.NE.NRP) THEN
         NRP=NR
         CALL WMSETM1_M(NR,CEMP)
         CALL WMSETM1_V(NR,CFVP)
         CALL WMSETM1_B(NR,CEMP,CFVP)
      ENDIF
C
      DO MB=1,2*MBND-1
...
CYY            KKX=MOD(NC-ND-KDMIN+2*KDSIZ,KDSIZ)+1
            KK=KKX+KDMIN-1
C
            NN=NPH0+NHC*ND
            NK=NPH0+NHC*NC
...
CYY            LLX=MOD(MC-MD-LDMIN+2*LDSIZ,LDSIZ)+1
            LL=LLX+LDMIN-1
C
            MM=NTH0+MD
            ML=NTH0+MC
...
C
      SUBROUTINE WMSETM1_B(NR,CEMP,CFVP)
C
      INCLUDE 'wmcomm.inc'
C
