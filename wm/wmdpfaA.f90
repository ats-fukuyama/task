...
!        WRITE(6,'(1I5,1P4E12.4)') NTHv,Cp0N,p0N,RTA
...
!-----SINGULAR POINT
        IF((p0N.GT.0.D0).AND.(p0N.LT.0.D0)) THEN
...
!  WRITE(6,'(3I5,1P3E12.4)') j,l,NTHv,Cp0N,p0N
  WRITE(LINE,'(3I5,1P1E12.4)') j,l,NTHv,p0N
!  IF(RHO0.EQ.7.2D-1) ILST=1
  
  IF(j.EQ.1) THEN
   CALL DEFTC(CINT,ES,H0,EPS,ILST,CFUNC1,KID='CFUNC1'//TRIM(LINE))
  ELSEIF(j.EQ.2) THEN
   CALL DEFTC(CINT,ES,H0,EPS,ILST,CFUNC2,KID='CFUNC2'//TRIM(LINE))
  ELSEIF(j.EQ.3) THEN
   CALL DEFTC(CINT,ES,H0,EPS,ILST,CFUNC3,KID='CFUNC3'//TRIM(LINE))
  ELSEIF(j.EQ.4) THEN
   CALL DEFTC(CINT,ES,H0,EPS,ILST,CFUNC4,KID='CFUNC4'//TRIM(LINE))
  ELSEIF(j.EQ.5) THEN
   CALL DEFTC(CINT,ES,H0,EPS,ILST,CFUNC5,KID='CFUNC5'//TRIM(LINE))
  ELSEIF(j.EQ.6) THEN
   CALL DEFTC(CINT,ES,H0,EPS,ILST,CFUNC6,KID='CFUNC6'//TRIM(LINE))
  ELSE
   RETURN
  ENDIF
...
!!-----
  SUBROUTINE PXOUT(lo,xo,paNo,Ao)
   ...
   REAL(8),INTENT(IN) :: xo
   REAL(8),INTENT(OUT) :: paNo !paN=pN(x)
