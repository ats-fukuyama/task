...
C
      SUBROUTINE WMSETM0(CA,CB,IGD,LA,NRP)
C
      INCLUDE 'wmcomm.inc'
C
      DIMENSION CA(LA)
...
C
      IF(NR.NE.NRP) THEN
         NRP=NR
         CALL WMSETM0_M(NR,CEMP)
         CALL WMSETM0_V(NR,CFVP)
         CALL WMSETM0_B(NR,CEMP,CFVP)
      ENDIF
C
      DO MB=1,2*MBND-1
...
C
      SUBROUTINE WMSETM0_B(NR,CEMP,CFVP)
C
      INCLUDE 'wmcomm.inc'
C
