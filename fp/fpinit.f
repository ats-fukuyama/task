...
C
      LOGICAL LEX
      CHARACTER KPNAME*32
      NAMELIST /FP/ BB,RR,RA,RB,RKAP,RDLT,QA,Q0,
     &              NSMAX,PA,PZ,PN,PNS,PTPR,PTPP,PTS,PZCL,
     &              ALPHN,ALPHT,MODELR,MODELG,MODELP,
     &              ZEFF,DRR0,E0,R1,DELR1,RMIN,RMAX,
     &              DEC,PEC1,PEC2,RFEC,DELYEC,
     &              DLH,PLH1,PLH2,RLH,DFW,PFW1,PFW2,RFW,
     &              NPMAX,NTHMAX,NRMAX,PMAX,RIMPL,
     &              EPSM,EPSE,LMAXE,EPSDE,H0DE,
     &              DELT,NTMAX,NTSTP1,NTSTP2,NTSTPC,
     &              MODELE,MODELA,MODELC,MODELW,NGLINE,
     &              NAVMAX,LLMAX,LMAXNWR,EPSNWR,
     &              RFDW,DELNPR,NCMIN,NCMAX,PWAVE,
     &              CEWR,CEWTH,CEWPH,RKWR,RKWTH,RKWPH,REWY,DREWY
C
    1 WRITE(6,*) '## INPUT &FP : '
      READ(5,FP,ERR=1,END=9000)
C
 3000 IERROR=0
      ...
      IF(MOD(NTHMAX,2).NE.0) THEN
         ...
         IERROR=1
      ENDIF
      IF(IERROR.NE.0) GOTO 1
C
      GOTO 9000      
C
 8000 WRITE(6,*) '&FP : RA,RR,BB,RKAP,Q0,QA,',
     &                 'ALPHN,ALPHT,ZEFF,DRR0,E0,'
      WRITE(6,*) '      DEC,PEC1,PEC2,RFEC,DELYEC,',
     &                 'DLH,PLH1,PLH2,RLH,DFW,PFW1,PFW2,RFW,'
      WRITE(6,*) '      NPMAX,NTHMAX,NRMAX,NAVMAX,PMAX,RIMPL,',
     &                 'EPSM,EPSE,LMAXE,EPSDE,H0DE,'
      WRITE(6,*) '      R1,DELR1,RMIN,RMAX,'
      WRITE(6,*) '      DELT,NTMAX,NTSTP1,NTSTP2,NTSTPC,'
      WRITE(6,*) '      MODELE,MODELR,MODELA,MODELC,MODELW,MODELG,'
      WRITE(6,*) '      RF,DELNPR,NCMIN,NCMAX,'
      WRITE(6,*) '      CEWR,CEWTH,CEWPH,RKWR,RWKTH,RWKPH,REWY,DREWY'
      WRITE(6,*) '      NGLINE,LLMAX,PWAVE,LMAXNWR,EPSNWR'
      GOTO 1
C
 9000 RETURN
C
...
C
  ...
  603 FORMAT(1H ,A6,'=',1PE11.3:2X,A6,'=',1PE11.3:
     &        2X,A6,'=',1PE11.3:2X,A6,'=',I7)
