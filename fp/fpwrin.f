...
C
      ...
      INCLUDE 'fpcom2.h'
C
      DIMENSION IDISP(NSM),NDISP1(NSM),NDISP2(NSM)
...
C
   30 CONTINUE
      READ(21) BB,RR,RA,RB,RKAP,RDLT,Q0,QA
      READ(21) NSMAX
      READ(21) (PA(NS),PZ(NS),PN(NS),PNS(NS),PZCL(NS),NS=1,NSMAX)
      READ(21) (PTPR(NS),PTPP(NS),PTS(NS),NS=1,NSMAX)
      READ(21) ALPHN,ALPHT
      READ(21) (IDISP(NS),NDISP1(NS),NDISP2(NS),NS=1,NSMAX)
      READ(21) MODELR,MODELG,MODELF
