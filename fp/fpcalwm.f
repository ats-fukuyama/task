...
c      THETA0=RTFP0*1.D3*AEE/(AMFP*VC*VC)
      ...
      CALL FPSETV(RHOL,ETAL,RKR,RKTH,RKPH,CER,CETH,CEPH,NSA)
...
c            A22=((1.D0-RKW*VPARA)*PCOS - RKW*VPERP*PSIN )**2*RTHETA2
            A11= RKW**2*PCOS**2  *RTHETA2
            A12=-RKW**2*PCOS*PSIN*RTHETA2
            A21=-RKW**2*PCOS*PSIN*RTHETA2
            A22= RKW**2*PSIN**2  *RTHETA2
C            IF(ABS(A22).GT.1.D10) THEN
C               WRITE(6,'(1P6E12.4)') A22,RKW,VPERP,PSIN,RTHETA2
C               PAUSE
C            ENDIF
            ENDIF
	 ELSE
...
c     &         *((1.D0-RKW*VPARA)*PCOS - RKW*VPERP*PSIN )*RTHETA2
c            A22=((1.D0-RKW*VPARA)*PCOS - RKW*VPERP*PSIN )**2*RTHETA2
            A11=(N*RWC/RW/RGAMMA*PSIN + RKW*PPERP*PCOS )**2*RTHETA2
            A12=(N*RWC/RW/RGAMMA*PSIN + RKW*PPERP*PCOS )
     &         *(N*RWC/RW/RGAMMA*PCOS - RKW*PPERP*PSIN )*RTHETA2
            A21=(N*RWC/RW/RGAMMA*PSIN + RKW*PPERP*PCOS )
     &         *(N*RWC/RW/RGAMMA*PCOS - RKW*PPERP*PSIN )*RTHETA2
            A22=(N*RWC/RW/RGAMMA*PCOS - RKW*PPERP*PSIN )**2*RTHETA2
            END IF
         ENDIF
C         IF(ABS(A11)+ABS(A22).GT.1.D10) THEN
C            WRITE(6,'(I5,1P5E12.4)') N,GZI,A11,A12,A21,A22
C            PAUSE
C         ENDIF
...
C
      ...
      DWTTL = DWC22
      END IF
C      IF(DWTTL.GT.1.D5) THEN
C         WRITE(6,'(1P6E12.4)') A11,A12,A21,A22,DWPPL,DWTTL
C      ENDIF
C
      RETURN
      END
...
C
      SUBROUTINE FPSETV(RHOL,ETAL,RKR,RKTH,RKPH,CER,CETH,CEPH,NSA)
C
      INCLUDE 'fpcomm.inc'
C
      IF(MODELW(NSA).EQ.3) THEN
         Y=RHOL*RA*SIN(ETAL)
C
         DREWY=1.D1
         REWY=0.D0
