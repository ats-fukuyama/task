...
!------------------------------------------
  ...
  SUBROUTINE FP_CALW
    ...
    double precision,dimension(nthmax,npmax,nrmax,nsamax):: recv_pp2, recv_pt2
...
!   CALL update_Dpcoef(DWPP,recv_pp)
!   CALL update_Dpcoef(DWPT,recv_pt)
!   CALL update_Dpcoef(DPP,recv_pp2)
!   CALL update_Dpcoef(DPT,recv_pt2)
!   IF(NRANK.eq.0)THEN
!      DO NP=1, NPMAX
!         DO NTH=1, NTHMAX
!            WRITE(20,'(10E14.6)') TIMEFP, PG(NP,1)*COSM(NTH), PG(NP,1)*SINM(NTH), &
!                 recv_pp(NTH,NP,1,1), recv_pt(NTH,NP,1,1), &
!                 recv_pp2(NTH,NP,1,1), recv_pt2(NTH,NP,1,1)
!         END DO
!      END DO
!      WRITE(20,*) " "
!      WRITE(20,*) " "
...
!                  DWECPT(NTH,NP,NR,NSA)=-SINM(NTH)*DECB
            ...
            ENDDO
  101       CONTINUE
            IF(NPENDWG.eq.NPMAX+1) DWECPP(NTH,NPENDWG,NR,NSA)=0.D0
            IF(NPENDWG.eq.NPMAX+1) DWECPT(NTH,NPENDWG,NR,NSA)=0.D0
         ENDDO
!
         IF(MODELA.EQ.1) THEN
!            DO NP=1,NPMAX+1
...
!                     DWECTP(NTH,NP,NR,NSA)=+SING(NTH)*DECB
                     DWECTP(NTH,NP,NR,NSA)= DECB
                  ELSE
!                     DWECTP(NTH,NP,NR,NSA)=-SING(NTH)*DECB
                     DWECTP(NTH,NP,NR,NSA)= DECB
                  ENDIF
!                  DWECTT(NTH,NP,NR,NSA)=             DECC/ABS(COSG(NTH))
                  DWECTT(NTH,NP,NR,NSA)=    DECC
               ENDDO
            ENDIF
         ENDDO
!
...
!         SUM5=SUM5+DECL*PCOS/RCOS/PSI
         SUM3=SUM3+DECL*RCOS/PCOS *PCOS**2
         SUM4=SUM4+DECL/SQRT(PSI) *(-PSIN*PCOS)
         SUM5=SUM5+DECL*PCOS/RCOS/PSI *PSIN**2
...
!
      ...
      integer:: ISW_DEC
      double precision:: gamma, threshold
!
      NS=NS_NSA(NSA)
...
!
         ...
         IF(ISW_DEC.eq.0)THEN
            ...
            ENDIF
         ELSE
            gamma=SQRT(1.D0+ (PPARA**2+PPERP**2)*THETA0(1) )
!            threshold = AMFP(1)*VC*gamma/PTFP0(1)
            threshold = gamma*1.D1
            IF(threshold*0.4D0.lt.PPARA.and.PPARA.lt.threshold*0.7D0)THEN
!            IF(3.D0.lt.PPARA.and.PPARA.lt.5.D0)THEN
               DECL=DEC
            ELSE
               DECL=0.D0
            END IF
