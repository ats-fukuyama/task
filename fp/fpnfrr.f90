...
!---- calculation of relative kinetic energy ----
      ...
      REAL(rkind):: VPARA1, VPARA2, VPERP1, VPERP2, V_MS, VA_MS
      REAL(rkind):: RED_MASS
      INTEGER:: NS1, NS2
...
!         cs_tt = SIGMAV_TT(RT_BULK(NR,NS1),ID)*1.D20*double_count*RN_BULK(NR,NS1)*RN_BULK(NR,NS2)
...
!*********************************************************
      ...
      cs_tt = double_count*SIGMAV_TT(RT_BULK(NR,NS1),ID)*1.D20*RN_BULK(NR,NS1)*RN_BULK(NR,NS2)
      RATE_NF_TT(NR,ID) = cs_tt
...
!     out RATE_NF, _BB, _TT, _D1, _D2. MODELS=-2 does not use _D1, _D2.
      ...
      real(rkind):: double_count, RSUM3, RSUM_B2, RSUM_sum, RSUM_TT
