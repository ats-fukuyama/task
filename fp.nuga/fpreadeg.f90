...
!      WRITE(35,'(4E14.6)') TIMEFP, RN_TEMP(1,1), RN_TEMP(1,2), RN_TEMP(1,3)
      WRITE(37,'(4E14.6)') TIMEFP, RN_BULK(1,1), RN_BULK(1,2), RN_BULK(1,3)
...
!------------------------------------
      ...
      integer:: i_row,j_column,NS,judge,judge_zeff,imax
...
! 2  eq. of Zeff
      IF(judge_zeff.eq.1)THEN
         i_row=i_row+1
!         IF(MODEL_DELTA_F_CN.eq.0)THEN ! ne != sum Z_i*n_i
!            DO j_column=1,NSMAX-1
!               Aji(i_row,j_column) = PZ(j_column+1)**2 - given_zeff*PZ(j_column+1)
!            END DO
!            bk(i_row) = 0.D0
!         ELSEIF(MODEL_DELTA_F_CN.eq.1)THEN
            DO j_column=1,NSMAX-1
...
! 4  eq. of HHe_RATIO ! Here, H means H and D. 
