...
!
      ...
      real(8),dimension(NSAMAX):: rtotal_RSP_IC
      character:: fmt0*50
!
      WRITE(6,*)"--------------------------------------------"
      WRITE(6,*)"-----Global data"
...
!==============================================================
      ...
      DO NSA=NSASTART,NSAEND
         ...
         DO NR=NRSTART,NREND
!-------    Calculation of bulk temperature
            CALL BULK_TEMPERATURE(NP_BULK(NR,NSA),NR,NSA)  ! RTL_BULK is updated
!-------    Calculation of bulk temperature
         ENDDO ! NSA
      ENDDO ! NR
      CALL FPSAVECOMM2
      ...
      IF(MODEL_EX_READ_Tn.eq.0)THEN
         ...
         DO NSA=1,NSAMAX
            ...
            IF(MODEL_DELTA_F(NS).eq.0)THEN
               ...
               END DO
!            ELSE !If MDOEL_DELTA_F=1, RN_TEMP, RT_BULK and RT_TEMP are const
            END IF
         ENDDO
      ELSEIF(MODEL_EX_READ_Tn.ne.0)THEN
         CALL MAKE_EXP_PROF(timefp+delt) ! update not rn_bulk, but rn_temp and rt_temp.
! IF MODEL_DELTA_F_NI_RATIO.eq.2, RN_BULK is also updated.
      END IF
