...
100    CONTINUE
    ...
    IF(NMODEL.EQ.7.OR.NMODEL.EQ.5.OR.NMODEL.EQ.3.OR.NMODEL.EQ.1) THEN
       DO NX=1,NXMAX
          NN        =NXPRNT(NX)
...
!    END IF
    ...
    DO NX=1,NXMAX
       NXN=NXPRNT(NX)
       DO NZ=1,NZMAX
          GFR(NX,NZ)=DBLE(CE2DA(NZ,NXN,1))*RNORM
          GFI(NX,NZ)=IMAG(CE2DA(NZ,NXN,1))*RNORM
       END DO
    END DO

    CALL GRD2D(1,GX,GZ,GFR,NXMAX,NXMAX,NZMAX,'@Re(Ex)@', &
               XMIN=-RB,XMAX=RB,YMIN=0.D0,YMAX=RZ, &
    ...
    DO NX=1,NXMAX
       NXN=NXPRNT(NX)
       DO NZ=1,NZMAX
          GFR(NX,NZ)=DBLE(CE2DA(NZ,NXN,2))*RNORM
          GFI(NX,NZ)=IMAG(CE2DA(NZ,NXN,2))*RNORM
       END DO
    END DO
    ...
    DO NX=1,NXMAX
       NXN=NXPRNT(NX)
       DO NZ=1,NZMAX
          GFR(NX,NZ)=DBLE(CE2DA(NZ,NXN,3))*RNORM
          GFI(NX,NZ)=IMAG(CE2DA(NZ,NXN,3))*RNORM
       END DO
    END DO
  ...
  SUBROUTINE W1GR1DJ
    ...
    CASE(1)
       DO NZ=1,NZMAX
          ...
          FDATA(NZ,2)=IMAG(CWG1(NZ))
       END DO
    CASE(2)
       DO NZ=1,NZMAX
          FDATA(NZ,1)=REAL(CWG2(NZ))
          FDATA(NZ,2)=IMAG(CWG2(NZ))
       END DO
    CASE(3)
       DO NZ=1,NZMAX
          FDATA(NZ,1)=REAL(CWG3(NZ))
          FDATA(NZ,2)=IMAG(CWG3(NZ))
       END DO
    CASE(4)
       DO NZ=1,NZMAX
          FDATA(NZ,1)=REAL(CWG4(NZ))
          FDATA(NZ,2)=IMAG(CWG4(NZ))
       END DO
    END SELECT
    ...
    CASE(1)
       DO NZ=1,NZMAX
          ...
          FDATA(NZ,2)=IMAG(CWG1(NSHIFT(NZ)))
       END DO
    CASE(2)
       DO NZ=1,NZMAX
          FDATA(NZ,1)=REAL(CWG2(NSHIFT(NZ)))
          FDATA(NZ,2)=IMAG(CWG2(NSHIFT(NZ)))
       END DO
    CASE(3)
       DO NZ=1,NZMAX
          FDATA(NZ,1)=REAL(CWG3(NSHIFT(NZ)))
          FDATA(NZ,2)=IMAG(CWG3(NSHIFT(NZ)))
       END DO
    CASE(4)
       DO NZ=1,NZMAX
          FDATA(NZ,1)=REAL(CWG4(NSHIFT(NZ)))
          FDATA(NZ,2)=IMAG(CWG4(NSHIFT(NZ)))
       END DO
    END SELECT
