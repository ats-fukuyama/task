...
C
      SUBROUTINE W1GRUD(NGRAPHL)
C
      USE w1comm
      USE w1comm_gdat
      IMPLICIT NONE
      INTEGER,INTENT(IN):: NGRAPHL
      INTEGER:: NZ,NX,NN,NS
...
C
      ...
      DO 1040 NS=1,NSMAX
         ...
         DO 1039 NX=1,NXP
            GDATA1(NX)=PABSX(NX,NS)*RNORM2
 1039    CONTINUE
         CALL GPLOTP(GXM,GDATA1,1,NXP,1,-NS,10,0)
 1040 CONTINUE
      CALL SETLIN(0,2,4)
C
...
C
      SUBROUTINE W1GPRM(NZ,NZPL)
C
      USE w1comm
      IMPLICIT NONE
      INTEGER,INTENT(IN):: NZ,NZPL
      INTEGER:: NS
      REAL(rkind):: RNORM2
C
...
C
 ...
 1220 CONTINUE
      ...
      CALL MOVE(14.,13.8)
      CALL TEXT('T-PR =',6)
      DO 1230 NS=1,MIN(NSMAX,4)
         CALL SETLIN(0,0,7-NS)
         CALL NUMBD(PTPR(NS),'(F8.3)',8)
 1230 CONTINUE
C
      IF(NZPL.EQ.1.AND.NZ.EQ.0) RETURN
C
      CALL SETLIN(0,0,7)
      CALL MOVE(14.,13.3)
      CALL TEXT('P/TOT=',6)
      DO 1240 NS=1,MIN(NSMAX,4)
         CALL SETLIN(0,0,7-NS)
...
C
      SUBROUTINE W1GR2D(NGRAPHL)
C
      USE w1comm
      USE w1comm_gdat
      IMPLICIT NONE
      INTEGER,INTENT(IN):: NGRAPHL
      INTEGER:: NS,NSS,NZ,NZZ,NX,NN
      REAL(rkind):: RNORM2
...
C
 ...
 1005 CONTINUE     
      GGZ(1    )=-AKZ(NZP/2+1)
      CALL GQSCAL(GGZ(1),GGZ(NZP+1),GZMIN,GZMAX,GZSCL)
C
      DO 1006 NZ=1,NZP
...
C
      CALL SETLIN(0,0,4)
      IF(NGRAPHL.EQ.3) THEN
         CALL GDEFIN(2.5,12.5,1.0,12.0,GGZ(1),GGZ(NZP+1),-0.1*GMAX,GMAX)
      ELSE
         CALL GDEFIN(2.5,12.5,1.0,6.5,GGZ(1),GGZ(NZP+1),-0.1*GMAX,GMAX)
      ENDIF
      CALL GFRAME
 ...
 2003 CONTINUE
      CALL GPLOTP(GGZ,GDATAZ,1,NZP+1,1,0,0,0)
      DO 2005 NZ=1,NZP+1
         GDATAZ(NZ)=0.0
 2005 CONTINUE
      DO 2010 NS=1,NSMAX
 ...
 2009    CONTINUE
         CALL SETLIN(0,0,7-NS)
         CALL GPLOTP(GGZ,GDATAZ,1,NZP+1,1,-NS,10,0)
 2010 CONTINUE
C
      IF(NGRAPHL.NE.2.AND.NGRAPHL.NE.3) THEN
         DO 2020 NZ=1,NZP+1
            IF(NZ.LE.NZP/2) THEN
               NZZ = NZ + NZP/2
            ELSE
 ...
 2020    CONTINUE
         ...
         CALL SETLIN(0,0,4)
         CALL GQSCAL(GAMIN,GAMAX,GMIN,GMAX,SCAL)
         CALL GDEFIN(2.5,12.5,6.5,12.0,GGZ(1),GGZ(NZP+1),GMIN,GMAX)
         CALL GFRAME
         CALL GSCALE(0.,  GZSCL,0.,  SCAL,0.1,9)
...
C
      SUBROUTINE W1_GDSPX(PKXMIN,PKXMAX)
C
      USE w1comm
      REAL(rkind),INTENT(IN):: PKXMIN,PKXMAX
      INTEGER:: NX,KK
      REAL(4):: SX(NXPM),SKR1(NXPM),SKR2(NXPM),SKR3(NXPM)
      REAL(4):: SKI1(NXPM),SKI2(NXPM),SKI3(NXPM),SQK2R,SQK2I,S1,S2
      REAL(4):: SXMIN,SXMAX,SKMIN,SKMAX,SCALX,SCALK,SXORG,SKORG,SCALV
C
