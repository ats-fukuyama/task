...
!     ****** SLAVE FUNCTION FOR DE INTEGRATEION ******
  ...
  FUNCTION W1FNA1(X,XP,XM)
    ...
    ARG=Y1*XX
    IF(ID.EQ.1) THEN
       BES=NN*NN*XX*BESJN(NN,ARG)**2/(Y1*Y1)
    ELSEIF(ID.EQ.2) THEN
       BES=NN*XX*XX*BESJN(NN,ARG) &
                   *(BESJN(NN-1,ARG)-BESJN(NN+1,ARG))*0.5D0/Y1
    ELSEIF(ID.EQ.3) THEN
       BES=NN*Y2*XX*BESJN(NN,ARG)**2/Y1
    ELSEIF(ID.EQ.4) THEN
       BES=0.25D0*XX*XX*XX*(BESJN(NN-1,ARG)-BESJN(NN+1,ARG))**2
    ELSEIF(ID.EQ.5) THEN
       BES=Y2*XX*XX*BESJN(NN,ARG) &
                  *(BESJN(NN-1,ARG)-BESJN(NN+1,ARG))*0.5D0
    ELSEIF(ID.EQ.6) THEN
       BES=Y2*Y2*XX*BESJN(NN,ARG)**2
    END IF
    Y=SQRT(XX*XX+Y2*Y2)
