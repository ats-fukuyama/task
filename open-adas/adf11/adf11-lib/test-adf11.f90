...
PROGRAM test_adf11
  ...
  INTEGER:: NL,NLMAX,NDI,NDR,NPTMAX,I
  REAL(dp):: PN,PT,DR,DRMIN,DRMAX,XMIN,XMAX,DX,YMIN,YMAX,DY
  REAL(dp):: PZAV,PZ2AV,PWBAV,PWLAV
  REAL(dp):: PNMIN,PNMAX,PTMIN,PTMAX,DPT
...
2 CONTINUE
  ...
  WRITE(6,*) '                    21,22,23 for Ionized eq'
  WRITE(6,*) '                    0 for end'
...
22   CONTINUE
     WRITE(6,'(A)') '## Input PN,PTMIN,PTMAX,NPTMAX (-10 for end):'
     READ(5,*,ERR=1,END=9000) PN,PTMIN,PTMAX,NPTMAX
     IF(PN.LE.-10.D0) GOTO 2
     ...
     CALL PAGES
     CALL GRD1D(0,XDATA,FDATA,NXMAX+1,NXMAX+1,NYMAX,'@PNZ vs Z for PT@',2)
     CALL PAGEE
...
23   CONTINUE
     WRITE(6,'(A)') '## Input PN,PTMIN,PTMAX,NXMAX (-10 for end):'
     READ(5,*,ERR=1,END=9000) PN,PTMIN,PTMAX,NXMAX
     IF(PN.LE.-10.D0) GOTO 2
     ...
     CALL GRD1D(2,XDATA,FDATA(1:NXMAX,2),NXMAX,NXMAX,1,'@PZ2AV vs PT@',1)
     CALL GRD1D(3,XDATA,FDATA(1:NXMAX,3),NXMAX,NXMAX,1,'@PWBAV vs PT@',1)
     CALL GRD1D(4,XDATA,FDATA(1:NXMAX,4),NXMAX,NXMAX,1,'@PWLAV vs PT@',1)
     CALL PAGEE
