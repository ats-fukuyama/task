  ...
  SUBROUTINE  READ_ADF11(IERR)
    ...
    DO ND=1,NDMAX
       WRITE(6,'(9I4,1X,A)') ND,IZ0A(ND),IM0A(ND),ICLASSA(ND),IDMAXA(ND), &
             ITMAXA(ND),ISMAXA(ND),IS1MINA(ND),IS1MAXA(ND),TRIM(KFNAMA(ND))
    END DO
...
!        
  ...
  SUBROUTINE CALC_ADF11(ND,IZ,PN,PT,DR,IERR)
    ...
    IF(IS.LT.IS1MINA(ND)) IS=IS1MINA(ND)
    IF(IS.GT.IS1MAXA(ND)) IS=IS1MAXA(ND)
...
9019 WRITE(6,*) 'XX LOAD_ADF11_bin: READ END OF FILE 19: IOSTAT=',IST
  ...
  SUBROUTINE IONIZE_EQ(IZ0)
    USE libgrf
    IMPLICIT NONE
    INTEGER,INTENT(IN):: IZ0
    INTEGER:: NDI,NDR,NXMAX,NX,IZ,IERR
    REAL(dp):: PN,PT,DR,PNZTOT,PZTOT,PZ2TOT,RATIO
    ...
    WRITE(6,'(A,1P2E12.4)') &
          'Log10_PN(n20): ',DDENSA(1,NDI)-14.D0,DDENSA(IDMAXA(NDI),NDI)-14.D0
    WRITE(6,'(A,1P2E12.4)') &
          'Log10_PT(keV): ',DTEMPA(1,NDI)-3.D0,DTEMPA(ITMAXA(NDI),NDI)-3.D0

1   CONTINUE

    WRITE(6,'(A)') '## Input Log10_PN and Log10_PT (-10 for end):'
    READ(5,*,ERR=1,END=9000) PN,PT
    IF(PN.LE.-10.D0) GOTO 9000
    ...
    ALLOCATE(DRI(0:NXMAX),DRR(0:NXMAX),PNZ(0:NXMAX))
    ALLOCATE(XDATA(0:NXMAX),FDATA(0:NXMAX,2))
    ...
    CMAT(3,1)= DRR(0)
    DO NX=1,NXMAX-1
       CMAT(1,NX+1)=  DRI(NX-1)
       CMAT(2,NX+1)=-(DRI(NX)+DRR(NX-1))
       CMAT(3,NX+1)=  DRR(NX)
    END DO
    CMAT(1,NXMAX+1)=  DRI(NXMAX-1)
    CMAT(2,NXMAX+1)= -DRR(NXMAX-1)
    CMAT(3,NXMAX+1)=  0.D0
...
!       write(6,'(A,I5,1P2E12.4)') 'IZ,PNZ,R/I=',IZ,PNZ(IZ),RATIO
    ...
    DO IZ=1,NXMAX
       ...
       PZTOT =PZTOT +IZ*PNZ(IZ)
       PZ2TOT=PZ2TOT+IZ*IZ*PNZ(IZ)
    END DO

    write(6,'(1P3E12.4)') PNZTOT,PZTOT,PZ2TOT
    DO IZ=0,NXMAX
       PNZ(IZ)=PNZ(IZ)/PNZTOT
       IF(PNZ(IZ).LT.1.D-30) PNZ(IZ)=1.D-30
    END DO
    PZTOT=PZTOT/PNZTOT
    PZ2TOT=PZ2TOT/PNZTOT
    ...
    WRITE(6,'(A,1PE12.4)') 'ZEFF   = ',PZTOT
    WRITE(6,'(A,1PE12.4)') 'ZEFF**2= ',PZ2TOT
