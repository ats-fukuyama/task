C     $Id: wmgout.f,v 1.23 2012/08/20 01:27:15 fukuyama Exp $
C
C     ****** CONTROL GRAPHICS ******
C
      SUBROUTINE WMGOUT
...
C
      ...
      GYMAX=GY(MDSIZ+1)
C
C      CALL GMNMX2(GZL,NRM,1,NX,1,1,MDSIZ+1,1,GZMIN,GZMAX)
C!! seki
       CALL GMNMX2(GZL,NRM,1,NX,1,1,MDSIZ+1,1,GZMIN,GZMAX)
       print *, GZMIN,GZMAX,NX
       DO NR=1,NX
          if ( 1d0 > xrho(NR) ) then
             NRLCFS=NR
          endif
       ENDDO
       NRLCFS=NRLCFS+1
       CALL GMNMX2(GZL,NRM,1,NRLCFS,1,1,MDSIZ+1,1,GZMIN,GZMAX)
       print *, GZMIN,GZMAX,NRLCFS
C
