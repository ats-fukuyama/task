C     $Id: wmvmec.f,v 1.23 2013/01/20 23:24:03 fukuyama Exp $
C
...
C            
         RPST_IPS(  NTH,NHH,NR)=RPSS
         ZPST_IPS(  NTH,NHH,NR)=ZPSS
C
         IF(NR.NE.1) THEN
            DRS=DRS/(2.D0*PSIPA*XRHO(NR))
            DZS=DZS/(2.D0*PSIPA*XRHO(NR))
            RG11_IPS(NTH,NHH,NR)=(DRS *DRS +DZS *DZS )*XRHO(NR)**2
            RG12_IPS(NTH,NHH,NR)=(DRS *DRTH+DZS *DZTH)
            RG13_IPS(NTH,NHH,NR)=(DRS *DRPH+DZS *DZPH)*XRHO(NR)
            RG22_IPS(NTH,NHH,NR)=(DRTH*DRTH+DZTH*DZTH)/XRHO(NR)**2
            RG23_IPS(NTH,NHH,NR)=(DRTH*DRPH+DZTH*DZPH)/XRHO(NR)
            RG33_IPS(NTH,NHH,NR)= DRPH*DRPH+DZPH*DZPH+DPHPH*DPHPH
         ENDIF
      ENDDO
      ENDDO
      ENDDO
C
      DO NHH=1,NHHMAX_IPS_F
      DO NTH=1,NTHMAX_IPS_F
         RG11_IPS(NTH,NHH,1)=RG11_IPS(NTH,NHH,2)
         RG12_IPS(NTH,NHH,1)=RG12_IPS(NTH,NHH,2)
         RG13_IPS(NTH,NHH,1)=RG13_IPS(NTH,NHH,2)
         RG22_IPS(NTH,NHH,1)=RG22_IPS(NTH,NHH,2)
         RG23_IPS(NTH,NHH,1)=RG23_IPS(NTH,NHH,2)
         RG33_IPS(NTH,NHH,1)=RG33_IPS(NTH,NHH,2)
      ENDDO
      ENDDO
C
...
C
         ...
         ENDDO
         BFLD_IPS(2,NTH,NHH,NR)=SBTH
         BFLD_IPS(3,NTH,NHH,NR)=SBPH
      ENDDO
      ENDDO
      ENDDO
!      DTH=2.D0*PI/NTHMAX
