C     $Id: wmprof.f,v 1.15 2013/01/20 23:24:03 fukuyama Exp $
C
C     ****** DENSITY PROFILE ******
C
...
C     ****** MAGNETIC FIELD PROFILE ******
C
      SUBROUTINE WMCMAG_F(NR,NTHF,NHHF,BABS,BSUPTH,BSUPPH)
C
      INCLUDE 'wmcomm.inc'
C
      BSUPTH=BFLD(2,NTHF,NHHF,NR)
...
C     ****** MAGNETIC FIELD PROFILE ******
C
      SUBROUTINE WMCMAG_IPS_F(NR,NTHF,NHHF,BABS,BSUPTH,BSUPPH)
C
      INCLUDE 'wmcomm.inc'
C
      BSUPTH=BFLD_IPS(2,NTHF,NHHF,NR)
      BSUPPH=BFLD_IPS(3,NTHF,NHHF,NR)
      BABS=BPST_IPS(NTHF,NHHF,NR)
C
      RETURN
      END
C
