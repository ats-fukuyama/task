...
MODULE libde
  ...
  PRIVATE
  PUBLIC DEFT,DEFTAB,DEFTC,DEFTCAB,DEHIFT,DEHIFTC,DEHIFE,DEHIFEC
...
!                 otherwize      y=(b-a)*x/2+(a+b)/2
  ...
  501 FORMAT(A1)
  601 FORMAT(1H ,ES13.5,2I8,ES24.16)
  602 FORMAT(1H ,ES13.5,2I8,ES24.16,ES14.6)
...
!        FINITE INTEGRAL BY DOUBLE-EXPONENTIAL FORMULA
!                    (a,b)
!         INTEGRAND SHOULD BE DEFINED BY FUNC(X,b-X,X-a)
!     ************************************************************
!
!        For integral with respect to y from a to b,
!            variable transformation should be
...
!                 otherwize      y=(b-a)*x/2+(a+b)/2
  ...
  SUBROUTINE DEFTAB(A,B,CS,ES,H0,EPS,ILST,FUNC,KID)
    IMPLICIT NONE
    REAL(dp),INTENT(IN)::  A    ! x_minimum
    REAL(dp),INTENT(IN)::  B    ! x_maximum
    REAL(dp),INTENT(OUT):: CS   ! Integral
    REAL(dp),INTENT(OUT):: ES   ! Estimated error 
    REAL(dp),INTENT(IN)::  H0   ! Initial step size
    REAL(dp),INTENT(IN)::  EPS  ! Convergence thrshold
    ...
    REAL(dp):: EPS1,H,X,CSI,CSP,ATP,ATM,HN,HC,HS,CC,BY,YA,CT,AT,Y
    INTEGER:: NP,NM,NPMIN,NMMIN,IND,NPD,NMD,NMAX
      ...
      X=0.D0
      Y=0.5D0*(A+B)
      CSI=HP*0.5D0*(B-A)*FUNC(Y,B-Y,Y-A)
      CS=H*CSI
      CSP=0.D0
      NP=0
      NM=0
   ...
   10 IF(IND.NE.1) THEN
         ...
         HS=HP*SINH(-HN)
         X=TANH(HS)
         Y=0.5D0*(B-A)*X+0.5D0*(A+B)
         CC=1.D0/COSH(HS)
         BY=0.5D0*(B-A)*CC*EXP(-HS)
         YA=0.5D0*(B-A)*CC*EXP( HS)
         CT=HC*0.5D0*(B-A)*FUNC(Y,BY,YA)*CC*CC
         CS=CS+CT
         AT=ATP
         ATP=ABS(CT)/H
         IF(NP.GE.NPMIN) THEN
  ...
  601 FORMAT(1H ,ES13.5,2I8,ES24.16)
  602 FORMAT(1H ,ES13.5,2I8,ES24.16,ES14.6)
  603 FORMAT(1H ,'XX DEFTAB: NMAX EXCEEDS 1000: FUNC=',A)
 9999 STOP
...
!                 otherwize      y=(b-a)*x/2+(a+b)/2
  ...
  501 FORMAT(A1)
  601 FORMAT(1H ,ES13.5,2I8,2ES18.10)
  602 FORMAT(1H ,ES13.5,2I8,2ES18.10,ES14.6)
  603 FORMAT(1H ,'XX DEFT: NMAX EXCEEDS 1000: FUNC=',A)
 9999 STOP
...
!                 otherwize      y=(b-a)*x/2+(a+b)/2
  ...
  SUBROUTINE DEFTCAB(A,B,CS,ES,H0,EPS,ILST,CFUNC,KID)
    IMPLICIT NONE
    REAL(dp),INTENT(IN)::  A    ! x_minimum
    REAL(dp),INTENT(IN)::  B    ! x_maximum
    COMPLEX(dp),INTENT(OUT):: CS   ! Integral
    ...
    COMPLEX(dp):: CSI,CSP,CT
    REAL(dp):: EPS1,H,X,ATP,ATM,HN,HC,HS,CC,BY,YA,AT,Y
    INTEGER:: NP,NM,NPMIN,NMMIN,IND,NPD,NMD,NMAX
      ...
      X=0.D0
      Y=0.5D0*(A+B)
      CSI=HP*0.5D0*(B-A)*CFUNC(Y,B-Y,Y-A)
      CS=H*CSI
      CSP=0.D0
      NP=0
      NM=0
   ...
   10 IF(IND.NE.1) THEN
         ...
         BY=0.5D0*(B-A)*CC*EXP(-HS)
         YA=0.5D0*(B-A)*CC*EXP( HS)
         CT=HC*0.5D0*(B-A)*CFUNC(Y,BY,YA)*CC*CC
         CS=CS+CT
         AT=ATP
         ATP=ABS(CT)/H
         IF(NP.GE.NPMIN) THEN
  ...
  601 FORMAT(1H ,ES13.5,2I8,2ES18.10)
  602 FORMAT(1H ,ES13.5,2I8,2ES18.10,ES14.6)
  603 FORMAT(1H ,'XX DEFT: NMAX EXCEEDS 1000: CFUNC=',A)
 9999 STOP
 END SUBROUTINE DEFTCAB
