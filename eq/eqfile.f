...
C      WRITE(21) ((UQPV(NA,NRV),NA=1,4),NRV=1,NRVMAX)
C      WRITE(21) ((UPSITV(NA,NRV),NA=1,4),NRV=1,NRVMAX)
C      WRITE(21) ((UTTV(NA,NRV),NA=1,4),NRV=1,NRVMAX)
...
C
      SUBROUTINE EQLOAD(MODELG,KNAMEQ1,MODEP,IERR)
C
      INCLUDE '../eq/eqcomc.inc'
...
C
      IF(MODELG.EQ.3.OR.MODELG.EQ.9) THEN
         CALL EQRTSK(MODELG,MODEP,IERR)
      ELSEIF(MODELG.EQ.5) THEN
         CALL EQDSKR(MODEP,IERR)
      ELSE
...
C     ***** LOAD TASK/EQ DATA *****
C
      SUBROUTINE EQRTSK(MODELG,MODEP,IERR)
C
      INCLUDE '../eq/eqcomc.inc'
      DIMENSION DERIV(NRVM)
C
      CALL FROPEN(21,KNAMEQ,0,MODEP,'EQ',IERR)
      IF(IERR.NE.0) RETURN
C
      READ(21) RR,BB,RIP
...
C
      ...
      READ(21) ((PSI(NSG,NTG),NSG=1,NSGMAX),NTG=1,NTGMAX)
      READ(21) ((DELPSI(NSG,NTG),NSG=1,NSGMAX),NTG=1,NTGMAX)
      READ(21) ((HJT(NSG,NTG),NSG=1,NSGMAX),NTG=1,NTGMAX)
C      READ(21) ((UQPV(NA,NRV),NA=1,4),NRV=1,NRVMAX)
C      READ(21) ((UPSITV(NA,NRV),NA=1,4),NRV=1,NRVMAX)
C      READ(21) ((UTTV(NA,NRV),NA=1,4),NRV=1,NRVMAX)
...
C
      neqdsk=21
      CALL FROPEN(neqdsk,KNAMEQ,1,MODEP,'EQ',IERR)
      IF(IERR.NE.0) RETURN
c
      REWIND(neqdsk)
