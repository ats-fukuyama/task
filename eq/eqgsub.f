...
C
      SUBROUTINE EQGR2D(GF,GR,GZ,GRS,GZS,NRM,NRMAX,NTHMAX,KA)
C
...
C
     ...
     &            GSXMIN,GSXMAX,GSYMIN,GSYMAX)
      ...
      CALL GVALUE(GXORG,2*GSTEPX,0.0,0.0,NGVLEN(GSTEPX))
      IF(MOD(MODE/8,2).EQ.1) THEN
         CALL GSCALL(0.0,0,GYORG,9,0.1,9)
         CALL GVALUL(0.0,0,GYORG,1,NGVLEN(GSTEPY))
      ELSE
         CALL GSCALE(0.0,0.0,GYORG,GSTEPY,0.1,9)
         CALL GVALUE(0.0,0.0,GYORG,2*GSTEPY,NGVLEN(GSTEPY))
      ENDIF
C
      DO 10 NG=1,NGMAX
