...
C
 9000 XX=X
  ...
  601 FORMAT(1H ,6X,'FXX,YY,XY = ',1P4E15.7)
      END
C
C     ****** SOLVE SIMULATANEOUS NON-LINEAR EQ. BY NEWTON METHOD ******
C
      SUBROUTINE NEWTN4(SUB,X,Y,XX,YY,DELT,EPS,ILMAX,LIST,IER)
C
      IMPLICIT REAL*8(A-H,O-Z)
      EXTERNAL SUB
C
...
c      DFPRE=1.D32
      ...
      ENDIF
      CALL SUB(X,   Y,   S0,G0)
      CALL SUB(X+HX,Y,   SX,GX)
      CALL SUB(X,   Y+HY,SY,GY)
      CALL SUB(X-HX,Y,   SX1,GX1)
      CALL SUB(X,   Y-HY,SY1,GY1)
      CALL SUB(X+HX,Y+HY,SXY,GXY)
      FX=(SX-S0)/HX
      FY=(SY-S0)/HY
      FXX=(SX+SX1-2*S0)/(HX*HX)
      FYY=(SY+SY1-2*S0)/(HY*HY)
      FXY=(SXY+S0-SX-SY)/(HX*HY)
      IF(LIST.GT.0) WRITE(6,600) X,Y,FX,FY,S0
      IF(LIST.GT.0) WRITE(6,601) FXX,FXY,FYY
      IF(LIST.GT.0) CALL GUFLSH
      DF=SQRT(FX*FX+FY*FY)
      DET=FXX*FYY-FXY*FXY
      H11= FYY/DET
...
C      HY=0.1D0*TT*DY
      ...
      FXN=(SX-S0)/HX
      FYN=(SY-S0)/HY
      IF(LIST.GT.0) WRITE(6,600) X,Y,FXN,FYN,S00
      IF(LIST.GT.0) CALL GUFLSH
      DFN=SQRT(FXN*FXN+FYN*FYN)
      IF(LIST.GT.0) WRITE(6,603) DF,DFN
      IF(DFN.GT.DF) THEN
         IF(DF.LE.EPS) GO TO 9000
         IF(IP2.EQ.0)THEN
            X1=X
         ...
         ELSEIF(IP1.EQ.1)THEN
            ...
            FXN1=(SX-S0)/HX
            FYN1=(SY-S0)/HY
            DFN1=SQRT(FXN1*FXN1+FYN1*FYN1)
            DF=DFN1
            IP=0
            IP1=0
            IP2=0
            IF(LIST.GT.0) WRITE(6,*) 'new prosece'
C         ELSEIF(IP1.EQ.3)THEN
C            TT=TT1
C            X=X1-1.5*TT*DX
C            Y=Y1-1.5*TT*DY
C            IP=0
C            IP1=1
C            IF(LIST.GT.0) WRITE(6,*) 'new prosece2'
         ENDIF    
         IF(ITER.GT.ILMAX) THEN
            GO TO 7000
         ENDIF
         IF(TT.LE.1.D-3) THEN
            IP1=IP1+1
            IP=1 
           IF(IP1.EQ.3)GOTO 8000  
         ENDIF 
         GOTO 2
      ENDIF 
C
...
c      STX=DX*H11+DY*H21
c      STY=DX*H12+DY*H22
      DSX=TT*DX-H11*DFX-H12*DFY
      DSY=TT*DY-H21*DFX-H22*DFY
      SS=DFX*DSX+DFY*DSY
      IF(SS.EQ.0) GOTO 6000         
      H11=H11+DSX*DSX/SS
      H12=H12+DSX*DSY/SS
      H21=H21+DSY*DSX/SS
      H22=H22+DSY*DSY/SS
...
C
 6000 IER=3
      IF(LIST.GT.0)
     &WRITE(6,*) 'XX NEWTNL: SS IS DIVIDED BY ZERO.'
C
 7000 IER=2
      IF(LIST.GT.0)
     &WRITE(6,*) 'XX NEWTNL: LOOP COUNT EXCEEDS UPPER BOUND.'
      GOTO 9000
C
 8000 IER=1
      IF(LIST.GT.0)
     &WRITE(6,*) 'XX NEWTN4: DOES NOT CONVERGE.'
      GOTO 9000
C
 9000 XX=X
      YY=Y
      RETURN
  600 FORMAT(1H ,3X,'X,Y,FX,FY,F = ',1P2E15.7,1P3E10.2)
  601 FORMAT(1H ,3X,'   XX,XY,YY = ',30X,     1P3E10.2)
  603 FORMAT(1H ,3X,'DF,DFN      = ',30X,     1P2E15.7)
      END
