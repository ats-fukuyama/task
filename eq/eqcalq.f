...
C
      NR=1
      QPS(NR)=(4.D0*QPS(2)-QPS(3))/3.D0
      DVDPSIP(NR)=(4.D0*DVDPSIP(2)-DVDPSIP(3))/3.D0
      DVDPSIT(NR)=(4.D0*DVDPSIT(2)-DVDPSIT(3))/3.D0
      DSDPSIT(NR)=(4.D0*DSDPSIT(2)-DSDPSIT(3))/3.D0
      RLEN(NR)=0.D0
...
C
Chonda      DO NR=1,NRMAX
Chonda         RHOT(NR)=SQRT(PSIT(NR)/PSITA)
Chonda      ENDDO
C
C     *** Convert F(PSIP) to F(RHOT) ***
C         where F is an arbitrary function.
C
Chonda      DERIV(1)=0.D0
Chonda      CALL SPL1D(PSIP,PSIP,DERIV,UPSIPS,NRMAX,1,IERR)
Chonda      IF(IERR.NE.0) WRITE(6,*) 'XX SPL1D for PSIP: IERR=',IERR
      DERIV(1)=0.D0
      CALL SPL1D(PSIP,PPS,DERIV,UPPS,NRMAX,1,IERR)
      IF(IERR.NE.0) WRITE(6,*) 'XX SPL1D for PPS: IERR=',IERR
      DERIV(1)=0.D0
      CALL SPL1D(PSIP,TTS,DERIV,UTTS,NRMAX,1,IERR)
      IF(IERR.NE.0) WRITE(6,*) 'XX SPL1D for TTS: IERR=',IERR
      DERIV(1)=0.D0
      CALL SPL1D(PSIP,QPS,DERIV,UQPS,NRMAX,1,IERR)
...
C
      ...
      DERIV(1)=0.D0
      CALL SPL1D(PSIP,SPS,DERIV,USPS,NRMAX,1,IERR)
...
C
      DERIV(1)=0.D0
      CALL SPL1D(PSIP,AVERR ,DERIV,UAVERR ,NRMAX,1,IERR)
      IF(IERR.NE.0) WRITE(6,*) 'XX SPL1D for AVERRR: IERR=',IERR
      DERIV(1)=0.D0
      CALL SPL1D(PSIP,AVERR2,DERIV,UAVERR2,NRMAX,1,IERR)
      IF(IERR.NE.0) WRITE(6,*) 'XX SPL1D for AVERRR2: IERR=',IERR
      DERIV(1)=0.D0
      CALL SPL1D(PSIP,AVEIR2,DERIV,UAVEIR2,NRMAX,1,IERR)
      IF(IERR.NE.0) WRITE(6,*) 'XX SPL1D for AVEIR2: IERR=',IERR
      DERIV(1)=0.D0
      CALL SPL1D(PSIP,AVEBB ,DERIV,UAVEBB ,NRMAX,1,IERR)
      IF(IERR.NE.0) WRITE(6,*) 'XX SPL1D for AVEBB: IERR=',IERR
      DERIV(1)=0.D0
      CALL SPL1D(PSIP,AVEBB2,DERIV,UAVEBB2,NRMAX,1,IERR)
      IF(IERR.NE.0) WRITE(6,*) 'XX SPL1D for AVEBB2: IERR=',IERR
      DERIV(1)=0.D0
      CALL SPL1D(PSIP,AVEIB2,DERIV,UAVEIB2,NRMAX,1,IERR)
      IF(IERR.NE.0) WRITE(6,*) 'XX SPL1D for AVEIB2: IERR=',IERR
      DERIV(1)=0.D0
      CALL SPL1D(PSIP,AVEGV ,DERIV,UAVEGV ,NRMAX,1,IERR)
      IF(IERR.NE.0) WRITE(6,*) 'XX SPL1D for AVEGV: IERR=',IERR
      DERIV(1)=0.D0
      CALL SPL1D(PSIP,AVEGV2,DERIV,UAVEGV2,NRMAX,1,IERR)
      IF(IERR.NE.0) WRITE(6,*) 'XX SPL1D for AVEGV2: IERR=',IERR
      DERIV(1)=0.D0
      CALL SPL1D(PSIP,AVEGR2,DERIV,UAVEGR2,NRMAX,1,IERR)
      IF(IERR.NE.0) WRITE(6,*) 'XX SPL1D for AVEGR2: IERR=',IERR
      DERIV(1)=0.D0
      CALL SPL1D(PSIP,AVEGP2,DERIV,UAVEGP2,NRMAX,1,IERR)
      IF(IERR.NE.0) WRITE(6,*) 'XX SPL1D for AVEGP2: IERR=',IERR
      DERIV(1)=0.D0
      CALL SPL1D(PSIP,AVEJPR,DERIV,UAVEJPR,NRMAX,1,IERR)
      IF(IERR.NE.0) WRITE(6,*) 'XX SPL1D for AVEJPR: IERR=',IERR
      DERIV(1)=0.D0
      CALL SPL1D(PSIP,AVEJTR,DERIV,UAVEJTR,NRMAX,1,IERR)
      IF(IERR.NE.0) WRITE(6,*) 'XX SPL1D for AVEJTR: IERR=',IERR
C
      DERIV(1)=0.D0
      CALL SPL1D(PSIP,RRPSI  ,DERIV,URRPSI  ,NRMAX,1,IERR)
      IF(IERR.NE.0) WRITE(6,*) 'XX SPL1D for RRPSI: IERR=',IERR
      DERIV(1)=0.D0
      CALL SPL1D(PSIP,RSPSI  ,DERIV,URSPSI  ,NRMAX,1,IERR)
      IF(IERR.NE.0) WRITE(6,*) 'XX SPL1D for RSPSI: IERR=',IERR
      DERIV(1)=0.D0
      CALL SPL1D(PSIP,ELIPPSI,DERIV,UELIPPSI,NRMAX,1,IERR)
      IF(IERR.NE.0) WRITE(6,*) 'XX SPL1D for ELIPPSI: IERR=',IERR
      DERIV(1)=0.D0
      CALL SPL1D(PSIP,TRIGPSI,DERIV,UTRIGPSI,NRMAX,1,IERR)
      IF(IERR.NE.0) WRITE(6,*) 'XX SPL1D for TRIGPSI: IERR=',IERR
      DERIV(1)=0.D0
      CALL SPL1D(PSIP,DVDPSIP,DERIV,UDVDPSIP,NRMAX,1,IERR)
      IF(IERR.NE.0) WRITE(6,*) 'XX SPL1D for DVDPSIP: IERR=',IERR
      DERIV(1)=0.D0
      CALL SPL1D(PSIP,DVDPSIT,DERIV,UDVDPSIT,NRMAX,1,IERR)
...
C     
      ...
      ENDDO
C
      CALL SPL2D(THIT,PSIP,RPS,D10,D01,D11,URPS,
     &           NTHMP,NTHMAX+1,NRMAX,4,0,IERR)
      IF(IERR.NE.0) WRITE(6,*) 'XX SPL2D for RPS: IERR=',IERR
C
      CALL SPL2D(THIT,PSIP,ZPS,D10,D01,D11,UZPS,
     &           NTHMP,NTHMAX+1,NRMAX,4,0,IERR)
...
C
      DO NR = 2, NRMAX
         AVEGR2R(NR) = AVEGR2(NR) / DVDRHO(NR)**2 ! <|grad rho|^2/R^2>
         AVEGRR(NR)  = AVEGV(NR)  / DVDRHO(NR)    ! <|grad rho|>
         AVEGRR2(NR) = AVEGV2(NR) / DVDRHO(NR)**2 ! <|grad rho|^2>
      ENDDO
      AVEGR2R(1) = FCTR(RHOT(2),RHOT(3),AVEGR2R(2),AVEGR2R(3))
      AVEGRR (1) = FCTR(RHOT(2),RHOT(3),AVEGRR (2),AVEGRR (3))
...
C
      ...
      DERIV(1)=0.D0
      CALL SPL1D(PSIP,AVEGR2R,DERIV,UAVEGR2R,NRMAX,1,IERR)
      IF(IERR.NE.0) WRITE(6,*) 'XX SPL1D for AVEGR2R: IERR=',IERR
      DERIV(1)=0.D0
      CALL SPL1D(PSIP,AVEGRR ,DERIV,UAVEGRR ,NRMAX,1,IERR)
      IF(IERR.NE.0) WRITE(6,*) 'XX SPL1D for AVEGRR: IERR=',IERR
      DERIV(1)=0.D0
      CALL SPL1D(PSIP,AVEGRR2,DERIV,UAVEGRR2,NRMAX,1,IERR)
