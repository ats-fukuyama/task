...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
      ...
      call VecGetArrayReadF90(x,x_value,ierr)
      IF(ierr.NE.0) WRITE(6,*) &
           'XX mtx_get_vector_j: VecGetArrayReadF90: ierr=',ierr
      v=x_value(j-istart)
      call VecRestoreArrayReadF90(x,x_value,ierr)
      IF(ierr.NE.0) WRITE(6,*) &
           'XX mtx_get_vector_j: VecRestoreArrayReadF90: ierr=',ierr
...
!      PetscScalar,pointer:: x_value(:)
      PetscScalar,DIMENSION(irange):: x_value
      PetscInt,DIMENSION(irange):: iv
      INTEGER:: j,ierr

      DO j=1,irange
      	 iv(j)=j-1+istart
      END DO
      call VecGetValues(x,irange,iv,x_value,ierr)
      IF(ierr.NE.0) WRITE(6,*) &
           'XX mtx_get_vector: VecGetValues: ierr=',ierr
      
!      call VecGetArrayReadF90(x,x_value,ierr)
!      IF(ierr.NE.0) WRITE(6,*) &
!           'XX mtx_get_vector: VecGetArrayReadF90: ierr=',ierr
      do j=1,irange
         v(j)=x_value(j)
      enddo
!      call VecRestoreArrayReadF90(x,x_value,ierr)
!      IF(ierr.NE.0) WRITE(6,*) &
!           'XX mtx_gather_vector: VecRestoreArrayReadF90: ierr=',ierr
      ...
      call mtx_get_vector(v)
      imax_=imax      
      call mtx_allgatherv_real8(v,irange,x_,imax_,isizex,istartx)
      RETURN
