...
C
      ...
      IF(IERR.NE.0) WRITE(6,*) 'XX wmfem_vmec_CRZ: SPL1D: PHI: IEER=',
     &     IERR
C
...
c
         ELSE
            ...
            IF(IERR.NE.0) THEN
               WRITE(6,*) 'XX wmfem_vmec_CRZ: SPL1DF: PSIP: IEER=',
     &              IERR
...
C         ENDIF
         ...
         IF(IERR.NE.0) 
     &        WRITE(6,*) 'XX wmfem_vmec_CRZ: SPL1D: YRBS: IEER=',IERR
         CALL SPL1D(XS,YZBS,FX2,U2(1,1,MN),NSRMAX,0,IERR)
         IF(IERR.NE.0) 
     &        WRITE(6,*) 'XX wmfem_vmec_CRZ: SPL1D: YZBS: IEER=',IERR
C
         DO NR=1,NRMAX+1
            IF(XRHO(NR).GT.1.D0) THEN
...
c
     ...
     &                     XS,U1(1,1,MN),NSRMAX,IERR)
               IF(IERR.NE.0) THEN
                  WRITE(6,*) 'XX wmfem_vmec_CRZ: SPL1DD: SRMNC: IEER=',
     &                 IERR
...
c
     ...
     &                     XS,U2(1,1,MN),NSRMAX,IERR)
               IF(IERR.NE.0) THEN
                  WRITE(6,*) 'XX wmfem_vmec_CRZ: SPL1DD: SSMNC: IEER=',
     &                 IERR
C                  WRITE(6,'(3I5,1P2E12.4)') MN,NR,IERR,XRHO(NR),XS(NSRMAX)
               ENDIF
            ENDIF
