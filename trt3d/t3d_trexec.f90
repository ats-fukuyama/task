...
!     ***********************************************************
     ...
     &   VLOOP, VSEC, X, XV, Y, YV, Z, ZV ,NEQMAXM, DIPDT, akdw, nt
      ...
      use T3D_FILE_IO 
!      use T3D_FILE_IO, only: TRCOEF_01      
      use t3d_er_param
      
      IMPLICIT NONE
      REAL(8),INTENT(IN) :: DT
      INTEGER(4),INTENT(OUT) :: IERR
      INTEGER(4):: I, ICHCK, IDGLOB, INFO, J, LDB, M, MWRMAX, &
...
!      print*,'2000 continue' 
!      print('(a10,4e20.10)'),'NTcheck',RN(1,1),RN(1,2),RT(1,1),RT(1,2)
!      CALL TR_EDGE_SELECTOR(0)
!=================================wakasa
! custamized by A. wakasa
! edge maker
!              call t3d_input_prof('te',0)
!              call t3d_input_prof('ti',0)
!               print'(4e20.10)',PTS(1),PTS(2),RT(NRMAX,1),RT(NRMAX,2)
!=================================walasa
...
!     /* Matrix Solver */
      IF(MDLPCK.EQ.0) THEN
         ...
         IF(IERR.EQ.30000) THEN
            WRITE(6,*)&
          & 'XX ERROR IN TRLOOP : MATRIX AA IS SINGULAR  AT T = ',T
            IERR=9000
            RETURN
         ENDIF
...
!     /* Convergence check */
 ...
 3000 continue
!        print*,'3000'
        CNT_TREXE_ITERATION=CNT_TREXE_ITERATION+1
      IF((CNT_TREXE_ITERATION).GE.LMAXTR) GOTO 4000
...
!      print *, '#1 start TRCALC for iteration'
...
!         CALL TRSNAP
      ...
      USE TRCOMM, ONLY : NGRSTP, NGTSTP, NTSTEP,RT
      use t3d_er_param
      IMPLICIT NONE
      ...
      integer:: IDGLOB,i
        ...
        case(20)
            CALL TRCALC_NOERAD(IERR)
        case(10,30:39)
            CALL TRCALC(IERR)
        case default
            print*,'[ER calculation mesthd] select ERROR [no.3]'
            stop
        end select   
...
!     ***********************************************************
     ...
     &                   VV, X, XV, Y, YV, Z, ZV, NEQMAXM, RDPS
      ...
      use task3d_tentative_flags, only:PIN_EQ      
...
!          +----------+
     ...
     &                  *DV53 *COULOG(NS,NS1,RN(NR,1),RT(NR,NS))
               PIN_EQ(ns,ns1,nr)=C1*RN(NR,NS)/DV53
...
!     ***** Band Matrix *****
      ...
      IF(MDLPCK.NE.0) CALL TR_BAND_LAPACK(A,B,C,AX,NRMAX,NEQRMAX,NVM,NRM,LDAB,MLM)
...
!     ***********************************************************
      ...
      USE TRCOMM, ONLY : AX, MDLPCK, NEQM, NEQMAX, NRM, NRMAX, NVM, XV
...
!     ***********************************************************
      ...
      SUBROUTINE TR_BAND_LAPACK(A,B,C,AX,NRMAX,NEQRMAX,NVM,NRM,LDAB,MLM)

      IMPLICIT NONE
      REAL(8),DIMENSION(NVM,NVM,NRMAX),INTENT(IN) :: A, B, C
      REAL(8),DIMENSION(LDAB,MLM)  ,INTENT(OUT) :: AX
      INTEGER(4),INTENT(IN) :: NRMAX, NEQRMAX, NVM, NRM, LDAB, MLM
...
!     ***********************************************************
      ...
      DO NR=1,NRMAX
         DO NEQ=1,NEQMAX
            ...
            ENDIF
         ENDDO
!         print*,'RE(NR,1)',RW(NR,1),'RW(NR,2)',RW(NR,2)
         YV(  1,NR) = RW(NR,1)
         YV(  2,NR) = RW(NR,2)
      ENDDO
      IF(MDTC.NE.0) THEN
...
!     ***********************************************************
     ...
     &                   DR, DT, DVRHO, ETA, FA, FB, FC, MDDIAG, MDLEQ0, MDLEQE, MDLFLX, NEQM, NEQMAX, NRM, NRMAX, NSS, NSV,&
...
!     ***********************************************************
      ...
      USE TRCOMM, ONLY : DVRHO, MDLEQ0, NEA, NRM, NSMAX, NVM, PN, TSIE
      USE TRCOM1, ONLY : B
      IMPLICIT NONE
!      INCLUDE 'trcomm.inc'
...
!     ***********************************************************
      ...
      USE TRCOMM, ONLY : DVRHO, MDLEQ0, NEA, NRM, NVM, TSCX
      USE TRCOM1, ONLY : B
      IMPLICIT NONE
      INTEGER(4),INTENT(IN) :: NR
      INTEGER(4):: NEQ, NV, NW
