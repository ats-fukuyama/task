...
!     ***********************************************************
      ...
      USE TRCOMM, ONLY : AJBS, AJRF, AR1RHOG, ARRHOG, BP, DT, DVRHOG, MDLEQ0, &
           ...
           & NROMAX, NSM, NSMAX, NT,PBCL, PBIN, PCX, PELTIM, PEX, PFCL, PFIN, &
           & PI, PIE, PIN, PN, PNB, PNF, POH, PRF, PRL, Q0, QP, RDP, RG, &
           & RHOA, RR, RA, SCX, SEX, SIE, SNB, SNF, SPE, SSIN, T, TTRHOG, &
           & KUFDEV, modelg,NGTSTP,ZEFF,ER, ntmax, &
           & AK, AKDW, AD,AV,ADDW
      use ncdn_mod
      use er_mod
      use t3d_er, only: pflx
      use t3d_er_param
      use T3D_FILE_IO

!       FIT3DとTASK3Dを連携する拡張に使用したモジュール
!       及び，出力ファイルに加熱項併記のために，エネルギー等分配項の読み込み用
      ...
      real :: g1,g2
      integer ::i
      integer,save::PINflg1=-1,PINflg2=-1,ERcalflg=0

      REAL(8), dimension(NRMAX,NSMAX) :: PEX0      
!      REAL(4)::PEX1_FACTOR,PEX2_FACTOR

      real(8)::flxe,flxi
      real(8), dimension(nrmax)::pfe,pfi
!      integer(4),save ::erlogflg_at_trcalc=0
      
!       FIT3DとTASK3Dを連携する拡張に使用したモジュール
       integer(4),save ::erlogflg_at_trcalc=0
!      integer(4),save ::flg_t3d_er_calc_timing=0
      
      real(8),dimension(nrmax,nsmax):: Integral_P_ALL_dV,PIN_ALL_dV

      if (ER_CALC_METHOD>=30)then
      if (erlogflg_at_trcalc==0)then
        open(848,file='er_dynamics_log_iteration.txt',status='replace')
        close(848)
        erlogflg_at_trcalc=1
      endif
      endif
...
!         else if (modelg==21 .or. modelg == 23) then
...
!print*,'ER_CALC_METHOD',ER_CALC_METHOD
!print*,'CNT_TREXE_ITERATION',CNT_TREXE_ITERATION
!print*,'ER_CALC_CONDITION',ER_CALC_CONDITION
!print*,'ER_CALC_INITIAL_VALUE',ER_CALC_INITIAL_VALUE
!print*,'ER_CALC_BOUNDARY_CONDITION',ER_CALC_BOUNDARY_CONDITION
!if(ER_CALC_CONDITION /= 0 .and. ER_CALC_CONDITION /= -1) then
!print*,'int(ntmax/ER_CALC_CONDITION)',int(ntmax/ER_CALC_CONDITION)
!print*,'mod(nt,int(ntmax/ER_CALC_CONDITION))',mod(nt,int(ntmax/ER_CALC_CONDITION))
!endif
...
!    print*,'nt',nt,'ntmax',ntmax
    ...
    case(10)
        if (nt == 0 .and. (CNT_TREXE_ITERATION ==-1)) then
        CALL TRERAD
        endif
    case (20,30:39)
        select case(ER_CALC_CONDITION)
        case(-2)
            if (CNT_TREXE_ITERATION ==-1 .or. CNT_TREXE_ITERATION == 0) then
                if (flg_t3d_er_calc_timing==0)then
                    call t3d_er_calc_initialize
                    flg_t3d_er_calc_timing=1
                endif
                if(nt == ER_CALC_TIMING(flg_t3d_er_calc_timing)) then
                    CALL TRERAD
                    if(flg_CNT_TREXE_ITERATION==1)then
                        flg_t3d_er_calc_timing=flg_t3d_er_calc_timing+1
                        flg_CNT_TREXE_ITERATION=0
                    endif
                endif
            endif
        ...
        case(0)
            if (nt == 0 .and. CNT_TREXE_ITERATION ==-1) then
!            if (nt == 0 .and. (CNT_TREXE_ITERATION ==1 .or. CNT_TREXE_ITERATION == 0)) then
            CALL TRERAD
            endif
        case (1:)
            print*,'NOW ',nt,(mod(nt,int(ntmax/ER_CALC_CONDITION)) == 0 .and. (CNT_TREXE_ITERATION ==-1 .or. CNT_TREXE_ITERATION == 0)) 
            if (mod(nt,int(ntmax/ER_CALC_CONDITION)) == 0 .and. (CNT_TREXE_ITERATION ==-1 .or. CNT_TREXE_ITERATION == 0)) then
            CALL TRERAD

            open(848,file='er_evo_log_at_trcalc.txt',position='APPEND') 
            do nr=1,nrmax
                write(848,'(e20.10,i6,2e20.10)') t,nr,rg(nr),er(nr)
            enddo
            write(848,*)
            close(848)
        ...
        case default
            print*,'ER Calculation condition error. You must select ER_CALC_CONDITION = -1 .or. 0 .or. 1:..'
            print*,'stopped @ ',__FILE__,__LINE__
        end select
    case default
        print*,'ER Calculation method error. You must select ER_CALC_METHOD = 10 .or. 20 .or. 30:39.'
        print*,'stopped @ ',__FILE__,__LINE__
        stop
    end select
...
!         CALL TR_NCLASS(IERR)
     ...
     do nr=1,nrmax
       if(PEX(NR,1)<0.d0)PEX(NR,1)=0.d0
       if(PEx(NR,2)<0.d0)PEX(NR,2)=0.d0
     enddo     
...
!        if (TEMPORARY_SETTING==1) then
!        if(nt<250)then     
!         PIN(NR,1)=PBCL(NR,1)+PFCL(NR,1) &
!              &   +POH(NR)-PRL(NR)-PIE(NR)+PEX(NR,1)
!         PIN(NR,2)=(PBCL(NR,2)+PFCL(NR,2)+PRF(NR,2) &
!              &   -PN(2)*PCX(NR)/(PN(2)+PN(3))+PEX(NR,2))
!        else if(nt>=250 .and. nt< 500)then     
!         PIN(NR,1)=PBCL(NR,1)+PFCL(NR,1)+PRF(NR,1) &
!              &   +POH(NR)-PRL(NR)-PIE(NR)+PEX(NR,1)
!         PIN(NR,2)=(PBCL(NR,2)+PFCL(NR,2)+PRF(NR,2) &
!              &   -PN(2)*PCX(NR)/(PN(2)+PN(3))+PEX(NR,2))
!        else if(nt>=500 .and. nt< 750)then     
!         PIN(NR,1)=PBCL(NR,1)+PFCL(NR,1)+PRF(NR,1) &
!              &   +POH(NR)-PRL(NR)-PIE(NR)+PEX(NR,1)
!         PIN(NR,2)=(PBCL(NR,2)+PFCL(NR,2)+PRF(NR,2) &
!              &   -PN(2)*PCX(NR)/(PN(2)+PN(3))+PEX(NR,2))
...
!         PIN(nr,2)=PIN(nr,2)+1d4
     enddo
     
!     do nr=1,nrmax
!       PEX0(NR,1)=PEX(NR,1)+100*PEX(NR,1)*Rg(nr)**4
!       PEx0(NR,2)=PEX(NR,2)+100*PEX(NR,2)*rg(nr)**4
!       print'(i4,5e20.10)',nr,rg(nr),PEX(nr,1),PEX(nr,2),PIN(nr,1),PIN(nr,2)
!     enddo

!     do nr=1,nrmax
!         PIN(nr,1)=PIN(nr,1)-er(nr)*(pflx(nr,0)-addw(nr,1)*dan(nr,0))*1d20*1.6e-19
!         PIN(nr,2)=PIN(nr,2)+er(nr)*(pflx(nr,1)-addw(nr,2)*dan(nr,1))*1d20*1.6e-19
!    enddo
...
!    if(PINflg1.eq.-1)then
!        open(851,file='PIN_LISTa_1.txt',status='replace')
!        open(852,file='PIN_LISTa_2.txt',status='replace')
!        close(851)
!        close(852)
!        PINflg1=1
!    endif
    
!       UFILE使用時には（現時点では）PINlogは1タイミングもに記述（TR計算中にPEX変化せず）
!       FIT3Dとの連携時には，FIT3Dの新規計算のタイミング毎にPEX記述
!    if(((FIT3D_tentative_flags==0).and.(nt==0)).or.((FIT3D_tentative_flags==1).and.(nt == FIT3D_CALC_TIMING(PINflg1))))then
...
!        open(851,file='PIN_LISTa_1.txt',position='APPEND')
!        open(852,file='PIN_LISTa_2.txt',position='APPEND')

!        write(851,'(a5,i6)')'#nt=',nt
!        write(851,'(a6,16a20)')'#1:NR','2:RG(NR)',&
!          & '3:PIN(NR,1)','4:PBCL(NR,1)','5:PFCL(NR,1)','6:PRF(NR,1)', &
!          & '7:POH(NR)','8:PRL(NR)','9:PIE(NR)','10:PEX(NR,1)',&
!          & '11:RG*RA', '12:dV','13:PdV','14:PIN_EQ(1,1,nr)','15:PIN_EQ(1,2,nr)','16:PIN_EQ(2,1,nr)','17:PIN_EQ(2,2,nr)'
!          do nr=1,nrmax-1
!              write(851,'(i6,16(e20.10))')NR,RG(NR),&
!              & PIN(NR,1),PBCL(NR,1),PFCL(NR,1),PRF(NR,1), &
!              & POH(NR),PRL(NR),PIE(NR),PEX(NR,1),RA*RG(NR),&
!              & 2.0d0*PI*PI*RR*(RA**2*((RG(NR+1)**2-RG(NR)**2))),&
!              & 2.0d0*PI*PI*RR*PEX(NR,1)*(RA**2*((RG(NR+1)**2-RG(NR)**2))),&
!              & PIN_EQ(1,1,nr),PIN_EQ(1,2,nr),PIN_EQ(2,1,nr),PIN_EQ(2,2,nr)
!              
!          enddo
!          write(851,'(i6,16(e20.10))')NR,RG(NR),&
!          & PIN(NR,1),PBCL(NR,1),PFCL(NR,1),PRF(NR,1), &
!          & POH(NR),PRL(NR),PIE(NR),PEX(NR,1),RA*RG(NR),&
!          & 2.0d0*PI*PI*RR*(RA**2*((RG(NR)**2-RG(NR-1)**2))),&
!          & 2.0d0*PI*PI*RR*PEX(NR,1)*(RA**2*((RG(NR)**2-RG(NR-1)**2))),&
!          & PIN_EQ(1,1,nr),PIN_EQ(1,2,nr),PIN_EQ(2,1,nr),PIN_EQ(2,2,nr)
!          
!          write(851,*)
!
!          write(852,'(a5,i6)')'#nt=',nt        
!          write(852,'(a6,20a20)')'#1:NR','2:RG(NR)',&
!             & '3:PIN(NR,2)','4:PBCL(NR,2)','5:PFCL(NR,2)','6:PRF(NR,2)', &
!             &  '7:-PN*PCX/(PN+PN)','8:PEX(NR,2)',&
!          & '9:SSIN(NR,2)','10:PN(2)','11:SIE/(PN(2)+PN(3))','12:SNF(NR)','13:SNB(NR)','14:SEX(NR,2)',&
!          & '15:RG*RA', '16:dV','17:PdV','18:PIN_EQ(1,1,nr)','19:PIN_EQ(1,2,nr)','20:PIN_EQ(2,1,nr)','21:PIN_EQ(2,2,nr)'
!          do nr=1,nrmax-1
!              write(852,'(i6,20(e20.10))')NR,RG(NR),&
!              & PIN(NR,2),PBCL(NR,2),PFCL(NR,2),PRF(NR,2), &
!              &   -PN(2)*PCX(NR)/(PN(2)+PN(3)),PEX(NR,2),&
!              & SSIN(NR,2),PN(2),SIE(NR)/(PN(2)+PN(3)),SNF(NR),SNB(NR),SEX(NR,2),&
!          & 2.0d0*PI*PI*RR*(RA**2*((RG(NR+1)**2-RG(NR)**2))),&
...
!!-----------------------------
!! 2009/07/09
      use t3d_er, only : t3d_er_ambplr
      use t3d_interface_tr2er, only : t3d_tr2er
!!------------------------------
      IMPLICIT NONE
      INTEGER(4):: NR
...
!     ***********************************************************
     ...
     &                   MDLJQ, MDLUF, NRMAX, POH, RDP, RMU0, RR, TTRHO, TTRHOG, rm
      IMPLICIT NONE
      INTEGER(4):: NR
...
!     ***********************************************************
 ...
 2000 IONE=NR
      ...
      IF(LQ.EQ.1) THEN
         ...
         RDP(1:IZEROX)=TTRHOG(1:IZEROX)*ARRHOG(1:IZEROX)*DVRHOG(1:IZEROX) &
...
!     ***********************************************************
      ...
      real :: g1,g2
      integer ::i
      integer,save::PINflg1=0,PINflg2=0,ERcalflg=0
      REAL(8), dimension(NRMAX,NSMAX) :: PEX0      
      REAL(4)::PEX1_FACTOR,PEX2_FACTOR
...
!      if(ercalflg.eq.0)then
!      if(mod(ercalflg,10).eq.0)then
!       if(nt==0.or.nt==1.or.nt==3.or.nt==10.or.nt==30.or.&
!      &   nt==50.or.nt==100.or.nt==300.or.nt==500.or.nt==1000)then

!      ========  This sub routine dont call TRERAD
!      ==     CALL TRERAD
