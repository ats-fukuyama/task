...
!     ***********************************************************
      ...
      SUBROUTINE TRCOEF
      use TRCOMM,ONLY: AD,AK,AV,AVK, ADNC,ADDW,AKNC,AKDW,AVNC,AVDW,AVKNC,AVKDW,nrmax,rg,T,nt,ntmax,rt,rn,er,PEX,PIN,CK0,CK1,MDLKAI
      use T3D_FILE_IO
      ...
      do nr=1,nrmax
      do i=1,2
        AD(nr,i)=  ADNC(nr,i)+ADDW(nr,i)
        AK(nr,i)=  AKNC(nr,i)+AKDW(nr,i)
        AV(nr,i)=  AVNC(nr,i)+AVDW(nr,i)
        AVK(nr,i)= AVKNC(nr,i)+AVKDW(nr,i)
      enddo
      enddo
      
      if(T3D_POWERBALANCE_CALC_FLG == 0 .and. flg_OUTPUT_FILE_TRCOEFLOG_LASTEST==1)call OUTPUT_FILE_TRCOEFLOG_LASTEST
      if(T3D_POWERBALANCE_CALC_FLG == 0 .and. flg_OUTPUT_FILE_TRCOEFLOG_EVO==1)call OUTPUT_FILE_TRCOEFLOG_EVO
...
!     ***********************************************************
     ...
     &                   TAUK, VC, VEXB, VGR1, VGR2, VGR3, VGR4, WEXB, ZEFF,NT,NGTSTP, AKNC
      use T3D_FILE_IO     
     
     
      IMPLICIT NONE
      INTEGER(4):: NS, NR08, NR, iii
     ...
     &             BPA, PROFDL
      ...
      REAL(8)   :: DERIV3P
      REAL(8) :: cofa, cofb, cofc, LINDQ
      integer(4), save:: cnt=0
      REAL(8)::CKES0,CKES1
      REAL(8):: WES0,WES1
      REAL(8):: GW,GW1,DD0,DD1
      REAL(8),save::TD_EDGE, DTD_EDGE,NE_EDGE,DNE_EDGE
      real(8) :: ave_TT
      integer(4) :: edge_NR_gBgaradT
      real(8) :: max_AKDWIL, max_AKDWEL 
      real(8) :: gb_mu_ele,gb_mu_ion     
!      real(8):: num_beki
      
!================================================wakasa
      DRL=1.D0/DR
      IF(NT.eq.0)then   
            DTD_EDGE = (RT(NRMAX-4,2)-RT(NRMAX-3 ,2))*DRL
            TD_EDGE = (RT(NRMAX-4,2)+RT(NRMAX-3,2))/2.d0
      endif
!================================================wakasa
      AMD=PA(2)*AMM
      AMT=PA(3)*AMM
...
!!wakasa ***************************************************************** 
!!wakasa 2009/10/27
!!wakasa Because iota by VMEC has a rapid change at the edge of plasma 
!!wakasa DQ and QL => smoothing 
!         if(NR.gt.NRMAX-5) then
!            DQ = DERIV3(NRMAX-5,RHOG,QP,NRMAX,1)
!            QL=QP(NRMAX-5)+DQ*(RHOG(NR)-RHOG(NRMAX-5))
!        endif
!        print *, 'for PLOT Q and DQ'
!        write(6,'(i5, 4e15.5)'), NR, RHOG(NR), QP(NR), DQ, QL
!        print *,'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'
!!wakasa ***************************************************************** 
!         do iii=1,NRMAX
!         DQ = DERIV3(iii,RHOG,QP,NRMAX,1)
!@wakasa         DQ calculatin method change
!         IF(NR == 1) THEN
!           DQ=(QP(NR+2)-QP(NR))/(RHOG(NR+2)-RHOG(NR))
!            LINDQ=(QP(NR+1)-QP(NR))/(RHOG(NR+1)-RHOG(NR))
!            IF(DQ  >= LINDQ ) THEN
!               cofc=1 
!            ELSE 
!               cofc=-1
!            ENDIF
!            cofa=(QP(NR+1)-QP(NR))+cofc*sqrt((QP(NR+1)-QP(NR))**2  &
!            & -2.*(RHOG(NR+1)**2-RHOG(NR)**2)*DQ*(RHOG(NR+1)-RHOG(NR))/RHOG(NR+1)) &
!            & /(2.*(RHOG(NR+1)**2-RHOG(NR)**2))
!            cofb=DQ/(2.*cofa*RHOG(NR+1))
!            DQ=2.*cofa*RHOG(NR)+cofb
!         ELSEIF(NR == NRMAX) THEN
!            DQ=(QP(NR)-QP(NR-2))/(RHOG(NR)-RHOG(NR-2))
!            LINDQ=(QP(NR)-QP(NR-1))/(RHOG(NR)-RHOG(NR-1))
!            IF(DQ  >= LINDQ ) THEN
!               cofc=1 
!            ELSE
!               cofc=-1
...
!         ELSE
!            print *, 'p3'
!            DQ=(QP(NR+1)-QP(NR-1))/(RHOG(NR+1)-RHOG(NR-1))
!         ENDIF          
!         QL = QP(iii)
!         write(6,'(i5, 3e15.5)'), iii, RHOG(iii), QP(iii), DQ
!         enddo
!         stop
!!wakasa         IF(NR == 1) THEN
!!wakasa             DQ=(QP(NR+2)-QP(NR))/(RHOG(NR+2)-RHOG(NR))
!!wakasa             LINDQ=(QP(NR+1)-QP(NR))/(RHOG(NR+1)-RHOG(NR))
!!wakasa             IF(DQ  >= LINDQ ) THEN
!!wakasa                cofc=1
!!wakasa             ELSE
!!wakasa                cofc=-1
!!wakasa             ENDIF
!!wakasa             cofa=(QP(NR+1)-QP(NR))+cofc*sqrt((QP(NR+1)-QP(NR))**2  &
!!wakasa             & -2.*(RHOG(NR+1)**2-RHOG(NR)**2)*DQ*(RHOG(NR+1)-RHOG(NR))/RHOG(NR+1)) &
!!wakasa             & /(2.*(RHOG(NR+1)**2-RHOG(NR)**2))
!!wakasa             cofb=DQ/(2.*cofa*RHOG(NR+1))
!!wakasa             DQ=2.*cofa*RHOG(NR)+cofb
!!wakasa             DQ=LINDQ
!!wakasa          ELSEIF(NR == NRMAX) THEN
!!wakasa             DQ=(QP(NR)-QP(NR-2))/(RHOG(NR)-RHOG(NR-2))
!!wakasa             LINDQ=(QP(NR)-QP(NR-1))/(RHOG(NR)-RHOG(NR-1))
!!wakasa             IF(DQ  >= LINDQ ) THEN
!!wakasa                cofc=1
!!wakasa             ELSE
!!wakasa                cofc=-1
...
!!wakasa             DQ=LINDQ
!!wakasa          ELSE
!!wakasa             DQ=(QP(NR+1)-QP(NR-1))/(RHOG(NR+1)-RHOG(NR-1))
!!wakasa          ENDIF
!!wakasa          QL = QP(NR)
!!wakasa 
!!wakasa !         write(6,'(i5, 3e15.5)'), iii, RHOG(iii), QP(iii), DQ
!!wakasa !         enddo
!!wakasa !         stop
...
!         print '(a10, 4e20.10)',  'S(NR)', S(NR),RHOG(NR), QL, DQ
!     pressure gradient for MHD instability
         
        
         ALPHA(NR)=-2.D0*RMU0*QL**2*RR/BB**2*(DPP*1.D20*RKEV)
!20091030.wakasa 
         ALPHA(NR)=-2.D0*RMU0*QL**2*RR/BB**2*(&
         & DPP**1 & 
         &  *1.D20*RKEV)
!         open(381,file='alpha.log')
!         ALPHA(NR)=1
!         if(cnt.eq.0)then
!         if(NR.eq.1)then
!         write(381,'(a5,10a20)')& 
!         & 'NR','ALPHA(NR)','RMU0','BB','RR','QL','DPP','RKEV',&
!         & '-2.D0*RMU0/BB**2*(DPP*1.D20*RKEV)','-QL**2*RR','DPP'
!         endif
!         write(381, '(i5, 10e20.10)')&
!         & NR,ALPHA(NR),RMU0,BB,RR,QL,DPP,RKEV,&
!         & -2.D0*RMU0/BB**2*(DPP*1.D20*RKEV),-QL**2*RR,DPP
!         if(NR.eq.NRMAX)then 
!         cnt=cnt+1 
!         endif
!         endif
!         print '(i5, 4e20.10)', NR,ALPHA(NR),-2.D0*RMU0/BB**2*(DPP*1.D20*RKEV),-QL**2*RR,DPP
!     magnetic curvature
...
!           open(766, file='MDLKAI31.log')
!               IF(mod(NR,NGTSTP).eq.0)then
!               IF(NR.eq.1) write(766,'(a6,3a20)')&
!                 & 'NR',rg(nr),'AKDWE', 'AKDWI'
!               write(766,'(i5, 3e20.10)')&
!                 & NR,rg(nr),AKDW(nr,1), AKDW(nr,2)
!               print'(i5, 3e20.10)',&
!                 & NR,rg(nr),AKDW(nr,1), AKDW(nr,2)
!              endif
...
!               WE1=SQRT(PA(2)/PA(1))*(QL*RR*DELTAE)/(2*SL*RA*RA)*DBDRR
            ...
            case(31)
!               integer,save::cnt=0
!               print *, '        anomalous parameter cal'
               ALPHAL=ALPHA(NR)*CALF
           
               ALPHA(NR)=ALPHA(NR)
!               FS=TRCOFS(S(NR),ALPHAL,RKCV(NR))
!               IF(NR>NRMAX-4) THEN
!               FS=TRCOFS(S(55),ALPHAL,RKCV(NR))
!               ELSE
               
!               FS=TRCOFS(S(NR),ALPHAL,RKCV(NR))
!               FS=1.d0/(0.4*sqrt(QL))
               
!               FS=3.d0*QL/(-RG(NR)/(2.d0*PI*QL**2)*DQ)* &
!              &((10.d0/2.d0/2.d0)* &
!              &(4.d0*RG(NR)/QL/2.d0/PI+RG(NR)**2*(-DQ/2.d0/PI/QL**2)))**1.5/  &
!              &QL**2/RR**(1.5) 

               FS=3.d0/((1.2*RG(NR)/(PI*RA**2))**2)* &
              &((10.d0/2.d0/2.d0)* &
              &((4.d0*RG(NR)*((0.2d0+0.6d0*(RG(NR)/RA)**2)/PI))+((RG(NR)**2)*1.2d0*RG(NR)/(PI*RA**2)))**(1.5d0))/  &
              &QL**2/RR**(1.5) 
              
!               WE1=-QL*RR/((RG(NR)*DQ/QL)*VA*BB)*DVE
!               TAUAP=(QL*RR/VA)*RA/RG(NR)
               TAUAP=QL*RR/BB/RG(NR)*sqrt(RMU0*AMI*ANE*1.d20)
!               WES0=RA*RR*QL/(RG(NR)*BB**2)*SQRT(RMU0)*DVE/((RG(NR)/QL*DQ)*RG(NR)*BB)
               WES1=TAUAP*DVE*-1.d0
               DD0=ALPHA(NR)/(QL**2*RG(NR))*(1.d0-1.d0/QL**2)*(-RG(NR)/RR)/(2.d0*(RG(NR)/RR)**2)
               DD1= RKCV(NR)*(-1.d0) &
              & *ALPHA(NR)/(QL**2*RR)&
              & /(2.d0*(RG(NR)/RR)**2)
!               DD0= 1.d0/sqrt((RR/QL)**2+RG(NR)**2) &
!              & *ALPHA(NR)/(QL**2*RR)&
!              & /(2.d0*(RG(NR)/RR)**2)
!               print*,NR,RKCV(NR),1.d0/sqrt((RR/QL)**2+RG(NR)**2),DD0,DD1
!               
!               GW=0.5d0/ALPHA(NR)**2               
               GW1=0.52d0/DD0
!             CKES0=1d0+&
!              & (0.5d0/(QL**2*&
!              & (2.D0*RMU0/BB**2*(DPP*1.D20*RKEV)) &
!              & *RR/RG(NR))**2*WES0**2)
!             CKES0=(1+GW*WES0**2) 
             CKES1=(1+GW1*WES1**2)
!               print'(4e20.10)',WES0,CKES0,GW,CKES1

!              CKES1=1.d0
              CKES0=CKES1
!               ENDIF
               IF(MDCD05.NE.0) FS=FS*(2.D0*SQRT(RKPRHO(NR))/(1.D0+RKPRHO(NR)**2))**1.5D0
               AKDWEL=CK0*FS*SQRT(ABS(ALPHA(NR)))**3*DELTA2*VA/(QL*RR)/CKES0
               AKDWIL=CK1*FS*SQRT(ABS(ALPHA(NR)))**3*DELTA2*VA/(QL*RR)/CKES1
               


               open(766, file='MDLKAI31.log')
               if(mod(nt,ngtstp).eq.0)then 
               IF(NR.eq.1) write(766,'(a6,21a20)')&
                 & 'NR','RG(NR)','AKDWEL', 'AKDWIL','FS','ALPHA(NR)',&
                 & 'SQRT(ABS(ALPHA(NR)))**3','S(NR)','QL','DELTA2','VA',&
                 & 'VA*DELTA2','DPP',&
                 & 'GW','GW1','WES0','WES1','CKES0','CKES1','ALPHAbyITO','D0','DD1','TAUAP'
!                 & 'RPP','RPM','DRL',&
!                 & 'RN(NR+1,1)','RN(NR,1)','RNTP','RNTM','RT(NR+1,1)',&
!                 & 'RT(NR,1)','DNE','DTE','DTD','DNE*(DTE+DTD)'
               write(766,'(i5, 21e20.10)')&
                 & NR,rg(nr),AKDWEL, AKDWIL,FS,ALPHA(NR),&
                 & SQRT(ABS(ALPHA(NR)))**3,S(NR),QL,DELTA2,VA,&
                 & VA*DELTA2,DPP,&
                 & GW,GW1,WES0,WES1,CKES0,CKES1,-QL**2*RR*(2.D0*RMU0/BB**2*(DPP*1.D20*RKEV)),DD0,DD1,TAUAP
!                & ,RPP,RPM,DRL,&
!                 & RN(NR+1,1),RN(NR,1),RNTP,RNTM,RT(NR+1,1),&
!                 & RT(NR,1),DNE,DTE,DTD,DNE*(DTE+DTD)
!               IF(NR.eq.1) print ('(a5,16a20)'),&
!                 & "NR","AKDWEL","AKDWIL","FS","ALPHA(NR)",&
!                 & "ALPHA^(1.5)","S(NR)","QL","DELTA2","VA",&
!                 & "VA*DELTA2","DPP","RPP","RPM","DRL",'CKES0','WES0'
!               print ('(i5, 24e20.10)'),&
!                 & NR,AKDWEL, AKDWIL,FS,ALPHA(NR),&
!                 & SQRT(ABS(ALPHA(NR)))**3,S(NR),QL,DELTA2,VA,&
!                 & VA*DELTA2,DPP,RPP,RPM,DRL,&
!                 & RN(NR+1,1),RN(NR,1),RNTP,RNTM,RT(NR+1,1),&
!                 & RT(NR,1),DNE,DTE,DTD,DNE*(DTE+DTD)
              endif  
            case(32)
...
!            VGR4(NR,3)=1.D0/(1.D0+RG1*WE1*WE1)
         ...
         ELSEIF((MDLKAI.GE.60).and.(MDLKAI.LT.200)) THEN
...
!               RHOS  = SQRT(2.D0*AMM/AEE)*SQRT(PA(2)*TE*1.D3)/(PZ(2)*RA*BB)
            ...
            ENDIF
         ELSEIF((MDLKAI.GE.200).and.(MDLKAI.LT.210)) THEN
!   **************************************************************************************************
!   ***  MDLKAI.EQ. 200  :GyroBohm                                                                 ***
!   ***  MDLKAI.EQ. 201  :GyroBohm(consider T=> use each Te and Ti,  * (a*dT/T)^(1.5)              ***
!   ***  MDLKAI.EQ. 206  :GyroBohm(consider T=>0.5*(Te+Ti))* (a*dT/T)^(1.5)                        ***
!   ***  MDLKAI.EQ. 202  :GyroBohm * (A + (a*dT/T)^(1.5))                                          ***
!   ***                                                                                            ***
!   ***  MDLKAI.EQ. 206  :GyroBohm(consider T=>0.5*(Te+Ti))* (a*dT/T)^(gn_mue .or. gb_mu_i)        ***
!   ***  MDLKAI.EQ. 207  :TEST case                                                                ***
!   ***                  :GyroBohm(consider T=>0.5*(Te+Ti))* (a*dT/T)^(gn_mue .or. gb_mu_i)        ***
!   ***                  :MAX AKDW is set                                                          ***
!   ***  MDLKAI.EQ. 208  :GyroBohm(consider T=> use each Te and Ti)*(a*dT/T)^(gn_mue .or. gb_mu_i) ***
!   **************************************************************************************************
         select case(MDLKAI)
!         case(200)
!!              Gyro=Bohm( Te and Ti are set individualy)
!               BPA=0.0d0
!               AKDWIL=(RT(NR,2)*RKEV)/(16.D0*AEE*sqrt(BB**2+BPA**2)) &
!           &*(sqrt(RT(NR,2)*RKEV*AMM*2.0d0)/(AEE*sqrt(BB**2+BPA**2)))/RA
!               AKDWEL=(RT(NR,1)*RKEV)/(16.D0*AEE*sqrt(BB**2+BPA**2)) &
!           &*(sqrt(RT(NR,2)*RKEV*AMM*2.0d0)/(AEE*sqrt(BB**2+BPA**2)))/RA
         case(200)
!              Gyro=Bohm( T are averaged Te and Ti: T=0.5*(TE+TI))
               BPA=0.0d0
            ave_TT=0.5d0*(TE+TD)
                AKDWIL=(ave_TT*RKEV)/(16.D0*AEE*sqrt(BB**2+BPA**2)) &
            &*(sqrt(RT(NR,2)*RKEV*AMM*2.0d0)/(AEE*sqrt(BB**2+BPA**2)))/RA
                AKDWEL=(ave_TT*RKEV)/(16.D0*AEE*sqrt(BB**2+BPA**2)) &
            &*(sqrt(RT(NR,2)*RKEV*AMM*2.0d0)/(AEE*sqrt(BB**2+BPA**2)))/RA
         case(201)
!            num_beki=1.0d0
            if(NR.lt.(NRMAX-6))then
               AKDWIL=(RT(NR,2)*RKEV)/(16.D0*AEE*sqrt(BB**2+BPA**2)) &
          &*(sqrt(RT(NR,2)*RKEV*AMM*2.0d0)/(AEE*sqrt(BB**2+BPA**2)))/RA &
          &*(-RA*DTD/TD)**(gb_mu)
               AKDWEL=(RT(NR,1)*RKEV)/(16.D0*AEE*sqrt(BB**2+BPA**2)) &
          ...
          &*(-RA*DTD/TD)**(gb_mu)          
            ...
            DTD_EDGE = (RT(NRMAX-6,2)-RT(NRMAX-7 ,2))*DRL
            TD_EDGE = (RT(NRMAX-6,2)+RT(NRMAX-7,2))/2.d0
!            print'(2i5,4e20.10)',NT,NR,DTD_EDGE,TD_EDGE,RT(NRMAX,2),RT(NRMAX-1,2)
               AKDWIL=(RT(NR,2)*RKEV)/(16.D0*AEE*sqrt(BB**2+BPA**2)) &
          &*(sqrt(RT(NRMAX-7,2)*RKEV*AMM*2.0d0)/(AEE*sqrt(BB**2+BPA**2)))/RA &
          &*(-RA*DTD_EDGE/TD_EDGE)**(gb_mu)
            DTD_EDGE = (RT(NRMAX-6,1)-RT(NRMAX-7 ,1))*DRL
            TD_EDGE = (RT(NRMAX-6,1)+RT(NRMAX-7,1))/2.d0
!            print'(2i5,4e20.10)',NT,NR,DTD_EDGE,TD_EDGE,RT(NRMAX,2),RT(NRMAX-1,2)
               AKDWEL=(RT(NR,1)*RKEV)/(16.D0*AEE*sqrt(BB**2+BPA**2)) &
          &*(sqrt(RT(NRMAX-7,2)*RKEV*AMM*2.0d0)/(AEE*sqrt(BB**2+BPA**2)))/RA &
          &*(-RA*DTD_EDGE/TD_EDGE)**(gb_mu)
...
!            AKDWEL=AKDWL
         case(202)
            if(NR.lt.(NRMAX-2))then
               AKDWL=(RT(NR,2)*RKEV)/(16.D0*AEE*sqrt(BB**2+BPA**2)) &
          &*(sqrt(RT(NR,2)*RKEV*AMM*2.0d0)/(AEE*sqrt(BB**2+BPA**2)))/RA &
          &*(5.d0 + (-RA*DTD/TD)**(1.5))
            else
            DTD_EDGE = (RT(NRMAX-1,2)-RT(NRMAX-2 ,2))*DRL
            TD_EDGE = (RT(NRMAX-1,2)+RT(NRMAX-2,2))/2.d0
            print'(2i5,4e20.10)',NT,NR,DTD_EDGE,TD_EDGE,RT(NRMAX,2),RT(NRMAX-1,2)
               AKDWL=(RT(NR,2)*RKEV)/(16.D0*AEE*sqrt(BB**2+BPA**2)) &
          &*(sqrt(RT(NRMAX-2,2)*RKEV*AMM*2.0d0)/(AEE*sqrt(BB**2+BPA**2)))/RA &
          &*(5.d0 + (- RA*DTD_EDGE/TD_EDGE)**(1.5))
            endif
         case(204)
               AKDWL=(RT(NR,2)*RKEV)/(16.D0*AEE*sqrt(BB**2+BPA**2)) &
          &*(sqrt(RT(NR,2)*RKEV*AMM*2.0d0)/(AEE*sqrt(BB**2+BPA**2)))/RA &
...
!            num_beki=1.0d0
            ...
            gb_mu_ele=0.0
            gb_mu_ion=1.0
          if(NR <= (NRMAX-edge_NR_gBgaradT))then
!               print'(i4,8e20.10)',nr,(ave_TT*RKEV)/(16.D0*AEE*sqrt(BB**2+BPA**2)) &
!          &*(sqrt(RT(NR,2)*RKEV*AMM*2.0d0)/(AEE*sqrt(BB**2+BPA**2)))/RA,&
!          & (-RA*(0.5*(DTD+DTE))/ave_TT)**(num_beki),&
!          & DTE,DTD,ave_TT,num_beki,RA,(-RA*(0.5*(DTD+DTE))/ave_TT)
            if((-RA*(0.5*(DTD+DTE))/ave_TT)>0.d0)then
                AKDWIL=(ave_TT*RKEV)/(16.D0*AEE*sqrt(BB**2+BPA**2)) &
            &*(sqrt(RT(NR,2)*RKEV*AMM*2.0d0)/(AEE*sqrt(BB**2+BPA**2)))/RA &
            &*(-RA*(0.5*(DTD+DTE))/ave_TT)**(gb_mu_ion)
                AKDWEL=(ave_TT*RKEV)/(16.D0*AEE*sqrt(BB**2+BPA**2)) &
            &*(sqrt(RT(NR,2)*RKEV*AMM*2.0d0)/(AEE*sqrt(BB**2+BPA**2)))/RA &
            &*(-RA*(0.5*(DTD+DTE))/ave_TT)**(gb_mu_ele)
            else
                AKDWIL=(ave_TT*RKEV)/(16.D0*AEE*sqrt(BB**2+BPA**2)) &
            &*(sqrt(RT(NR,2)*RKEV*AMM*2.0d0)/(AEE*sqrt(BB**2+BPA**2)))/RA &
            &*-(RA*(0.5*(DTD+DTE))/ave_TT)**(gb_mu_ion)
            ...
            DTD_EDGE =0.5d0*(((RT(NRMAX-edge_NR_gBgaradT,1)-RT(NRMAX-(edge_NR_gBgaradT+1),1))*DRL)+((RT(NRMAX-(edge_NR_gBgaradT),2)-RT(NRMAX-(edge_NR_gBgaradT+1) ,2))*DRL))
            TD_EDGE = 0.5d0*(((RT(NRMAX-edge_NR_gBgaradT,1)+RT(NRMAX-(edge_NR_gBgaradT+1),1))/2.d0)+((RT(NRMAX-(edge_NR_gBgaradT),2)+RT(NRMAX-(edge_NR_gBgaradT+1),2))/2.d0))
!            print'(2i5,4e20.10)',NT,NR,DTD_EDGE,TD_EDGE,RT(NRMAX,2),RT(NRMAX-1,2)
               AKDWIL=(ave_TT*RKEV)/(16.D0*AEE*sqrt(BB**2+BPA**2)) &
          &*(sqrt(RT(NRMAX-(edge_NR_gBgaradT+1),2)*RKEV*AMM*2.0d0)/(AEE*sqrt(BB**2+BPA**2)))/RA &
          &*(-RA*DTD_EDGE/TD_EDGE)**(gb_mu_ion)
!            DTD_EDGE = (RT(NRMAX-6,1)-RT(NRMAX-7 ,1))*DRL
!            TD_EDGE = (RT(NRMAX-6,1)+RT(NRMAX-7,1))/2.d0
!            print'(2i5,4e20.10)',NT,NR,DTD_EDGE,TD_EDGE,RT(NRMAX,2),RT(NRMAX-1,2)
               AKDWEL=(ave_TT*RKEV)/(16.D0*AEE*sqrt(BB**2+BPA**2)) &
          &*(sqrt(RT(NRMAX-(edge_NR_gBgaradT+1),2)*RKEV*AMM*2.0d0)/(AEE*sqrt(BB**2+BPA**2)))/RA &
          &*(-RA*DTD_EDGE/TD_EDGE)**(gb_mu_ele)
            endif
!            AKDWIL=AKDWL
!            AKDWEL=AKDWL
         case(207)
            edge_NR_gBgaradT=0
!            num_beki=2d0
            max_AKDWEL=15.d0/CK0
            max_AKDWIL=15.d0/CK1
            ave_TT=0.5d0*(TE+TD)
          if(nr==1)then
          pre_AKDWEL=0.d0
          pre_AKDWIL=0.d0
...
!          & DTE,DTD,ave_TT,num_beki,RA,(-RA*(0.5*(DTD+DTE))/ave_TT)
            ...
            if(AKDWEL>max_AKDWEL)AKDWEL=0.5d0*(pre_AKDWEL+max_AKDWEL)
            if(AKDWIL>max_AKDWIL)AKDWIL=0.5d0*(pre_AKDWIL+max_AKDWIL)
            pre_AKDWEL=AKDWEL
            pre_AKDWIL=AKDWIL
            print*,nr,AKDWEL,AKDWIL
            else
            DTD_EDGE =0.5d0*(((RT(NRMAX-edge_NR_gBgaradT,1)-RT(NRMAX-(edge_NR_gBgaradT+1),1))*DRL)+((RT(NRMAX-(edge_NR_gBgaradT),2)-RT(NRMAX-(edge_NR_gBgaradT+1) ,2))*DRL))
            TD_EDGE = 0.5d0*(((RT(NRMAX-edge_NR_gBgaradT,1)+RT(NRMAX-(edge_NR_gBgaradT+1),1))/2.d0)+((RT(NRMAX-(edge_NR_gBgaradT),2)+RT(NRMAX-(edge_NR_gBgaradT+1),2))/2.d0))
!            print'(2i5,4e20.10)',NT,NR,DTD_EDGE,TD_EDGE,RT(NRMAX,2),RT(NRMAX-1,2)
...
!            print'(2i5,4e20.10)',NT,NR,DTD_EDGE,TD_EDGE,RT(NRMAX,2),RT(NRMAX-1,2)
          ...
          print'(i5,8e20.10)',nr,rt(nr,1),rt(nr,2),ave_TT,DTE,DTD,0.5d0*(DTE+DTD),AKDWEL,AKDWIL
         case(208)
!            num_beki=1.0d0
            ave_TT=0.5d0*(TE+TD)
            edge_NR_gBgaradT=0
...
!          & DTE,DTD,ave_TT,num_beki,RA,(-RA*(0.5*(DTD+DTE))/ave_TT)
            if((-RA*(0.5*(DTD+DTE))/ave_TT)>0.d0)then
                AKDWIL=(TD*RKEV)/(16.D0*AEE*sqrt(BB**2+BPA**2)) &
            &*(sqrt(RT(NR,2)*RKEV*AMM*2.0d0)/(AEE*sqrt(BB**2+BPA**2)))/RA &
            &*(-RA*(0.5*(DTD+DTE))/ave_TT)**(gb_mu_ion)
                AKDWEL=(TE*RKEV)/(16.D0*AEE*sqrt(BB**2+BPA**2)) &
            &*(sqrt(RT(NR,2)*RKEV*AMM*2.0d0)/(AEE*sqrt(BB**2+BPA**2)))/RA &
            &*(-RA*(0.5*(DTD+DTE))/ave_TT)**(gb_mu_ele)
            else
                AKDWIL=(TD*RKEV)/(16.D0*AEE*sqrt(BB**2+BPA**2)) &
...
!            AKDWEL=AKDWL
         case(209)
             if(NRMAX >NRMAX4M_FILE_WRITE_FLGS)then
                print*,"ERROR @ M.FILE_WRITE_FLGS."
                print*,"NRMAX > NRMAX4M_FILE_WRITE_FLGS."
                print*,"CHECK size of NRMAX4M_FILE_WRITE_FLGS."
                stop
             endif
!            num_beki=1.0d0
            ave_TT=0.5d0*(TE+TD)
...
!          & (-RA*(0.5*(DTD+DTE))/ave_TT)**(num_beki),&
!          & DTE,DTD,ave_TT,num_beki,RA,(-RA*(0.5*(DTD+DTE))/ave_TT)
            if((-RA*((DTD))/TD)>0.d0)then

            tmpAKDWi_GB_0 = &
            &    CKi0 &
            &*(TD*RKEV)/(16.D0*AEE*sqrt(BB**2+BPA**2)) &
            &*(sqrt(RT(NR,2)*RKEV*AMM*2.0d0)/(AEE*sqrt(BB**2+BPA**2)))/RA
            ...
            AKDWIL=tmpAKDWi_GB_0 + tmpAKDWi_GB_1_5
                       

            tmpAKDWe_GB_0 = &
            &    CKe0  &
            &*((TE*RKEV)/(16.D0*AEE*sqrt(BB**2+BPA**2)) &
            &*(sqrt(RT(NR,2)*RKEV*AMM*2.0d0)/(AEE*sqrt(BB**2+BPA**2)))/RA)
            
!            print'(i4,5e20.10)',NR,TD,(TD*RKEV)/(16.D0*AEE*sqrt(BB**2+BPA**2)),sqrt(RT(NR,2)*RKEV*AMM*2.0d0)/(AEE*sqrt(BB**2+BPA**2))/RA ,(-RA*((DTD))/TD),(-RA*((DTD))/TD)**(gb_mu_e)
            tmpAKDWe_GB_1_5 =&
            &   CKe1_5  &
            &*((TD*RKEV)/(16.D0*AEE*sqrt(BB**2+BPA**2)) &
            &*(sqrt(RT(NR,2)*RKEV*AMM*2.0d0)/(AEE*sqrt(BB**2+BPA**2)))/RA &
            ...
            AKDWEL=tmpAKDWe_GB_0+tmpAKDWe_GB_1_5
...
!            AKDWEL=AKDWL         
         ...
         if(MDLKAI==209)then
            ...
            AKDW(NR,3)=AKDWL
            AKDW(NR,4)=AKDWL
            AKDWe_GB_0(nr)=tmpAKDWe_GB_0
            AKDWi_GB_0(nr)=tmpAKDWi_GB_0
            AKDWe_GB_1_5(nr)=tmpAKDWe_GB_1_5
            AKDWi_GB_1_5(nr)=tmpAKDWi_GB_1_5
            
         else
            AKDW(NR,1)=CK0*AKDWEL
            AKDW(NR,2)=CK1*AKDWIL
            AKDW(NR,3)=CK1*AKDWL
            AKDW(NR,4)=CK1*AKDWL
        endif
! 以下，VGRに関しては，現状，MDLKAI30番台のコピー
            FS=0.d0
            RNST2=0.0d0
            OMEGASS=0.0d0
            SLAMDA=0.0d0
            ...
            VGR4(NR,2)=1.D0/(1.D0+OMEGASS**2)
            VGR4(NR,3)=0.0d0
!  OUTPUT of about AKDW parameters 
!            open(767, file='AKDW_LOGS.log')
!               if(mod(nt,ngtstp).eq.0)then 
!               IF(NR.eq.1) write(767,'(a6,8a20)')&
!                 & 'NR','RG(NR)','AKDWe', 'AKDWi','RA','LarmorRadi',&
!                 & 'DTD','TD','DTD/TD'
!               write(767,'(i5, 8e20.10)')&
!                 & NR,rg(nr),AKDW(NR,1), AKDW(NR,2),RA,&
!                 &(sqrt(RT(NR,2)*RKEV*AMM*2.0d0)/(AEE*sqrt(BB**2+BPA**2))),&
!                 & DTD,TD,DTD/TD
!               endif
         ELSEIF((MDLKAI.GE.210).and.(MDLKAI.LT.220)) THEN
!   *********************************************************
!   ***  MDLKAI.EQ. 210   :1/n model              ***
!   ***  MDLKAI.EQ. 211   :T(r/a=0.6)^1.5/n model              ***
!   ***  MDLKAI.EQ. 212   :T^(1.5)+1/n model              ***
!   ***  MDLKAI.EQ. 213   :dn/n model              ***
!   ***  MDLKAI.EQ. 214   :(TEST) 1/(1+rho) model              ***
!   *********************************************************
         select case(MDLKAI)
         case(210)
!              1/n model
               AKDWEL=(1.0d0/(RN(nr,1)*10.0d0))
               AKDWIL=(1.0d0/(RN(nr,2)*10.0d0))
         case(211)
!              1/n model
               AKDWEL=(1.0d0/(RN(NR,1)*10.0d0))*RT(36,2)**(1.5)
               AKDWIL=(1.0d0/(RN(NR,2)*10.0d0))*RT(36,2)**(1.5)
         case(212)
!              1/n model
               AKDWEL=(1.0d0/(RN(NR,1)*10.0d0))+RT(NR,2)**(1.5)
               AKDWIL=(1.0d0/(RN(NR,2)*10.0d0))+RT(NR,2)**(1.5)
         case(213)
!              dn/n model
               if(NR.lt.NRMAX-2)then 
                  AKDWEL=(1.0d0/(RN(NR,1)*10.0d0))*-DNE
                  AKDWIL=(1.0d0/(RN(NR,2)*10.0d0))*-DNE
               else
                  DNE_EDGE = (RN(NRMAX-1,1)-RN(NRMAX-2 ,1))*DRL
                  NE_EDGE = (RN(NRMAX-1,1)+RN(NRMAX-2,1))/2.d0
                  AKDWEL=(1.0d0/(NE_EDGE*10.0d0))*-DNE_EDGE
                  AKDWIL=(1.0d0/(NE_EDGE*10.0d0))*-DNE_EDGE
               endif
         case(214)
              AKDWEL=1.d0/(1.d0 + RG(NR))**2
              AKDWIL=AKDWEL
         case(218)
             if(NRMAX >NRMAX4M_FILE_WRITE_FLGS)then
                print*,"ERROR @ M.FILE_WRITE_FLGS."
...
!          & DTE,DTD,ave_TT,num_beki,RA,(-RA*(0.5*(DTD+DTE))/ave_TT)
            ...
            &    CKe0  &
            &*((TE*RKEV)/(16.D0*AEE*sqrt(BB**2+BPA**2)) &
            &*(sqrt(RT(NR,2)*RKEV*AMM*2.0d0)/(AEE*sqrt(BB**2+BPA**2)))/RA)*(1.0d0/(RN(nr,1)*4.d0))
...
!            print'(i4,5e20.10)',NR,TD,(TD*RKEV)/(16.D0*AEE*sqrt(BB**2+BPA**2)),sqrt(RT(NR,2)*RKEV*AMM*2.0d0)/(AEE*sqrt(BB**2+BPA**2))/RA ,(-RA*((DTD))/TD),(-RA*((DTD))/TD)**(gb_mu_e)
            ...
            &*((TD*RKEV)/(16.D0*AEE*sqrt(BB**2+BPA**2)) &
            &*(sqrt(RT(NR,2)*RKEV*AMM*2.0d0)/(AEE*sqrt(BB**2+BPA**2)))/RA &
            &*(-RA*((DTD))/TD)**(gb_mu_e))*(1.0d0/(RN(nr,1)*4.d0))
...
!            print'(2i5,4e20.10)',NT,NR,DTD_EDGE,TD_EDGE,RT(NRMAX,2),RT(NRMAX-1,2)
          ...
          &     CKe0   &     
          &*((TE*RKEV)/(16.D0*AEE*sqrt(BB**2+BPA**2)) &
          &*(sqrt(RT(NRMAX-(edge_NR_gBgaradT+1),2)*RKEV*AMM*2.0d0)/(AEE*sqrt(BB**2+BPA**2)))/RA)*(1.0d0/(RN(nr,1)*4.d0))

            tmpAKDWe_GB_1_5 = &
          & CKe1_5     &
          &*((TD*RKEV)/(16.D0*AEE*sqrt(BB**2+BPA**2)) &
          &*(sqrt(RT(NRMAX-(edge_NR_gBgaradT+1),2)*RKEV*AMM*2.0d0)/(AEE*sqrt(BB**2+BPA**2)))/RA &
          &*(-RA*DTD_EDGE/TD_EDGE)**(gb_mu_e))*(1.0d0/(RN(nr,1)*4.d0))

            AKDWEL= tmpAKDWe_GB_0 + tmpAKDWe_GB_1_5

            endif              
         end select
         if(MDLKAI==218 .or. MDLKAI==219)then
            AKDW(NR,1)=AKDWEL
            AKDW(NR,2)=AKDWIL
            AKDW(NR,3)=AKDWL
            AKDW(NR,4)=AKDWL
...
!            open(767, file='AKDW_LOGS.log')
!            if(mod(nt,ngtstp).eq.0)then 
!               IF(NR.eq.1) write(767,'(a6,5a20)')&
!              & 'NR','RG(NR)','AKDWEL','AKDWIL','RN(NR,1)','RN(NR,2)'
!               write(767,'(i5, 5e20.10)')&
!              & NR,rg(nr),AKDW(NR,1), AKDW(NR,2),RN(NR,1),RN(NR,2)
!            endif
            ELSE
               WRITE(6,*) 'XX INVALID MDLKAI : ',MDLKAI
               AKDW(NR,1)=0.D0
...
!     ***********************************************************
     ...
     &                   RKEV, RN, RR, RT, ZEFF, &
     &                   MDLER,NT,NGTSTP,ADNC,AVNC,AVKNC
!!-------------------------------------
!! 2009/07/09
      use t3d_er, only : t3d_er_nccoef
!!-------------------------------------
      IMPLICIT NONE
      INTEGER(4):: NR, NS, NS1
...
!     ***** NEOCLASSICAL TRANSPORT (HINTON, HAZELTINE) *****
      IF((MDLER.NE.4).or.(MDLER.NE.5))then
      IF(MDLKNC.EQ.1) THEN
...
!      do NR=1,NRMAX
!      do NS=1,NSM
!          print *, 'IN trcoef_t3d: AK=', NR, NS, AK(NR,NS)
!      enddo 
!      enddo
!!------------------------------------
!! 2009/07/10

      if (MDLER==4 .or. MDLER==5) then
         call t3d_er_nccoef(3)
      end if
!!------------------------------------
...
!     AKLD : heat flux coefficient for density gradient
      ...
      if(MDDIAG.EQ.0)then
!      print*,'Now AKLP & AKLD are not calculated @S.TRCFNC'
      endif
      IF(MDEDGE.EQ.1) CDH=CDHSV
...
!     ***********************************************************
      ...
      USE TRCOMM, ONLY : AD, AD0, ADDW, ADDWD, ADDWP, ADLD, AKNC,ADLP, ADNC, ADNCP, ADNCT, AEE, AKDW, ALP, AME, AMM, AV, AV0,    &
...
!            print '(a20, 2i4, 3e20.10)', 'NET PARTICLE PINTCH', NR,NS,AV(NR,NS), AVDW(NR,NS), AVNC(NR,NS)
         ENDDO
      ENDDO
      IF(MDEDGE.EQ.1) CDP=CDPSV
...
!         print *, 'NOW AVKNC&AVKDW&AVK = 0 : NT='
...
!!--------------------------------------------
!! 2009/07/09
! この時点では，AD，AV，AKVはすべてゼロとなっている．
! すなわち，AD，AK，AKVには新古典由来の値のみが代入されることになる．

      if (MDLER==4 .or. MDLER==5) then
         call t3d_er_nccoef(1)
         call t3d_er_nccoef(2)
         call t3d_er_nccoef(4)
      end if
