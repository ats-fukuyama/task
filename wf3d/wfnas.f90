...
9000 call wfbdy_deallocate
     call wfnas_deallocate
RETURN
END SUBROUTINE WFNAS

!     ******* INPUT NAS ELEMENT DATA *******
...
SUBROUTINE WFRNAS(IERR)
  ...
  INTEGER,PARAMETER:: NNM=86200
  INTEGER,PARAMETER:: NEM=392200
  integer :: IERR,NB,NBP,NM,MODE_TANDL(NMM),IST,I,ISP,IDF,IDTEMP,NK
  real(8) :: PNL,PZCLL,TANDLT(NMM)
  LOGICAL LEX
...
301  IF(NNMAX.GE.NNM) THEN
     ...
     READ(26,'(10A8)',ERR=9100,END=9000) & 
          &        K0,K1,K2,K3,K4,K5,K6,K7,K8,K9
!    WRITE(6,'(A,2I8,1P3E12.4)') &
!     &   ' --- NODE ',NNMAX,IDND(NNMAX), &
!     &                XND(NNMAX),YND(NNMAX),ZND(NNMAX)
...
!    WRITE(6,'(A,3I8,4I8)') &
!     &   ' --- ELM  ',NEMAX,IDELM(NEMAX),KAELM(NEMAX),&
!     &        (NDELM(I,NEMAX),I=1,4)
     READ(26,'(10A8)',ERR=9100,END=9000) &
          &        K0,K1,K2,K3,K4,K5,K6,K7,K8,K9
...
502        READ(26,'(10A8)',ERR=9100,END=9000) &
        ...
        IF(MODE_TANDL(NMMAX).EQ.1) THEN
           WRITE(6,'(A,I5,1P3E12.4)') ' --- NM,EPS,AMU,SIG=',&
                & NMMAX,EPSDM(NMMAX),AMUDM(NMMAX),SIGDM(NMMAX)
        ELSE
           WRITE(6,'(A,I5,1P3E12.4)') ' --- NM,EPS,AMU,TDL=',&
...
END SUBROUTINE WFRNAS

!     ******* MODIFY NAS ELEMENT DATA *******

SUBROUTINE WFMNAS(IERR)
  
  use wfcomm
  implicit none
  integer :: IERR,NN,I,ID,NB,NBP,NE,ND,NK1,NM,N,KA,NN1,NN2,NN3
  real(8) :: V,XC,YC,ZC,ANX1,ANY1,ANZ1,AN1,ZN1,ANX2,ANY2,ANZ2,AN2
  real(8) :: ANX3,ANY3,ANZ3,SIZE1,SIZE2,SIZE1L,SIZE2L,AN3,SIZEL

  EXTERNAL WFSRTS,WFSRTX

  IERR=0
  
!     ----- MULTIPLY UNIT LENGTH -----
...
!     ----- PREPARE TO REPLACE NODE INDEX BY NODE NUMBER -----
...
!     ----- PREPARE TO REPLACE ELM INDEX BY ELM NUMBER -----
...
!     ----- REPLACE ELM INDEX BY ELM NUMBER -----
...
!     ----- Modify element numbers in boundary block -----
...
!     ----- Modify material number of element -----
...
!!!!
!!!! Assuming second vector is always given
!!!!
           ANX1=EX1WG(NB)
           ANY1=EY1WG(NB)
           ANZ1=EZ1WG(NB)
        ...
        ELSE
!!!!
!!!! Assuming first vector is always given
!!!!
           ANX1=EX1WG(NB)
           ANY1=EY1WG(NB)
           ANZ1=EZ1WG(NB)
