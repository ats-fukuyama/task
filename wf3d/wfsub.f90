...
SUBROUTINE WFVLIM
  ...
  if(MODELG.eq.1) then
     ...
     CALL RCTORT(XND(1),YND(1),ZND(1),XT,YT,ZT)
     XNDMIN=XT
     XNDMAX=XT
     YNDMIN=YT
     YNDMAX=YT
     ...
     DO IN=2,NNMAX
        ...
        XNDMIN=MIN(XNDMIN,XT)
        XNDMAX=MAX(XNDMAX,XT)
        YNDMIN=MIN(YNDMIN,YT)
        YNDMAX=MAX(YNDMAX,YT)
        ZNDMIN=MIN(ZNDMIN,ZT)
        ZNDMAX=MAX(ZNDMAX,ZT)
        RND   =SQRT(XT**2+YT**2)
...
! *********** R in cartesian to R in torus ********
SUBROUTINE RCTORT(XC,YC,ZC,XT,YT,ZT)

  use wfcomm
  implicit none

  real(8) :: XC,YC,ZC,XT,YT,ZT,PHI
  ...
  return
END SUBROUTINE RCTORT
! ************ R in torus to R in cartesian ****
SUBROUTINE RTTORC(XT,YT,ZT,XC,YC,ZC)
  ...
  return
END SUBROUTINE RTTORC

! ************ A in torus to A in cartesian ****                                          
SUBROUTINE ATTOAC(ATX,ATY,ATZ,PHI,ACX,ACY,ACZ)

  use wfcomm
  implicit none

  real(8) :: ATX,ATY,ATZ,PHI,ACX,ACY,ACZ

  ACX=COS(PHI)*ATX-SIN(PHI)*ATZ
  ACY=ATY
  ACZ=SIN(PHI)*ATX+COS(PHI)*ATZ

  return
END SUBROUTINE ATTOAC
