...
!-------------------------------------------------------------------------------
  ...
  subroutine fp_chieff
    ...
    ps(:,:)=rspb(:,:)+rspf(:,:)+rsps(:,:)+rspl(:,:)+rsps_cx(:,:)+rpcs(:,:)&
            +rpss(:,:)! &
...
close(1010)
   ...
   CALL PAGES
      CALL GRD1D(1,RM,tps,NRMAX,NRMAX,NSAMAX,'@Particle Source vs rho 10e20 /m3 @')
      CALL GRD1D(2,RM,inPower,NRMAX,NRMAX,NSAMAX,'@Input Power vs rho 10e20 /m2*t @')
      ! CALL GRD1D(5,RG,GAMMAP,NRMAX,NRMAX,NSAMAX,'@Gammap vs rho 10e20 /m2*t @')
      ! CALL GRD1D(6,RG,hf,NRMAX,NRMAX,NSAMAX,'@hf vs rho 10e20 /m2*t @')
      ! CALL GRD1D(7,RG,dtdr,  NRMAX,  NRMAX,  NSAMAX,'@dtdr vs rho 10e20 /m3@')
      ! CALL GRD1D(8,RG,keff,NRMAX,NRMAX,NSAMAX,'@chieff vs rho 10e20 /m2*t @')
      ! CALL GRD1D(8,RG,deff,NRMAX,NRMAX,NSAMAX,'@deff vs rho 10e20 /m2*t @')
      ! CALL GRD1D(9,RG,dndr,  NRMAX,  NRMAX,  NSAMAX,'@dndr vs rho 10e20 /m3@')
   CALL PAGEE

   call fpcsv_diffus(deff,chieff,keff,gammap,hf,temps*mj2kev,ps)
