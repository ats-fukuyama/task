...
!     ***********************************************************
      ...
      USE TRCOMM, ONLY : &
           GRM, GYR, MDLEQ0, MDLNF, NFM, NRMAX, NRMP, NSM, RKEV, RN, RNF, RT
      IMPLICIT NONE
      INTEGER(4),INTENT(IN) :: INQ
      INTEGER(4) :: NF, NR, NS
      REAL(4)    :: GUCLIP
...
!     ***********************************************************
      ...
      USE TRCOMM, ONLY : &
           GRM, GYR, NFM, NRMAX, NRMP, NSM, PBCL, PBIN, PFCL, PFIN, RKEV, &
           RTF, RW
      IMPLICIT NONE
      INTEGER(4),INTENT(IN) :: INQ
      INTEGER(4) :: NF, NR, NS
...
!     ***********************************************************
      ...
      USE TRCOMM, ONLY : &
           AJ, AJBS, AJNB, AJOH, AJRF, ETA, EZOH, GRG, GRM, GYR, &
           NRMAX, NRMP, Q0, QP
      IMPLICIT NONE
      INTEGER(4),INTENT(IN) :: INQ
      INTEGER(4) :: NR
...
!     ***********************************************************
      ...
      USE TRCOMM, ONLY : GRM, GYR, NRMAX, NRMP, NSM, PIN, &
     ...
     &                   NSMAX, PNSS, RGFLS, RHOG, RHOM, RJCB, RN, TAUB, TAUF
      ...
      ELSE
         ...
         IF(MODE.NE.0) THEN
            ...
            ENDDO
            DO NS=1,NSMAX
               DNN(1:NRMAX-1,NS)=(RN(1:NRMAX-1+1,NS)-RN(1:NRMAX-1,NS)) &
                                *RJCB(1:NRMAX-1)/DR
            ENDDO
            NR=NRMAX
            DO NS=1,NSMAX
            ...
            ENDDO
            ADNCG(1:NRMAX,1:NSMAX)=-RGFLSUM(1:NRMAX,1:NSMAX) &
                                   /DNN(1:NRMAX,1:NSMAX)
         ENDIF
...
!     ***********************************************************
      ...
      USE TRCOMM, ONLY : &
           BETA, BETA0, BETAL, BETAP, BETAP0, BETAPL, BETAQ, BETAQ0, &
           GRG, GRM, GYR, NRMAX, NRMP, NSM, PCX, PEX, PIE, PNB, PNF, &
           POH, PRF, PRL
      IMPLICIT NONE
      INTEGER(4),INTENT(IN) :: INQ
      INTEGER(4) :: NS, NR
...
!     ***********************************************************
      ...
      CALL TRGR1DD( 3.0,12.0,11.0,17.0,GBL,GYBLA,GYBLB,NLM,NLMAX(NA),1,3, &
                   KFID,3+INQ,2+INQ)
      ...
      WRITE(KRTG,'(F5.3)') RTG(NA)
      KFID='@(L)R, (R)RHO, P1*10$+2$=, ANL vs r, RTG='//KRTG//' m@'
      CALL TRGR1DD(15.5,24.5,11.0,17.0,GBL,GYBLA,GYBLB,NLM,NLMAX(NA),1,3, &
                   KFID,3+INQ,2+INQ)
      ...
      WRITE(KRTG,'(F5.3)') RTG(NA)
      KFID='@(L)R, (R)RHO, P1*10$+2$=, ANL vs r, RTG='//KRTG//' m@'
      CALL TRGR1DD( 3.0,12.0, 2.0, 8.0,GBL,GYBLA,GYBLB,NLM,NLMAX(NA),1,3, &
                   KFID,3+INQ,2+INQ)
      ...
      WRITE(KRTG,'(F5.3)') RTG(NA)
      KFID='@(L)R, (R)RHO, P1*10$+2$=, ANL vs r, RTG='//KRTG//' m@'
      CALL TRGR1DD(15.5,24.5, 2.0, 8.0,GBL,GYBLA,GYBLB,NLM,NLMAX(NA),1,3, &
                   KFID,3+INQ,2+INQ)
