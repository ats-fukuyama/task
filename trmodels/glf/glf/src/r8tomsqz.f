!clf -- 03/12/2010: replaced double precision with real(kind=rspec)
!                   "D" exponent with e" _rspec"
!
...
C-----------------------------------------------------------------------
      ...
      INTEGER N, NA
      real(KIND=rspec) AR(N,NA),AI(N,NA),BR(N,NA),BI(N,NA)
 
C-----------------------------------------------------------------------
C DECALRATIONS FOR OUTPUT VARIABLES
C-----------------------------------------------------------------------
 
      real(KIND=rspec) ALFR(N),ALFI(N),BETA(N)
      real(KIND=rspec) ZVR(N,NA), ZVI(N,NA)
      INTEGER IFAIL
...
C-----------------------------------------------------------------------
      ...
      LOGICAL WANTX
      real(KIND=rspec) EPS1
      real(KIND=rspec) ZERO,ZONE
...
C
      ...
      real(KIND=rspec) AR(NM,N),AI(NM,N),BR(NM,N),BI(NM,N),
     &       ZR(NM,N),ZI(NM,N)
      real(KIND=rspec) R,S,T,TI,U1,U2,XI,XR,YI,YR,RHO,U1I
      LOGICAL MATZ
      real(KIND=rspec) ZERO,ZONE
C
...
C
     ...
     X       BETA(N),ZR(NM,N),ZI(NM,N)
      real(KIND=rspec) R,S,A1,A2,EP,SH,U1,U2,XI,XR,YI,YR,
     ...
     X       EPSA,EPSB,EPS1,ANORM,BNORM,B3344,B3344I
      INTEGER max
      LOGICAL MATZ
      complex(KIND=rspec) Z3
C
      real(KIND=rspec) ZERO,ZONE,ZTWO
C
C
C
C
...
C
   ...
   30 CONTINUE
C
      IF (ANORM .EQ. ZERO) ANORM = 1.0e0_rspec
      IF (BNORM .EQ. ZERO) BNORM = 1.0e0_rspec
      EP = EPS1
...
C     ********** DETERMINE SHIFT **********
  ...
  124 B3344 = B33 * B44 - B33I * B44I
     ...
     X     - AR(NA,NA) * BI(NA,EN) - AI(NA,NA) * BR(NA,EN)
      ...
      YR = (A33 - SH) / 2.0e0_rspec
      YI = (A33I - SHI) / 2.0e0_rspec
      Z3 = sqrt(CMPLX(YR**2-YI**2+XR,2.0e0_rspec*YR*YI+XI))
      U1 = REAL(Z3,KIND=rspec)
      U1I = AIMAG(Z3)
...
C
         IF (K .EQ. L) GO TO 240
         AI(K,KM1) = 0.0e0_rspec
         AR(K1,KM1) = 0.0e0_rspec
         AI(K1,KM1) = 0.0e0_rspec
C     ********** ZERO B(K+1,K) **********
...
C
     ...
     X       ALFR(N),ALFI(N),
     X       BETA(N),ZR(NM,N),ZI(NM,N)
      real(KIND=rspec) R,T,RI,TI,XI,ALMI,ALMR,BETM,EPSB
      COMPLEX(KIND=rspec) Z3
      real(KIND=rspec) ZERO
C
C
C
C
