...
!--------------------------------------------------
  ...
  SUBROUTINE DPFPGRA(PMAX,NTHM,NPMAX,NTHMAX,P,TH,F,STRING)
    ...
    USE bpsd_kinds,ONLY: rkind
      IMPLICIT NONE
      REAL(rkind),INTENT(IN):: PMAX
      INTEGER,INTENT(IN):: NTHM,NPMAX,NTHMAX
      real(rkind),dimension(NPMAX),INTENT(IN):: P
      real(rkind),dimension(NTHMAX),INTENT(IN):: TH
      real(rkind),DIMENSION(NTHM,NPMAX),INTENT(IN):: F
      ...
      INTEGER:: ILN(NGLM)
      real:: GPMAX, GPMIN1, GPMAX1, GPSTEP
      real:: GFMIN, GFMAX, GFMIN1, GFMAX1, GFSTEP
      integer:: NP,NTH
      integer:: NGLMAX,NGL
      REAL:: GUCLIP
      INTEGER:: NGULEN
      EXTERNAL PAGES,SETLNW,SETCHS,SETFNT,GMNMX2,GQSCAL,PAGEE
      EXTERNAL GDEFIN,GFRAME,GSCALE,GVALUE
      EXTERNAL SETLIN,MOVE,TEXT,NUMBR
...
!     ****** CONTOUR PLOT : R-THETA, VARIABLE STEP, PATTERN ******
  ...
  SUBROUTINE CONTG4X(Z,R,T,NXA,NXMAX,NYMAX, &
    ...
    REAL,INTENT(IN):: Z(NXA,NYMAX),R(NXMAX),T(NYMAX)
    REAL,INTENT(IN):: ZL(NSTEP),RGB(3,NSTEP),WLN(NSTEP)
    INTEGER,INTENT(IN):: ILN(NSTEP)
    COMMON /GSCTR4/ RMAX,RT,TT,XT,YT
    REAL:: RMAX,RT,TT,XT,YT
    EXTERNAL INQRGB,INQLNW,SETRGB
...
!     ****** CONTOUR PLOT : COMMON SUB ******
  ...
  SUBROUTINE CONTG0X(Z,X,Y,NXA,NXMAX,NYMAX, &
    ...
    REAL,INTENT(IN):: Z(NXA,NYMAX),X(NXMAX),Y(NYMAX)
    REAL,INTENT(IN):: ZL(NSTEP),RGB(3,NSTEP),WLN(NSTEP)
    INTEGER,INTENT(IN):: ILN(NSTEP)
    INTEGER:: KA(2,NXMAX*NYMAX)
    EXTERNAL SUBV
    INTEGER,PARAMETER:: NH=101
    REAL:: ZLS(NH),RGBS(3,NH),WLNS(NH)
    INTEGER:: ILNS(NH)
!      PARAMETER (NFMAX=2000,NGMAX=4000)
    INTEGER,PARAMETER:: NFMAX=200,NGMAX=400
    REAL:: XF(NFMAX),YF(NFMAX)
    REAL:: XP(NFMAX),YP(NFMAX)
    REAL:: XG(NGMAX),YG(NGMAX)
    REAL:: RS,GS,BS,WS,ZORG,U0,U1,U2,U3,U4,UMAX,UMIN
    REAL:: XA,XB,YA,YB,RT
    INTEGER:: KMAX,I,NXM,NYM,NX,NY,IE,NXP,NYP,K,J,NF,NP,NN
    INTEGER:: IEL,NXL,NYL,NXL1,NYL1,IE1
    INTEGER:: N1X,N1Y,N2X,N2Y,N3X,N3Y,N4X,N4Y,I2X,I2Y,I3X,I3Y,I4X,I4Y
    INTEGER:: NAX,NAY,NBX,NBY,UA,UB,MODE
    INTEGER:: NSAX,NSAY,NSBX,NSBY,USA,USB,MODES,MODEL
    LOGICAL:: LINV,LEND
    EXTERNAL INQLNW,INQRGB,SETLNW,SETRGB
...
!     ****** CONTOUR PLOT SLAVE ROUTINE : CONV RT ******
  ...
  SUBROUTINE CONTV2X(RA,TA,N,XB,YB,M,NN)
    ...
    REAL,INTENT(IN):: RA(N),TA(N)
    REAL,INTENT(OUT):: XB(M),YB(M)
    COMMON /GSGFXY/ DX,DY,PXS,PYS,PXE,PYE,GXS,GYS,GXE,GYE,LGF
    REAL:: DX,DY,PXS,PYS,PXE,PYE,GXS,GYS,GXE,GYE
    LOGICAL:: LGF
    COMMON /GSCTR4/ RMAX,RT,TT,XT,YT
    REAL:: RMAX,RT,TT,XT,YT
    INTEGER:: J,I,IMAX,K
    REAL:: RS,TS,XS,YS,DELR,DELT,RI,TI,XI,YI,X0,Y0,S
...
!     ****** DRAW LINES WITH PATTERN ******
  ...
  SUBROUTINE LINEPTX(XG,YG,N,IPAT)
    ...
    IMPLICIT NONE
    INTEGER,INTENT(IN):: N,IPAT
    REAL,INTENT(IN):: XG(N),YG(N)
    INTEGER:: I
    EXTERNAL MOVEPT,DRAWPT
...
!     ****** SPLINE INTERPOLATION OF 2D LINES ******
  ...
  SUBROUTINE GUSP2DX(XH,YH,N,XP,YP,NPM,NP,ISPL)
    ...
    IMPLICIT NONE
    INTEGER,INTENT(IN):: N,NPM,ISPL
    INTEGER,INTENT(OUT):: NP
    REAL,INTENT(IN):: XH(N),YH(N)
    REAL,INTENT(OUT):: XP(NPM),YP(NPM)
    INTEGER,PARAMETER:: NPA=2001
    INTEGER,PARAMETER:: M=3
    INTEGER:: IKN(0-M:NPA+M)
    INTEGER:: I,NQ,IOC
...
!     ****** SPLINE ******
  ...
  SUBROUTINE GUCSPLX(N,X,Y,IKN,IOC,NP,XP,YP)
    ...
    IMPLICIT NONE
    INTEGER,INTENT(IN):: N,NP,IOC
    INTEGER,PARAMETER:: M=3
    INTEGER,INTENT(IN):: IKN(0-M:N+M)
    REAL,INTENT(IN):: X(0:N),Y(0:N)
    REAL,INTENT(OUT):: XP(NP),YP(NP)
    REAL:: B(0-M:M,0:M)
    REAL:: H,TP,XV,YV
    INTEGER:: J,ITM,I,IV
...
!     ****** B SPLINE ******
  ...
  SUBROUTINE GUBSPLX(TP,ITM,N,IKN,M,B)
    ...
    INTEGER,INTENT(IN):: N,M
    INTEGER,INTENT(IN):: IKN(0-M:N+M)
    INTEGER,INTENT(OUT):: ITM
    REAL,INTENT(OUT):: B(0-M:M,0:M)
    INTEGER:: JT,K,I,IV
    REAL:: B0,B1
