...
! --- input paramters ---
  ...
  INTEGER:: NSA_NS_dp(NSM)
  REAL(rkind):: PMAX_dp(NSM),EMAX_dp(NSM),RHON_MIN(NSM),RHON_MAX(NSM)
  ...
  SUBROUTINE dpfp_allocate
    ...
    ELSE
       IF((NTHMAX_DP.EQ.NTHMAX_DP_SAVE).AND. &
          ...
          (NSAMAX_DP.EQ.NSAMAX_DP_SAVE).AND. &
          (NSMAX.EQ.NSMAX_SAVE)) RETURN
       CALL dpfp_deallocate
    ENDIF

    ALLOCATE(NS_NSA_DP(NSAMAX_DP))
    ALLOCATE(AEFP(NSAMAX_DP),AMFP(NSAMAX_DP),RNFP0(NSAMAX_DP),RTFP0(NSAMAX_DP))
    ALLOCATE(DELP(NSAMAX_DP))
...
!    ALLOCATE(CHI(NCHMAX),CCHI(NCHMAX),SCHI(NCHMAX))
    ...
    NTHMAX_DP_SAVE=NTHMAX_DP
    NPMAX_DP_SAVE=NPMAX_DP
    NRMAX_DP_SAVE=NRMAX_DP
    NSAMAX_DP_SAVE=NSAMAX_DP
    NSMAX_SAVE=NSMAX
    RETURN
  END SUBROUTINE dpfp_allocate

  SUBROUTINE dpfp_deallocate
    DEALLOCATE(NS_NSA_DP,AEFP,AMFP,RNFP0,RTFP0,DELP)
    DEALLOCATE(FNS)
    DEALLOCATE(FP)
