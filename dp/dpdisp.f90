...
!     ****** CALCULATE DETERMINANT OF DISPERSION TENSOR ******
  ...
  FUNCTION CFDISP(CRF,CKX,CKY,CKZ,XPOS,YPOS,ZPOS)
    ...
    CASE(1)
       ...
       CKYN=CKY/RK
       CKZN=CKZ/RK
       CFDISP=CKXN*(CDTNS(1,1)*CKXN+CDTNS(1,2)*CKYN+CDTNS(1,3)*CKZN) &
             +CKYN*(CDTNS(2,1)*CKXN+CDTNS(2,2)*CKYN+CDTNS(2,3)*CKZN) &
             +CKZN*(CDTNS(3,1)*CKXN+CDTNS(3,2)*CKYN+CDTNS(3,3)*CKZN)
    END SELECT
    RETURN
  END FUNCTION CFDISP
