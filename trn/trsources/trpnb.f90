...
! ***********************************************************
  ...
  INTEGER(ikind),DIMENSION(2) :: nsa_beam

CONTAINS

  SUBROUTINE tr_power_nb

    USE trcomm, ONLY : mdlnb, nrmax, snb, taub
    IMPLICIT NONE

    ! identify the species heated by NBI --> nsaf_nsa, nsab_nsaf

    SELECT CASE(mdlnb)
    CASE(0)
       taub(0:nrmax) = 0.d0
    CASE(1) ! no particle source
       CALL tr_nb_gaussian
       snb(1:nsamax,0:nrmax) = 0.d0
       CALL tr_nb_current
    ...
    RETURN
  END SUBROUTINE tr_power_nb

! *************************************************************************

  SUBROUTINE tr_nb_gaussian
! -------------------------------------------------------------------------
!   Neutral Beam Injection : (Gaussian profile)
! -------------------------------------------------------------------------
    USE trcomm, ONLY : rkev,nrmax,dvrho,rmnrho, &
                       pnb,pnbeng,pnbr0,pnbrw, pnbtot,snb
    IMPLICIT NONE
    INTEGER(ikind):: nr
    REAL(rkind)   :: pnb0, sumnb
    ...
    DO nr = 1, nrmax
       ...
       pnbm = DEXP(-((rmnrho(nr-1)-pnbr0)/pnbrw)**2)*dvrho(nr-1)
       pnbp = DEXP(-((rmnrho(nr  )-pnbr0)/pnbrw)**2)*dvrho(nr  )

       sumnb = sumnb + 0.5d0*(pnbm + pnbp)*drhog
    ENDDO
    
    pnb0 = pnbtot*1.d6 / sumnb

    DO nr = 0, nrmax
       pnb(2,nr) = pnb0 * DEXP(-((rmnrho(nr)-pnbr0)/pnbrw)**2)
       snb(2,nr) = pnb(nr)/(pnbeng*rkev)*1.d-20
    ENDDO

    RETURN
  END SUBROUTINE tr_nb_gaussian


  SUBROUTINE tr_nb_pencilb
! -------------------------------------------------------------------------
!   Neutral Beam Injection : (pencil beam)
! -------------------------------------------------------------------------
    USE trcomm, ONLY : rkev,nrmax,nsamax,pnb,pnbeng,pnbrtg,pnbrw,pnbtot, &
                       pnbvw,pnbvy,snb, RTG! ??
    IMPLICIT NONE
    INTEGER(ikind) :: i, j, nrnbmax
    REAL(rkind)    :: drtg, dvy, rdd, vy
    REAL(rkind),DIMENSION(10) ::                     &
         ar = (/0.02D0,0.06D0,0.12D0,0.14D0,0.16D0,  &
                0.16D0,0.14D0,0.12D0,0.06D0,0.02D0/)

    IF(pnbtot <= 0.D0) RETURN

    nrnbmax = 10
    snb(1:nsamax,1:nrmax) = 0.D0
    ...
    pnb(2,1:nrmax) = snb(1:nrmax)*1.d20*pnbeng*rkev

    RETURN
  END SUBROUTINE tr_nb_pencilb
...
! -------------------------------------------------------------------------
    USE TRCOMM, ONLY : ANC, ANFE, DR, DVRHO, &
...
! *************************************************************************
  ...
  SUBROUTINE tr_nb_current
! -------------------------------------------------------------------------
!   Neutral Beam driven current
! -------------------------------------------------------------------------
    USE trcomm, ONLY : AEE, AJNB, AME, AMM, ANC, ANFE, EPS0, EPSRHO, MDLUF, NRMAX, PA, PBCL, PBIN, PI, PNBCD, PNBENG, &
     &      PZ, PZC, PZFE, RKEV, RN, RNF, RT, RTF, RW, TAUB, ZEFF
    IMPLICIT NONE
    REAL(rkind)    :: AMA,AMB,AMD,AMT, ANE, COULOG, EC, EPS, HY, HYB, P2, P3, P4, PAB, PZB, TAUS, TAUS0, TE, VB, VC3,  &
