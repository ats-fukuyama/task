...
!!$          limtx(1,1,neq) = dh0*4.d0*coef1*ldrm
!!$          limtx(2,1,neq) = dh0*2.d0*coef1*ldrm
!!$          limtx(1,2,neq) = dh0*2.d0*coef1*ldrp
!!$          limtx(2,2,neq) = dh0*4.d0*coef1*ldrp
...
! ---------------------------------------------------------------------------
    ...
    IF(neq == 1)THEN ! magnetic field: d psi/d rho
       dvdrp = 1.d0
       dvdrm = 1.d0
       gm2p  = dvrho(nr  ) * abrho(nr  ) / ttrho(nr  ) ! C1 i+1
       gm2m  = dvrho(nr-1) * abrho(nr-1) / ttrho(nr-1) ! C1 i
       coef1 = 1.d0
       coef2 = 1.d0
    ...
    ELSE IF(MOD((neq-1),3) == 2)THEN ! density
       dvdrp = dvrho(nr)
       dvdrm = dvrho(nr-1)
       gm2p  = dvrho(nr  )*ar2rho(nr  )
       gm2m  = dvrho(nr-1)*ar2rho(nr-1)
       coef1 = 1.d0
       coef2 = 1.d0
       cjexp = 0.d0
       cjexm = 0.d0
    ...
    ELSE IF(MOD((neq-1),3) == 0)THEN ! temperature
       ...
       coef2 = 1.5d0 ! the coefficient of heat pinch term
       cjexp = 0.d0
       cjexm = 0.d0
    END IF
